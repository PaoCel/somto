...
final class WatchlistWidgetSnapshotTests: XCTestCase {
    ...
    func testUnaPiattaformaSenzaRicercaFunzionanteRipiegaSuTmdb() {
        // REGRESSIONE — i modelli di Disney+, NOW, Sky Go, Mediaset Infinity e
        // discovery+ rispondevano 404 (verificato l'8/9/2026): il tasto portava
        // sulla pagina di errore della piattaforma, che da fuori sembra un bug
        // di Somto. Meglio un tap in piu' su TMDB che un tap su un 404.
        for nome in ["Disney Plus", "Now TV", "Sky Go", "Mediaset Infinity", "discovery+", "TIMVISION"] {
            let destination = StreamingDestinationResolver.destination(
                providerName: nome,
                deepLinks: [:],
                titleName: "Un titolo",
            ...
            XCTAssertEqual(destination?.source, .tmdbWatchPage, "\(nome) doveva ripiegare su TMDB")
        }
    }

    func testAppleTvCercaNelCatalogoItaliano() {
        // Senza il paese Apple redirige su /us/, che qui non si puo' guardare.
        let destination = StreamingDestinationResolver.destination(
        ...
        XCTAssertEqual(destination?.url.absoluteString, "https://tv.apple.com/it/search?term=Severance")
    }
    ...
    func testLaMigliorePiattaformaNonPerdeIlCanaleAmazon() {
        // REGRESSIONE — `ranked` collassa "MGM Plus Amazon Channel" in "MGM
        // Plus", che non e' fra i modelli di ricerca: risolvendo sul nome
        // collassato il tasto finiva sulla pagina TMDB invece che dentro Prime
        // Video, dove quel marchio si guarda davvero. `destination(providerName:)`
        // faceva la cosa giusta, `best(providerNames:)` no.
        let best = StreamingDestinationResolver.best(
        ...
        XCTAssertEqual(best?.source, .providerSearch)
        XCTAssertTrue(best?.url.absoluteString.hasPrefix("https://www.primevideo.com/") ?? false)
        // Ma il nome mostrato resta corto: "MGM Plus Amazon Channel" e' il nome
        // di un abbonamento, non di un'app.
        XCTAssertEqual(best?.providerName, "MGM Plus")
    }

    func testIlNomeMostratoEQuelloCorto() {
        let apple = URL(string: "https://tv.apple.com/show/umc.cmc.vtoh0mn0xn7t3c643xqonfzy")!
        let destination = StreamingDestinationResolver.destination(
            providerName: "Apple TV Amazon Channel",
            deepLinks: ["Apple TV": apple],
    ...
    // MARK: - Il widget usa i link diretti che il server ha gia' risolto

    func testIlWidgetPrendeIlLinkDirettoDalTitolo() {
        // REGRESSIONE — il costruttore dello snapshot metteva `watchUrl: nil`
        // sempre: il tasto play del widget ripiegava sulla ricerca anche sui
        // titoli per cui `titles.watchDeepLinks` esisteva gia' in Firestore.
        let netflix = URL(string: "https://www.netflix.com/title/81234567")!
        let title = Self.title(
            providerNames: ["Netflix"],
            deepLinks: ["Netflix": netflix.absoluteString]
        )

        XCTAssertEqual(WatchlistWidgetSnapshotBuilder.directWatchLink(for: "Netflix", in: title), netflix)
    }

    func testIlLinkDirettoDelWidgetSiTrovaAncheColNomeLungo() {
        // Il nome mostrato e' gia' collassato ("Apple TV"), la chiave salvata
        // dal server puo' essere quella lunga o viceversa.
        let apple = URL(string: "https://tv.apple.com/show/umc.cmc.vtoh0mn0xn7t3c643xqonfzy")!
        let title = Self.title(
            providerNames: ["Apple TV Amazon Channel"],
            deepLinks: ["Apple TV Amazon Channel": apple.absoluteString]
        )

        XCTAssertEqual(WatchlistWidgetSnapshotBuilder.directWatchLink(for: "Apple TV", in: title), apple)
    }

    func testSenzaLinkPerQuellaPiattaformaIlWidgetNonNeInventaUnAltro() {
        // Un logo solo: mandare su Disney+ chi ha toccato play sotto il logo
        // Netflix e' peggio di una ricerca.
        let disney = URL(string: "https://www.disneyplus.com/movies/wd/3sAcYG9CeWFw")!
        let title = Self.title(
            providerNames: ["Netflix", "Disney Plus"],
            deepLinks: ["Disney Plus": disney.absoluteString]
        )

        XCTAssertNil(WatchlistWidgetSnapshotBuilder.directWatchLink(for: "Netflix", in: title))
    }

    func testIlWidgetTieneIlNomeLungoPerLaRicerca() {
        // REGRESSIONE — il riassunto salvava solo il nome collassato, e con
        // quello "MGM Plus Amazon Channel" diventa "MGM Plus": un marchio che
        // non sappiamo aprire, quindi pagina TMDB invece della ricerca dentro
        // Prime Video, che e' l'app dove si guarda. 837 titoli del catalogo
        // l'8/9/2026.
        let title = Self.title(providerNames: ["MGM Plus Amazon Channel"], deepLinks: [:])
        let state = Self.state(id: "t", name: "Un titolo", isSeries: true, progress: .empty, title: title)
        let snapshot = WatchlistWidgetSnapshotBuilder.snapshot(
            from: Self.dashboard(inProgressSeries: [state], generalWatchlist: [])
        )

        let row = snapshot.rows.first
        XCTAssertEqual(row?.providerName, "MGM Plus")
        XCTAssertEqual(row?.providerSearchName, "MGM Plus Amazon Channel")

        // E col nome lungo il resolver trova Prime Video, come sulla scheda.
        let destination = StreamingDestinationResolver.destination(
            providerName: row?.providerSearchName ?? "",
            deepLinks: [:],
            titleName: "Un titolo",
            tmdbId: 1234,
            isSeries: true
    ...
    func testLaMappaDeiLinkDirettiSiLeggeDaFirestore() {
        let parsed = TitleParsing.parseWatchDeepLinks(from: [
            "watchDeepLinks": [
                "Netflix": "https://www.netflix.com/title/81234567",
                // Righe non valide: si scartano invece di rompere il parsing.
                "Vuoto": "",
                "Non stringa": 42,
                "": "https://example.com",
            ],
        ...
        XCTAssertEqual(parsed["Netflix"]?.absoluteString, "https://www.netflix.com/title/81234567")
        XCTAssertTrue(TitleParsing.parseWatchDeepLinks(from: [:]).isEmpty)
    }
    ...
    /// Un titolo come lo costruisce il parser da Firestore: si passa dal doc
    /// vero e proprio invece che dall'init, cosi' il test copre anche la
    /// decodifica di `watchDeepLinks`.
    ...
    private static func state(
        ...
        title titleOverride: Title? = nil
    ) -> TitlePersonalState {
        let title = titleOverride
            ?? name.map { TitleParsing.title(from: ["name": $0, "type": isSeries ? "tv" : "movie"], documentID: id) }
