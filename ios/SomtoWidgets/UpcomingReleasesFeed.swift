...
enum UpcomingReleasesFeedLoader {
    ...
    static func loadImageData(for url: URL) async -> Data? {
        // Prima il disco: una locandina gia' vista non vale un'altra richiesta,
        // e la rete e' l'unica ragione per cui ridisegnare il widget si sente.
        if let cached = WidgetImageCache.data(for: url) { return cached }
        do {
            ...
            WidgetImageCache.store(data, for: url)
            return data
        } catch {
