...
struct MarkEpisodeWatchedIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        // Ridisegna SUBITO, prima della richiesta: il giro di rete puo' durare
        // secondi e senza questo il widget resta identico per tutto il tempo.
        // E' il difetto che porta a toccare due volte — "non e' successo
        // niente" — e la guardia sopra lo ferma, ma la sensazione di rotto
        // resta finche' nessuno dice che sta lavorando.
        reloadWidgets()
...
/// I due widget che mostrano lo stesso progresso: quando cambia, cambiano
/// insieme.
func reloadWidgets() {
...
struct EpisodeCheckProvider: AppIntentTimelineProvider {
    ...
    func timeline(for configuration: SelectSeriesIntent, in context: Context) async -> Timeline<EpisodeCheckEntry> {
        ...
        // Riassunto presente ma illeggibile (prima del primo sblocco dopo un
        // riavvio): si ritenta presto, come il widget Watchlist.
        ...
        return Timeline(
            entries: [entry],
            policy: .after(entry.date.addingTimeInterval(interval))
        )
    }
    ...
    private static func makeEntry(for configuration: SelectSeriesIntent) async -> EpisodeCheckEntry {
        ...
        return EpisodeCheckEntry(date: now, state: .series(EpisodeCheckRow(
            ...
            pending: WatchlistWidgetPendingStore.displayState(for: chosen.id),
            deepLink: deepLink,
...
private struct EpisodeCheckButton: View {
    ...
    var body: some View {
        Button(intent: MarkEpisodeWatchedIntent(titleId: row.id)) {
            Label(row.buttonLabel, systemImage: row.buttonIcon)
                .font(.caption)
                .fontWeight(.semibold)
                .lineLimit(1)
                .minimumScaleFactor(0.8)
        ...
        .buttonStyle(.plain)
        // Mentre la richiesta e' in volo il bottone si spegne: e' l'unico modo
        // che ha un widget di dire "sto lavorando", e senza si guarda un
        // riquadro immobile chiedendosi se il tap sia arrivato.
        .foregroundStyle(row.pending == .inFlight ? TwoWatchTheme.textSecondary : TwoWatchTheme.textPrimary)
        .background(
            TwoWatchTheme.brandPrimary.opacity(row.pending == .inFlight ? 0.10 : 0.22),
...
extension EpisodeCheckRow {
    ...
    /// Il bottone dice in che momento si trova: sta salvando, e' andata male,
    /// oppure e' pronto per il prossimo episodio.
    var buttonLabel: LocalizedStringKey {
        switch pending {
        case .inFlight: return "Salvo l'episodio…"
        case .failed: return "Riprova"
        case nil: return "Episodio visto"
        }
    }

    /// Un avanti veloce, non una spunta: il bottone non conferma qualcosa di
    /// gia' fatto, porta la serie all'episodio dopo.
    ...
    var statusColor: Color {
        pending == .failed ? TwoWatchTheme.textSecondary : TwoWatchTheme.brandPrimary
    }
}
