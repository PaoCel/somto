...
struct WatchlistWidgetRow: Identifiable, Sendable {
    ...
    let watchUrl: URL?
    /// Se `watchUrl` apre la scheda esatta del titolo o solo una ricerca. Serve
    /// al copy di VoiceOver: dire "Guarda su Netflix" per una ricerca promette
    /// un tap che invece ne costa due.
    let watchIsExact: Bool
    let deepLink: URL
    let posterData: Data?
    /// Lo stato dell'ultimo tap su "episodio visto" per questa riga: serve al
    /// bottone del riquadro grande, che senza mostrerebbe la stessa faccia
    /// mentre la richiesta e' in volo e dopo che e' fallita.
    let pending: WatchlistWidgetPendingEntry.State?
}
...
struct WatchlistWidgetProvider: TimelineProvider {
    ...
    /// Se il riassunto c'e' ma non e' leggibile (telefono mai sbloccato dopo
    /// un riavvio) si ritenta presto: sei ore di "Apri Somto" per un file che
    /// esiste sarebbero il sintomo di prima.
    private static let retryInterval: TimeInterval = 15 * 60
    private static let siteURL = "https://somto.it"

    private static func refreshInterval(for entry: WatchlistWidgetEntry) -> TimeInterval {
        if case .missing = entry.state, WatchlistWidgetSnapshotStore.isPresentButUnreadable {
    ...
    func getTimeline(in context: Context, completion: @escaping @Sendable (Timeline<WatchlistWidgetEntry>) -> Void) {
        ...
        Task {
            ...
            completion(Timeline(
                entries: [entry],
                policy: .after(entry.date.addingTimeInterval(Self.refreshInterval(for: entry)))
            ))
        }
    }
    ...
    private static func makeRows(from planned: [WatchlistWidgetSnapshot.Row]) async -> [WatchlistWidgetRow] {
        // Le immagini si prendono tutte insieme e una sola volta per URL — le
        // piattaforme si ripetono, tre serie Netflix non sono tre loghi.
        //
        // In fila indiana erano quattro giri di rete sommati, ed e' il tempo
        // che si sentiva toccando una miniatura per cambiare serie: il tap
        // sembrava non fare niente per un paio di secondi. Con la cache su
        // disco (`WidgetImageCache`) il caso normale non tocca nemmeno la rete.
        let images = await loadImages(for: planned)
        var rows: [WatchlistWidgetRow] = []
        for row in planned {
            guard let deepLink = URL(string: "\(siteURL)\(row.path)") else { continue }
            let posterData = row.posterUrl.flatMap { images[$0] }
            let logoData = row.providerLogoUrl.flatMap { images[$0] }
            // Si risolve col nome per esteso ("HBO Max Amazon Channel"), si
            // mostra quello corto: il suffisso e' l'unica traccia del fatto che
            // quel marchio si guarda dentro Prime Video.
            let lookupName = row.providerSearchName ?? row.providerName ?? ""
            let destination = StreamingDestinationResolver.destination(
                providerName: lookupName,
                deepLinks: row.watchUrl.map { [lookupName: $0] } ?? [:],
                titleName: row.name,
                tmdbId: row.tmdbId,
            ...
            rows.append(WatchlistWidgetRow(
                ...
                watchUrl: destination?.url,
                watchIsExact: destination?.isExact ?? false,
                deepLink: deepLink,
                posterData: posterData,
                pending: WatchlistWidgetPendingStore.displayState(for: row.id)
            ))
        }
        return rows
    }

    private static func loadImages(for rows: [WatchlistWidgetSnapshot.Row]) async -> [URL: Data] {
        var urls: Set<URL> = []
        for row in rows {
            if let posterUrl = row.posterUrl { urls.insert(posterUrl) }
            if let logoUrl = row.providerLogoUrl { urls.insert(logoUrl) }
        }
        ...
        await withTaskGroup(of: (URL, Data?).self) { group in
            for url in urls {
                group.addTask { (url, await UpcomingReleasesFeedLoader.loadImageData(for: url)) }
            }
        ...
        VStack(alignment: .leading, spacing: SomtoSpacing.ml) {
            ...
            HStack(alignment: .top, spacing: SomtoSpacing.ml) {
                ...
                VStack(alignment: .leading, spacing: SomtoSpacing.xs) {
                    ...
                    WatchlistWidgetProgressDots(
                        watched: hero.episodesWatched,
                        total: hero.episodesTotal,
                        percent: hero.percentComplete
                    )
                ...
                VStack(spacing: SomtoSpacing.m) {
                    ...
                    if hero.kind == .inProgress || hero.isSeries {
                        Button(intent: MarkEpisodeWatchedIntent(titleId: hero.id)) {
                            WatchlistWidgetCircleIcon(
                                systemName: hero.markIcon,
                                isProminent: false,
                                isMuted: hero.pending == .inFlight
                            )
                        }
                        .buttonStyle(.plain)
                        .accessibilityLabel(hero.markAccessibilityLabel)
                    }
                }
            }
...
/// Il progresso a pallini: pieno l'episodio visto, vuoto quello che manca.
///
/// PERCHE' NON PIU' LA BARRA — una barra dice "circa a meta'", i pallini dicono
/// quanti episodi mancano, che e' la domanda vera di chi guarda una serie.
/// Funziona finche' i pallini si contano a colpo d'occhio: oltre una dozzina
/// diventano una barra tratteggiata e basta, quindi da li' in su si passa ai
/// decimi (dieci pallini, uno ogni 10% della serie). Il conteggio esatto resta
/// scritto sopra, in "Sei a S3·E8" o "12/24 episodi".
private struct WatchlistWidgetProgressDots: View {
    @ScaledMetric(relativeTo: .caption) private var dotSide: CGFloat = 7

    let watched: Int?
    let total: Int?
    let percent: Double?

    /// Sopra questa soglia un pallino per episodio non si conta piu'.
    private static let maxPerEpisodeDots = 12
    private static let quantizedDots = 10

    private var model: (filled: Int, count: Int)? {
        if let total, total > 0, total <= Self.maxPerEpisodeDots {
            return (max(0, min(total, watched ?? 0)), total)
        }
        guard let fraction = resolvedFraction else { return nil }
        let filled = Int((fraction * Double(Self.quantizedDots)).rounded(.down))
        return (max(0, min(Self.quantizedDots, filled)), Self.quantizedDots)
    }

    private var resolvedFraction: Double? {
        if let percent { return max(0, min(1, percent)) }
    ...
    var body: some View {
        if let model {
            HStack(spacing: dotSide * 0.5) {
                ForEach(0 ..< model.count, id: \.self) { index in
                    Circle()
                        .fill(index < model.filled ? TwoWatchTheme.brandPrimary : TwoWatchTheme.panelStrong)
...
private struct WatchlistWidgetCircleIcon: View {
    ...
    /// Spento mentre la richiesta e' in volo: un widget non ha altro modo di
    /// dire che sta lavorando.
    var isMuted = false

    private var tint: Color {
        if isMuted { return TwoWatchTheme.textSecondary }
        return isProminent ? TwoWatchTheme.textPrimary : TwoWatchTheme.brandPrimary
    }
    ...
    var body: some View {
        Image(systemName: systemName)
            ...
            .background(
                Circle().fill(
                    isProminent
                        ? TwoWatchTheme.brandPrimary.opacity(isMuted ? 0.4 : 0.9)
                        : TwoWatchTheme.brandPrimary.opacity(isMuted ? 0.08 : 0.16)
                )
            )
    }
}
...
extension WatchlistWidgetRow {
    ...
    var watchAccessibilityLabel: Text {
        ...
        // uguali e' il modo piu' rapido per far sembrare rotto un bottone che
        // ha fatto quello che poteva.
        return watchIsExact ? Text("Guarda su \(providerName)") : Text("Cerca su \(providerName)")
    }

    /// Un avanti veloce, non una spunta: il bottone porta la serie all'episodio
    /// dopo, non conferma qualcosa di gia' fatto. Mentre salva gira, se e'
    /// andata male invita a ritentare.
    var markIcon: String {
        switch pending {
        case .inFlight: return "arrow.triangle.2.circlepath"
        case .failed: return "arrow.clockwise"
        case nil: return "forward.fill"
        }
    }

    var markAccessibilityLabel: Text {
        switch pending {
        case .inFlight: return Text("Salvo l'episodio…")
        case .failed: return Text("Non riuscito, riprova")
        case nil: return Text("Segna episodio visto")
        }
    }
