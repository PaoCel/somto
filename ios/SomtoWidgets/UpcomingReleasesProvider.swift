...
struct UpcomingReleasesProvider: TimelineProvider {
    ...
    private static func nextRefreshDate(after entry: UpcomingReleasesEntry) -> Date {
        // Feed non raggiungibile: e' quasi sempre rete assente per un attimo,
        // non vale sei ore di widget vuoto. Si ritenta dopo mezz'ora.
        if case .unavailable = entry.state {
            return entry.date.addingTimeInterval(30 * 60)
        }
