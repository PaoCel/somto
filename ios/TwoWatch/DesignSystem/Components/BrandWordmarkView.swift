...
private struct BrandChromePillWrapper<Content: View>: View {
    ...
    var body: some View {
        content
            ...
            // La lente vive qui e non nell'header: essendo un overlay dentro la
            // tab, la sua safe area esclude già la tab bar, quindi il tasto si
            // appoggia sopra la barra classica senza offset calcolati a mano.
            // Da iOS 26 non serve: la tab bar di sistema ha già il suo tasto
            // di ricerca staccato (`Tab(role: .search)` in `AppTabShellView`),
            // che apre lo stesso foglio.
            .overlay(alignment: .bottomTrailing) {
                if !AppShellStore.usesNativeSearchTab && isVisible && !isHidden && !shell.isSearchPresented {
                    FloatingSearchButton { shell.presentSearch() }
                        .padding(.trailing, SomtoSpacing.xxl)
                        .padding(.bottom, SomtoSpacing.l)
                        .transition(.scale(scale: 0.7).combined(with: .opacity))
                }
            }
            .animation(.easeInOut(duration: 0.25), value: isHidden)
            .animation(.spring(response: 0.34, dampingFraction: 0.8), value: shell.isSearchPresented)
    }
}
