...
struct CharacterPickRow: View {
    ...
    private func accessibilityDescription(for candidate: CharacterCandidate, order: Int?, reaction: TitleEmotion?) -> String {
        ...
        if candidate.isGuest { parts.append(String(localized: "guest")) }
        if let order { parts.append(String(localized: "scelto come preferito numero \(order + 1)")) }
        if let reaction { parts.append(String(localized: "reazione \(reaction.label)")) }
