...
final class HomeRepository {
    ...
    ///
    /// PERCHE' NON PIU' `providerLane` — `getMatchQueue` restituisce sia `items`
    /// (la classifica per gusto) sia `providerLane`, che e' la stessa classifica
    /// ridotta a un solo servizio. La Home mostrava solo la seconda: buttava via
    /// buoni suggerimenti per il motivo sbagliato, visto che il valore di Somto
    /// e' consigliare a prescindere da chi trasmette. La piattaforma resta un
    /// dettaglio della scheda, non un criterio di selezione.
    ///
    /// Stessa callable e stesso costo di prima: cambia solo cosa si legge.
    func fetchForYou() async -> [Title] {
        do {
            ...
            // Chiavi = doc id Firestore, unici per costruzione: qui
            // `uniqueKeysWithValues` e' ammesso (IOS_CODE_STYLE §1.1).
            let byID = Dictionary(uniqueKeysWithValues: titles.map { ($0.id, $0) })
            // L'ordine e' il punteggio: si tiene quello della callable.
