...
struct TitleUpdateEvent: Identifiable, Sendable {
    ...
    /// Nome del titolo "sorgente" dell'evento, quando diverso dal titolo
    /// della pagina corrente (arrivato via `linkedTitleIds`, es. un trailer
    /// di stagione pubblicato sul film della saga). `nil` quando l'evento e'
    /// nativo di questo titolo — mostrato come "Da {nome}" nella timeline.
    ...
    init(
        ...
        self.sourceTitleName = sourceTitleName
    }
...
final class TitleRepository {
    ...
    /// Ricerca titoli "smart": lancia in parallelo match esatto, prefix e
    /// ricerca per token (con eventuale secondo token) e fa scoring con
    /// `TitleParsing.searchScore`. Permette a "beekeeper" di trovare "The
    /// Beekeeper" (via `stripLeadingArticle`) senza sacrificare un match
    /// esatto come "Natale a Rio", che il vecchio flusso a singola query
    /// poteva perdere dentro 80 risultati non ordinati.
    ///
    /// Costo Firestore worst-case per chiamata: 3 (exact) + prefixLimit
    /// (prefix) + tokenLimit (primo token) + tokenLimit (secondo token,
    /// SOLO se la query ha >=2 token utili e il primo token satura il limite
    /// o non copre il secondo) letture. Exact e prefix condividono l'indice
    /// composito esistente `status asc, nameLower asc`; i token usano
    /// l'indice esistente `search.tokens CONTAINS, ratingCount DESC` (senza
    /// `status`, filtrato client-side — v. nota in firestore.indexes.json).
    /// Nessun nuovo indice, nessun backfill.
    private func smartSearchCandidates(
        normalized: String,
        tokens: [String],
        tokenLimit: Int,
        prefixLimit: Int
        ...
        let bestTokens = TitleParsing.bestSearchTokens(from: tokens, max: 2)

        async let exactTask = fetchApprovedTitles(orderByNameLowerFrom: normalized, to: normalized, limit: 3)
        async let prefixTask = fetchApprovedTitles(
            orderByNameLowerFrom: normalized, to: normalized + "\u{f8ff}", limit: prefixLimit
        )
        async let firstTokenTask: [Title] = {
            guard let token = bestTokens.first else { return [] }
            return await fetchApprovedTitles(byToken: token, limit: tokenLimit)
        }()

        let exact = await exactTask
        let prefix = await prefixTask
        let firstTokenResults = await firstTokenTask

        var secondTokenResults: [Title] = []
        if bestTokens.count > 1 {
            let secondToken = bestTokens[1]
            let saturated = firstTokenResults.count >= tokenLimit
            let coversSecondToken = firstTokenResults.contains { title in
                SearchNormalizer.tokens(from: title.nameLower).contains(secondToken)
                    || SearchNormalizer.normalize(title.searchableText).contains(secondToken)
            }
            if saturated || !coversSecondToken {
                secondTokenResults = await fetchApprovedTitles(byToken: secondToken, limit: tokenLimit)
            }
        }

        var seenIDs = Set<String>()
        var merged: [Title] = []
        for title in exact + prefix + firstTokenResults + secondTokenResults where seenIDs.insert(title.id).inserted {
            merged.append(title)
        }
        return merged
    }

    private func fetchApprovedTitles(orderByNameLowerFrom start: String, to end: String, limit: Int) async -> [Title] {
        guard let snapshot = try? await db.collection("titles")
            .whereField("status", isEqualTo: "approved")
            .order(by: "nameLower")
            .start(at: [start])
            .end(at: [end])
    ...
    private func fetchApprovedTitles(byToken token: String, limit: Int) async -> [Title] {
        guard let snapshot = try? await db.collection("titles")
            .whereField("search.tokens", arrayContains: token)
            .order(by: "ratingCount", descending: true)
            .limit(to: limit)
            .getDocuments()
        else { return [] }
        return snapshot.documents.compactMap(snapshotToTitle).filter { $0.status == "approved" }
    }
    ...
    func searchTitles(_ query: String, limit: Int = 20) async throws -> [Title] {
        ...
        let candidates = await smartSearchCandidates(
            normalized: normalized,
            tokens: tokens,
            tokenLimit: min(80, max(limit * 4, 40)),
            prefixLimit: 20
        )

        let ranked = candidates
            .map { title in (title, TitleParsing.searchScore(for: title, normalized: normalized, tokens: tokens)) }
            .filter { $0.1 > 0 }
            .sorted { lhs, rhs in
                if lhs.1 != rhs.1 { return lhs.1 > rhs.1 }
                if lhs.0.ratingCount != rhs.0.ratingCount { return lhs.0.ratingCount > rhs.0.ratingCount }
                return (lhs.0.year ?? 0) > (rhs.0.year ?? 0)
            }
    ...
    func searchTitlesForListBuilder(_ query: String, limit: Int = 40) async throws -> [Title] {
        ...
        // Stesso merge exact+prefix+token(+secondo token) di `searchTitles`,
        // ma scoring via `smartListSearchScore` (bonus su collection/keyword/
        // tipo) invariato: continua a valere per aggiungere un titolo a una
        // lista, non solo la ricerca "beekeeper" -> "The Beekeeper".
        let candidates = await smartSearchCandidates(
            normalized: normalized,
            tokens: tokens,
            tokenLimit: min(120, max(limit * 4, 40)),
            prefixLimit: min(40, max(limit, 20))
        )
    ...
    private static let relatedCuratedMax = 30
    private static let relatedCollabMax = 24
    private static let relatedGenreFallbackMax = 24
    private static let relatedCandidatesMax = 40
    private static let sagaTitlesMax = 30

    /// Dedup preservando l'ordine di prima occorrenza. Non e'
    /// `Dictionary(uniqueKeysWithValues:)` su chiave derivata: qui la chiave
    /// e' sempre un doc id Firestore, permesso dal footgun (CLAUDE.md).
    private static func uniqueOrdered(_ ids: [String]) -> [String] {
        var seen = Set<String>()
        var result: [String] = []
        for id in ids {
    ...
    /// Correlati unificati per la scheda titolo: curati (`title.related`,
    /// solo approvati) + collaborativi (`titles/{id}/aggregates/similar`,
    /// solo se loggato) + fallback per genere quando i primi due segnali non
    /// bastano. Mirror di `getRelatedTitlesUnified`
    /// (`public/js/api/titles.api.js`): stessa forma dei candidati, stesso
    /// ordinamento (`RelatedTitlesRanking`, mirror di `relatedRanking.js`) —
    /// iOS e web devono mostrare lo stesso ordine.
    ///
    /// `featureSums` va passato dal chiamante (caricato una volta sola per
    /// apertura scheda, non qui: vedi `TitleDetailViewModel`). `excludeIDs`
    /// tiene fuori la saga, gia' mostrata nella sua sezione dedicata.
    ///
    /// LETTURE per apertura scheda, worst case (utente loggato):
    /// - 1 getDoc `titles/{id}/aggregates/similar` (anche se assente)
    /// - fino a 40 letture doc per i candidati curati+collaborativi (batch da
    ///   30 via `listTitles`, cache 60s — quasi mai il worst case pieno)
    /// - se dopo curati+collaborativi restano <24 candidati: 1 query genere,
    ///   fino a 24 letture doc aggiuntive
    /// Totale worst case ~1 getDoc + ~64 letture doc (esclusa la
    /// `tasteProfile`, condivisa con altre sezioni e caricata a monte);
    /// tipico (pochi correlati curati, poche letture mancanti in cache)
    /// molto piu' basso.
    func fetchRelatedTitles(
        for title: Title,
        currentUserID: String?,
        featureSums: TasteFeatureSums? = nil,
        excludeIDs: Set<String> = [],
        limit: Int = TitleRepository.relatedCandidatesMax
    ) async throws -> [Title] {
        let pageID = title.id
        let curatedIDs = Array(Self.uniqueOrdered(title.related).prefix(Self.relatedCuratedMax))

        var neighbors: [(id: String, score: Double)] = []
        if let currentUserID, !currentUserID.isEmpty {
            do {
                let snapshot = try await db.collection("titles").document(pageID)
                    .collection("aggregates").document("similar")
                    .getDocument()
                let rows = (snapshot.data()?["neighbors"] as? [[String: Any]]) ?? []
                neighbors = rows.compactMap { row in
                    guard let id = FirestoreValueReader.string(row, key: "id"), !id.isEmpty else { return nil }
                    return (id, FirestoreValueReader.double(row, key: "score") ?? 0)
                }
            } catch {
                SilentFailure.record(error, context: "TitleRepository.relatedSimilarity")
            }
        }

        let collabIDs = Array(
            Self.uniqueOrdered(neighbors.map(\.id))
                .filter { !curatedIDs.contains($0) }
                .prefix(Self.relatedCollabMax)
        )
        let similarityByID = Dictionary(neighbors.map { ($0.id, $0.score) }, uniquingKeysWith: { first, _ in first })

        let primaryIDs = Array(Self.uniqueOrdered(curatedIDs + collabIDs).prefix(Self.relatedCandidatesMax))
        let primaryTitles = (try? await listTitles(ids: primaryIDs)) ?? []
        let primaryByID = Dictionary(primaryTitles.map { ($0.id, $0) }, uniquingKeysWith: { first, _ in first })

        let approvedCuratedIDs = curatedIDs.filter { $0 != pageID && primaryByID[$0]?.status == "approved" }
        let approvedCollabIDs = collabIDs.filter { $0 != pageID && primaryByID[$0]?.status == "approved" }

        var candidates = (approvedCuratedIDs + approvedCollabIDs).compactMap { primaryByID[$0] }

        if candidates.count < Self.relatedGenreFallbackMax, !title.genres.isEmpty {
            do {
                let snapshot = try await db.collection("titles")
                    .whereField("status", isEqualTo: "approved")
                    .whereField("genres", arrayContainsAny: Array(title.genres.prefix(10)))
                    .limit(to: Self.relatedGenreFallbackMax)
                    .getDocuments()
                var seen = Set(candidates.map(\.id))
                for genreTitle in snapshot.documents.compactMap(snapshotToTitle) {
                    guard genreTitle.id != pageID, seen.insert(genreTitle.id).inserted else { continue }
                    candidates.append(genreTitle)
                }
            } catch {
                SilentFailure.record(error, context: "TitleRepository.relatedGenreFallback")
            }
        }

        let ranked = RelatedTitlesRanking.rank(
            curatedIDs: approvedCuratedIDs,
            candidates: candidates,
            similarityByID: similarityByID,
            featureSums: featureSums,
            excludeIDs: excludeIDs.union([pageID])
        )
        return Array(ranked.prefix(limit))
    }

    /// Titoli della stessa saga (`collectionId` condiviso, solo film),
    /// ordinati per anno crescente. Mirror di `listSagaTitles`
    /// (`public/js/api/titles.api.js`). Richiede l'indice composito
    /// `collectionId + status + year` (`firestore.indexes.json`). Al massimo
    /// `Self.sagaTitlesMax` letture doc; fallisce in silenzio (lista vuota)
    /// se l'indice non e' pronto o la query va storta — la sezione Saga
    /// semplicemente non appare.
    func fetchSagaTitles(collectionID: Int, excludingTitleID: String) async throws -> [Title] {
        let snapshot = try await db.collection("titles")
            .whereField("collectionId", isEqualTo: collectionID)
            .whereField("status", isEqualTo: "approved")
            .order(by: "year")
            .limit(to: Self.sagaTitlesMax)
            .getDocuments()
        return snapshot.documents
            .compactMap(snapshotToTitle)
    }

    /// `featureSums` del tasteProfile dell'utente (`users/{uid}/tasteProfile/agg`),
    /// per la formula di affinita' gusti dei correlati (`RelatedTitlesRanking`).
    /// Lettura owner-only, 1 getDoc; `nil` se il documento non esiste ancora
    /// (utente nuovo, nessuna azione tracciata) o su qualunque errore — i
    /// correlati degradano al solo punteggio collaborativo + popolarita',
    /// esattamente come sul web senza tasteProfile.
    func fetchTasteFeatureSums(userID: String) async throws -> TasteFeatureSums? {
        guard !userID.isEmpty else { return nil }
        let snapshot = try await db.collection("users").document(userID)
            .collection("tasteProfile").document("agg")
            .getDocument()
        guard let raw = snapshot.data()?["featureSums"] as? [String: Any] else { return nil }

        func bucket(_ key: String) -> [String: TasteFeatureEntry] {
            let map = FirestoreValueReader.map(raw[key])
            var result: [String: TasteFeatureEntry] = [:]
            for (id, value) in map {
                let entry = FirestoreValueReader.map(value)
                let sum = FirestoreValueReader.double(entry, key: "sum") ?? 0
                let weight = FirestoreValueReader.double(entry, key: "weight") ?? 0
                guard weight > 0 else { continue }
                result[id] = TasteFeatureEntry(sum: sum, weight: weight)
            }
            return result
        }

        return TasteFeatureSums(genres: bucket("genres"), people: bucket("people"))
    }

    /// Crea una proposta di correlato (`titleRelatedSuggestions/{uid}__{titleId}__{suggestedTitleId}`).
    /// Auto-approvata server-side alla terza proposta utente distinta; le
    /// rules impongono id deterministico + `hasOnly` + `createdAt ==
    /// request.time`, quindi il payload deve avere ESATTAMENTE questi campi.
    /// Mirror di `createRelatedSuggestion` (`public/js/api/titles.api.js`).
    func createRelatedSuggestion(userID: String, titleID: String, suggestedTitleID: String) async throws {
        guard !userID.isEmpty, !titleID.isEmpty, !suggestedTitleID.isEmpty else {
            throw NSError(domain: "TwoWatch", code: 422, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Dati mancanti per il suggerimento.")
            ])
        }
        guard titleID != suggestedTitleID else {
            throw NSError(domain: "TwoWatch", code: 422, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Un titolo non può essere correlato a se stesso.")
            ])
        }
        let docID = "\(userID)__\(titleID)__\(suggestedTitleID)"
        try await db.collection("titleRelatedSuggestions").document(docID).setData([
            "uid": userID,
            "titleId": titleID,
            "suggestedTitleId": suggestedTitleID,
            "status": "pending",
    ...
    /// Query aggiuntiva `linkedTitleIds array-contains titleID`: eventi
    /// "in prestito" da un altro titolo della saga (es. un trailer di
    /// stagione pubblicato sul film "sorgente"). Wrappata a parte e non
    /// throwing: un indice ancora in costruzione o doc pre-backfill senza il
    /// campo non deve rompere la lista legacy, degrada in silenzio a [].
    /// Mirror di `listTitleUpdateEvents` (`public/js/api/titleUpdates.api.js`).
    private func fetchLinkedTitleUpdateDocs(titleID: String) async -> [QueryDocumentSnapshot] {
        do {
            ...
            SilentFailure.record(error, context: "TitleRepository.linkedTitleUpdates")
            return []
        }
    }

    /// Eventi titolo pubblicati: doppia query merge per id (legacy `titleId ==`
    /// + nuova `linkedTitleIds array-contains`, vedi
    /// `fetchLinkedTitleUpdateDocs`). Un evento "in prestito" da un altro
    /// titolo (`titleId` del doc diverso da `titleID` della pagina) porta
    /// un'etichetta "Da {nome}" risolta con una singola `listTitles` batch
    /// (gia' cache 60s).
    private func fetchPersistedTitleUpdates(titleID: String) async throws -> [TitleUpdateEvent] {
        async let legacyTask = db.collection("titleUpdateEvents")
            .whereField("titleId", isEqualTo: titleID)
            .whereField("status", isEqualTo: "published")
            .order(by: "sortAt", descending: true)
            .limit(to: 20)
            .getDocuments()
        async let linkedDocsTask = fetchLinkedTitleUpdateDocs(titleID: titleID)

        let legacySnapshot = try await legacyTask
        let linkedDocs = await linkedDocsTask

        var seenIDs = Set<String>()
        var mergedDocs: [(id: String, data: [String: Any])] = []
        for document in legacySnapshot.documents + linkedDocs {
            guard seenIDs.insert(document.documentID).inserted else { continue }
            mergedDocs.append((document.documentID, document.data()))
        }

        var sourceTitleIDs = Set<String>()
        for entry in mergedDocs {
            if let sourceTitleID = FirestoreValueReader.string(entry.data, key: "titleId"), sourceTitleID != titleID {
                sourceTitleIDs.insert(sourceTitleID)
            }
        }
        var sourceNamesByID: [String: String] = [:]
        if !sourceTitleIDs.isEmpty {
            let sourceTitles = (try? await listTitles(ids: Array(sourceTitleIDs))) ?? []
            for sourceTitle in sourceTitles {
                sourceNamesByID[sourceTitle.id] = sourceTitle.name
            }
        }

        let events = mergedDocs.compactMap { entry -> TitleUpdateEvent? in
            let sourceTitleID = FirestoreValueReader.string(entry.data, key: "titleId")
            var sourceTitleName: String?
            if let sourceTitleID, sourceTitleID != titleID {
                sourceTitleName = sourceNamesByID[sourceTitleID]
            }
            return Self.makeUpdateEvent(id: entry.id, data: entry.data, sourceTitleName: sourceTitleName)
        }
        return Array(
            events
                .sorted { ($0.sortAt ?? .distantPast) > ($1.sortAt ?? .distantPast) }
                .prefix(20)
        )
    }
    ...
    private static func makeUpdateEvent(id: String, data: [String: Any], sourceTitleName: String? = nil) -> TitleUpdateEvent? {
        ...
        return TitleUpdateEvent(
            ...
            sourceTitleName: sourceTitleName
        )
    }
    ...
    /// Ultimi voti dell'utente (qualunque livello), ridotti al minimo che
    /// serve a `CompanionDefaults` per calcolare i default del picker "Con
    /// chi l'hai visto?". Stesso trade-off di `fetchProfileReviewsPage`
    /// (`WatchlistRepository`): filtra solo su `uid` (indice automatico a
    /// singolo campo, nessun composito nuovo) invece di ordinare
    /// server-side per `updatedAt`, e sposta sort/cap sul client. Prende un
    /// margine di documenti in piu' del `limit` richiesto per compensare
    /// l'assenza di un ordine server-side prevedibile.
    func fetchRecentRatingsForCompanionDefaults(
        userID: String,
        limit: Int = 60
    ) async throws -> [CompanionRatingSample] {
        guard !userID.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return [] }

        let fetchLimit = max(limit * 2, 90)
        ...
        let titleIDs = Array(Set(snapshot.documents.compactMap { FirestoreValueReader.string($0.data(), key: "titleId") }))
        let titles = try await listTitles(ids: titleIDs)
        let mediaTypeByTitleID = Dictionary(titles.map { ($0.id, $0.type) }, uniquingKeysWith: { first, _ in first })

        let samples: [CompanionRatingSample] = snapshot.documents.compactMap { document in
            let data = document.data()
            guard let titleId = FirestoreValueReader.string(data, key: "titleId"),
                  let mediaType = mediaTypeByTitleID[titleId]
            else { return nil }

            let companions = (data["watchedWith"] as? [Any] ?? []).compactMap { row -> FeedTaggedUser? in
            ...
            return CompanionRatingSample(
                mediaType: mediaType,
                companions: companions,
                occurredAt: FirestoreValueReader.date(data["updatedAt"]) ?? .distantPast
            )
        }
