...
final class AuthenticationRepository {
    ...
    private var passwordSignupCreationInProgress = false
    private(set) var pendingPasswordSignupUID: String?
    ...
    func signUp(email: String, password: String) async throws -> User {
        passwordSignupCreationInProgress = true
        do {
            let user = try await auth.createUser(withEmail: email, password: password).user
            pendingPasswordSignupUID = user.uid
            passwordSignupCreationInProgress = false
            return user
        } catch {
            passwordSignupCreationInProgress = false
            throw error
    ...
    func isUnverifiedPasswordUser(_ user: User) -> Bool {
        user.isEmailVerified == false && user.providerData.contains { $0.providerID == "password" }
    }

    func shouldHoldForEmailVerification(_ user: User) -> Bool {
        isUnverifiedPasswordUser(user)
            && (passwordSignupCreationInProgress || pendingPasswordSignupUID == user.uid)
    }

    func registerPendingSignup(displayName: String, ageConfirmed: Bool) async throws {
        let attribution = signupAttribution()
        _ = try await invokeCallable(name: "registerPendingSignup", payload: [
            "displayName": displayName,
            "ageConfirmed": ageConfirmed,
            "communitySafetyAccepted": true,
            "communitySafetyAcceptedSource": "ios_signup",
            "attribution": attribution
        ])
    }

    func sendSignupVerification() async throws {
        guard let user = auth.currentUser else {
            throw NSError(domain: "TwoWatch", code: 17, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Nessun account da verificare.")
            ])
        }
        let settings = ActionCodeSettings()
        settings.handleCodeInApp = false
        settings.url = URL(string: "https://somto.app/login.html?verify=1")
        try await user.sendEmailVerification(with: settings)
    }

    func reloadCurrentUser() async throws -> User {
        guard let user = auth.currentUser else {
            throw NSError(domain: "TwoWatch", code: 18, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Nessun account da verificare.")
            ])
        }
        try await user.reload()
        guard let refreshed = auth.currentUser else {
            throw NSError(domain: "TwoWatch", code: 19, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Sessione scaduta. Accedi di nuovo.")
            ])
        }
    ...
    func completeVerifiedSignup() async throws -> SignupCompletionResult {
        let result = try await invokeCallable(name: "completeVerifiedSignup", payload: [
            "communitySafetyAcceptedSource": "ios_signup",
            "attribution": signupAttribution()
        ])
        guard let payload = result.data as? [String: Any], payload["status"] as? String == "active" else {
            throw CloudFunctionsCaller.CallableError.decodingError
        }
        pendingPasswordSignupUID = nil
        return SignupCompletionResult(
            created: payload["created"] as? Bool ?? false,
            pendingCompleted: payload["pendingCompleted"] as? Bool ?? false
        )
    }

    func signupState() async throws -> SignupLifecycleState {
        let result = try await invokeCallable(name: "getSignupState", payload: [:])
        guard let payload = result.data as? [String: Any], let status = payload["lifecycleStatus"] as? String else {
            throw CloudFunctionsCaller.CallableError.decodingError
        }
        return SignupLifecycleState(
            lifecycleStatus: status,
            emailVerified: payload["emailVerified"] as? Bool ?? false,
            legacyEmailUnverified: payload["legacyEmailUnverified"] as? Bool ?? false,
            profilePresent: payload["profilePresent"] as? Bool ?? false,
            pendingStatus: payload["pendingStatus"] as? String
        )
    }
    ...
    private func signupAttribution() -> [String: Any] {
    ...
    private func randomNonceString(length: Int = 32) -> String {
        ...
        while remainingLength > 0 {
            ...
            if errorCode != errSecSuccess {
                // i18n-ok: diagnostica fatalError per sviluppatori, non UI.
