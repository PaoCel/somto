...
final class NotificationRepository {
    ...
    func destinationForPushPayload(_ payload: [AnyHashable: Any]) -> AppDestination {
        ...
        case "weekly_digest":
            // Con una novita' sola il backend mette gia' il deep link al
            // titolo in `ctaUrl`; con piu' di una si apre la lista, perche'
            // portare su un titolo solo nasconderebbe le altre.
            break
        case "title_update":
            // Se l'uscita ha il suo post commentabile si apre quello: e' il
            // solo posto dove si puo' rispondere a qualcuno. Senza post si
            // torna alla scheda, come prima.
            if let postID = payloadString(payload["postId"]) {
                return .post(id: postID)
            }
            if let titleID = payloadString(payload["titleId"]) {
    ...
    private func mapNotification(document: QueryDocumentSnapshot, users: [String: AppUser]) -> AppNotification? {
        ...
        case "recommendation":
            icon = "🎬"
            title = String(localized: "\(fromName) ti ha consigliato un titolo")
            destination = .profileInbox
            text = nil
        case "follow":
            icon = "👀"
            title = String(localized: "\(fromName) ha iniziato a seguirti")
            destination = destinationForProfile(fromUid)
            text = nil
        case "friend_request":
            icon = "🤝"
            title = String(localized: "\(fromName) ti ha inviato una richiesta")
            destination = destinationForProfile(fromUid)
            text = nil
        case "friend_accept":
            icon = "✅"
            title = String(localized: "\(fromName) ha accettato la richiesta")
            destination = destinationForProfile(fromUid)
            text = nil
        case "thread_message":
            icon = "💬"
            title = String(localized: "\(fromName) ha scritto nel thread")
        ...
        case "thread_mention":
            icon = "@"
            title = String(localized: "\(fromName) ti ha menzionato in un thread")
            text = preview?.prefix(100).description
            if let threadId {
                destination = .thread(id: threadId)
            } else {
        ...
        case "quiz_challenge":
            ...
            title = String(localized: "\(fromName) ti ha sfidato a un quiz")
            text = n > 0 ? String(localized: "\(n) domande da rispondere") : String(localized: "Apri la sfida nella sezione Quiz")
            destination = .quizChallenges
        case "quiz_challenge_completed":
            icon = "🏆"
            title = String(localized: "\(fromName) ha completato la sfida")
            text = String(localized: "Vedi chi ha vinto e rivedi le risposte")
            destination = .quizChallenges
        case "post_mention":
            icon = "@"
            title = context == "rating_comment"
                ? String(localized: "\(fromName) ti ha menzionato su un voto")
                : String(localized: "\(fromName) ti ha menzionato")
            text = preview?.prefix(100).description
            destination = destinationForHomePost(postId, fallbackTitleID: titleId)
        case "post_like":
            icon = "❤️"
            title = String(localized: "\(fromName) ha messo like al tuo post")
            text = nil
            destination = destinationForHomePost(postId, fallbackTitleID: titleId)
        case "post_comment":
            icon = "💬"
            title = String(localized: "\(fromName) ha commentato il tuo post")
            text = preview?.prefix(100).description
            destination = destinationForHomePost(postId, fallbackTitleID: titleId)
        case "comment_reply":
            icon = "↩️"
            title = String(localized: "\(fromName) ha risposto al tuo commento")
            text = preview?.prefix(100).description
            destination = destinationForHomePost(postId, fallbackTitleID: titleId)
        case "comment_like":
            if let threadId {
                icon = reaction ?? "👍"
                title = String(localized: "\(fromName) ha reagito al tuo commento")
                text = reaction
                destination = .thread(id: threadId)
            } else {
                icon = "👍"
                title = String(localized: "\(fromName) ha messo like al tuo commento")
                text = nil
                destination = destinationForHomePost(postId, fallbackTitleID: titleId)
            }
        case "rating_like":
            icon = "⭐"
            title = String(localized: "\(fromName) ha reagito al tuo voto")
            text = nil
            destination = destinationForHomePost(postId, fallbackTitleID: titleId)
        case "rating_comment":
            icon = "🗨️"
            title = String(localized: "\(fromName) ha commentato la tua recensione")
            text = preview?.prefix(100).description
            destination = destinationForHomePost(postId, fallbackTitleID: titleId)
        case "watched_with_tag":
            icon = "🍿"
            title = String(localized: "\(fromName) ti ha taggato in una visione")
            text = String(localized: "Hai visto \(titleName) insieme. Ti va di votarlo?")
            destination = destinationForTitle(titleId, focus: "rating")
        case "engagement_nudge":
            icon = "✨"
            title = localizedNotificationText(nested["messageByLocale"])
                ?? sourceItalianNotificationText(nested["message"])
                ?? String(localized: "Nuovi titoli ti aspettano su Somto")
            text = nil
        ...
        case "engagement_friend_watched":
            ...
            title = localizedNotificationText(nested["messageByLocale"])
                ?? sourceItalianNotificationText(nested["message"])
                ?? String(localized: "\(fromName) ha visto \(titleName)")
            text = String(localized: "Potrebbe interessarti!")
            destination = destinationForTitle(titleId, focus: nil)
        case "engagement_watchlist_reminder":
            icon = "📋"
            let count = FirestoreValueReader.int(nested["count"]) ?? 0
            title = count > 0
                ? String(localized: "Hai \(count)+ titoli in watchlist da recuperare")
                : (localizedNotificationText(nested["messageByLocale"])
                    ?? sourceItalianNotificationText(nested["message"])
                    ?? String(localized: "Hai titoli in watchlist da recuperare"))
        ...
        case "engagement_friend_activity":
            ...
            let count = FirestoreValueReader.int(nested["count"]) ?? 0
            title = count > 0
                ? String(localized: "\(count) nuove attività dai tuoi amici")
                : (localizedNotificationText(nested["messageByLocale"])
                    ?? sourceItalianNotificationText(nested["message"])
                    ?? String(localized: "I tuoi amici sono stati attivi"))
            text = String(localized: "Scopri cosa hanno visto e recensito.")
            destination = destinationForCTA(ctaURL)
        ...
        case "new_user":
            icon = "🆕"
            title = FirestoreValueReader.string(nested["message"]) ?? String(localized: "\(fromName) si è appena registrato")
            text = nil
            destination = destinationForProfile(fromUid)
        case "new_season_available":
            icon = "🆕"
            ...
            title = seasonNumber > 0
                ? String(localized: "Stagione \(seasonNumber) di \(seriesName) disponibile")
                : String(localized: "\(seriesName) ha nuovi episodi")
            text = String(localized: "Riprendi a guardare quando vuoi.")
            destination = destinationForTitle(titleId, focus: nil)
        case "weekly_digest":
            icon = "🎞️"
            // Il titolo e' gia' la novita' piu' forte della settimana: un
            // sopratitolo generico ruberebbe la riga al nome della serie.
            title = localizedNotificationText(nested["messageByLocale"])
                ?? sourceItalianNotificationText(nested["message"])
                ?? String(localized: "Novità sui tuoi titoli")
            // Sotto, le altre novita' per nome: contarle non diceva quali
            // fossero e restavano invisibili.
            text = localizedNotificationText(nested["previewByLocale"])
        ...
        case "title_update":
            ...
            // NB: la variabile `postId` di questo scope ripiega su `eventId`,
            // che qui c'e' sempre — quindi si legge il campo esplicito.
            if let conversationPostId = FirestoreValueReader.string(nested["postId"]) {
                destination = destinationForHomePost(conversationPostId, fallbackTitleID: titleId)
            } else {
                destination = destinationForTitle(titleId, focus: focus)
            }
        case "official_update":
            icon = "✓"
            title = FirestoreValueReader.string(nested["title"]) ?? String(localized: "Aggiornamento Somto")
            text = preview?.prefix(100).description
            destination = destinationForHomePost(postId, fallbackTitleID: titleId)
        case "titles_import_completed":
            icon = "✅"
            title = String(localized: "\(importLabel) completato")
            text = localizedNotificationText(nested["messageByLocale"])
                ?? sourceItalianNotificationText(nested["message"])
                ?? String(localized: "I risultati sono nella tua libreria.")
            destination = .watchlist
        case "titles_import_needs_review":
            icon = "🧩"
            title = String(localized: "\(importLabel) da controllare")
            text = localizedNotificationText(nested["messageByLocale"])
                ?? sourceItalianNotificationText(nested["message"])
                ?? String(localized: "Alcuni titoli richiedono una conferma.")
            destination = .titlesImport(importId: FirestoreValueReader.string(nested["importId"]))
        case "titles_import_failed":
            icon = "⚠️"
            title = String(localized: "\(importLabel) non riuscito")
            text = localizedNotificationText(nested["messageByLocale"])
                ?? sourceItalianNotificationText(nested["message"])
                ?? String(localized: "Puoi riprovare quando vuoi.")
            destination = .titlesImport(importId: FirestoreValueReader.string(nested["importId"]))
        case "admin_import_started":
            ...
            title = String(localized: "\(fromName) ha avviato un import \(sourceLabel)")
            text = rows > 0 ? String(localized: "\(rows) righe da elaborare — apri il profilo") : String(localized: "Apri il profilo di chi ha importato")
        ...
        case "comment_review_pending":
            ...
            title = eligible == 1
                ? String(localized: "1 commento TV Time da revisionare")
                : String(localized: "\(eligible) commenti TV Time da revisionare")
            text = resolved == eligible
                ? String(localized: "\(fromName): tutti pronti per la revisione.")
                : String(localized: "\(fromName): \(resolved) di \(eligible) già associati ai titoli.")
        ...
        case "friend_post":
            ...
            icon = "📝"
            title = String(localized: "\(fromName) ha pubblicato un post")
            text = preview?.prefix(100).description
            destination = destinationForHomePost(postId, fallbackTitleID: titleId)
        default:
            icon = "🔔"
    ...
    /// I documenti legacy hanno `message` solo in italiano. Su un device EN
    /// non va mostrato come se fosse copy localizzato: si usa il fallback del
    /// catalogo, oppure la nuova mappa `messageByLocale` quando presente.
    private func sourceItalianNotificationText(_ value: Any?) -> String? {
        let locale = (Bundle.main.preferredLocalizations.first ?? "it").lowercased()
        guard locale.hasPrefix("it") else { return nil }
        return FirestoreValueReader.string(value)
    }
