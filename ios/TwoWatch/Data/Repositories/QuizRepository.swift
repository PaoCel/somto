...
final class QuizRepository {
    ...
    /// Saghe giocabili dall'aggregato pubblico `quizMeta/sagas` (una lettura,
    /// stesse rules di `quizMeta/themes`). Il doc puo' non esistere ancora su
    /// un progetto dove l'aggregato non e' mai stato costruito: in quel caso
    /// l'elenco e' vuoto e la sezione "Saghe" semplicemente non compare.
    func fetchPlayableSagas() async throws -> [QuizSaga] {
        let snapshot = try await db.collection("quizMeta").document("sagas").getDocument()
        guard let raw = snapshot.data()?["sagas"] as? [[String: Any]] else { return [] }
        return raw.compactMap { item in
            guard
                let sagaId = item["sagaId"] as? String, !sagaId.isEmpty,
                let name = item["name"] as? String, !name.isEmpty,
                let titleIds = item["titleIds"] as? [String], !titleIds.isEmpty
            else { return nil }
            let media = QuizMediaType(rawValue: (item["mediaType"] as? String) ?? "unknown") ?? .unknown
            return QuizSaga(
                sagaId: sagaId,
    ...
    /// Annulla un invito esterno non ancora reclamato: il server cancella la
    /// sfida e il token, quindi il link smette di funzionare. Solo chi ha
    /// creato l'invito puo' farlo — il controllo vive nella function.
    func cancelExternalInvite(challengeId: String) async throws {
        _ = try await CloudFunctionsCaller.call(
            name: "cancelQuizExternalInvite",
    ...
    private func decodeChallenge(_ snap: DocumentSnapshot) -> QuizChallenge? {
        ...
        return QuizChallenge(
            ...
            sagaId: data["sagaId"] as? String,
            sagaName: data["sagaName"] as? String
        )
    }
