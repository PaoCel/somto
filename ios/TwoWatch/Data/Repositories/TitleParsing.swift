...
enum TitleParsing {
    ...
    /// `titles/{id}.watchDeepLinks` = mappa `nome piattaforma -> url`, scritta dal
    /// server (`functions/modules/streamingLinks.js`) accanto a
    /// `watchProviderNames`. Forma diversa da `watchProviderLogos`, che e' un
    /// array: qui il backend salva l'oggetto cosi' com'e' risolto da Wikidata.
    ///
    /// Parse difensivo: righe senza URL valido si scartano. La chiave e' il nome
    /// del provider — testo libero, non un doc id — quindi `uniquingKeysWith`,
    /// mai `uniqueKeysWithValues` (§1.1 dello stile).
    static func parseWatchDeepLinks(from data: [String: Any]) -> [String: URL] {
        guard let map = data["watchDeepLinks"] as? [String: Any], !map.isEmpty else { return [:] }
        let pairs: [(String, URL)] = map.compactMap { key, value in
            let name = key.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !name.isEmpty else { return nil }
            guard let raw = value as? String, let url = URL(string: raw), url.scheme?.hasPrefix("http") == true else { return nil }
            return (name, url)
        }
        return Dictionary(pairs, uniquingKeysWith: { first, _ in first })
    }
    ...
    /// Articoli guida (it/en/es/fr/de) usati SOLO per lo scoring e per una
    /// query prefix aggiuntiva sulla query "spogliata": non vengono MAI
    /// tolti da `search.tokens` (l'indice Firestore resta invariato).
    ...
    /// Toglie l'eventuale articolo guida iniziale da una stringa già
    /// normalizzata (`SearchNormalizer.normalize`), una parola sola e mai
    /// fino a svuotarla. Permette a "beekeeper" di trovare "The Beekeeper"
    /// e a "il primo natale" di non scavalcare un match esatto senza
    /// articolo.
    static func stripLeadingArticle(_ normalized: String) -> String {
        var parts = normalized.split(separator: " ").map(String.init)
        guard parts.count > 1, let first = parts.first, leadingArticles.contains(first) else {
            return normalized
        }
        parts.removeFirst()
    ...
    /// euristica "il più raro" senza una lettura extra per contare le
    /// occorrenze reali, quindi si usa la lunghezza (i token lunghi sono
    /// tipicamente più selettivi) escludendo sempre gli articoli guida. Se
    /// dopo il filtro non resta nulla (query fatta solo di articoli), si
    /// ripiega sui token originali.
    static func bestSearchTokens(from tokens: [String], max: Int = 2) -> [String] {
        let candidates = tokens.filter { !leadingArticles.contains($0) }
        let pool = candidates.isEmpty ? tokens : candidates
        return Array(
    ...
    /// Scoring puro condiviso da `searchTitles` e (via `smartListSearchScore`)
    /// da `searchTitlesForListBuilder`. Specchio di
    /// `public/js/utils/titleSearchRank.js` `scoreTitleMatch` — stessa scala
    /// e stesso ordine di priorità, va aggiornato in entrambi i posti.
    /// `tokens`, se omesso, è ricalcolato da `normalized` (compatibilità coi
    /// chiamanti esistenti che passavano solo `normalized`).
    static func searchScore(for title: Title, normalized: String, tokens: [String]? = nil) -> Int {
        guard !normalized.isEmpty, !title.nameLower.isEmpty else { return 0 }

        let nameLower = title.nameLower
        let queryTokens = tokens ?? SearchNormalizer.tokens(from: normalized)
        let queryStripped = stripLeadingArticle(normalized)

        if nameLower == normalized { return 12 }
        if nameLower.hasPrefix(normalized) { return 10 }

        // Match esatto/prefix "spogliato" dell'articolo guida, su entrambi i
        // lati (query senza articolo contro titolo con articolo, e viceversa).
        let nameStripped = stripLeadingArticle(nameLower)
        if nameStripped == normalized || nameStripped == queryStripped || nameLower == queryStripped {
            return 9
        }
        if nameStripped.hasPrefix(normalized)
            || (!queryStripped.isEmpty && (nameStripped.hasPrefix(queryStripped) || nameLower.hasPrefix(queryStripped))) {
            return 9
        }

        // Tutti i token della query compaiono nel nome, anche se non in
        // ordine/contigui (es. query con token invertiti).
        if queryTokens.count > 1, queryTokens.allSatisfy({ nameLower.contains($0) }) {
            return 8
        }

        let aliasHit = title.aliases.contains { SearchNormalizer.normalize($0).contains(normalized) }
        let originalName = title.originalName ?? ""
        let originalHit = !originalName.isEmpty && SearchNormalizer.normalize(originalName).contains(normalized)
        if aliasHit || originalHit { return 8 }

        if queryTokens.contains(where: { !leadingArticles.contains($0) && nameLower.contains($0) }) {
            return 6
        }

        if nameLower.contains(normalized) { return 5 }
        ...
        if collectionHit || keywordHits || searchableHit { return 3 }
    ...
    static func title(from data: [String: Any], documentID: String) -> Title {
        ...
        return Title(
            ...
            related: FirestoreValueReader.stringArray(data["related"]),
            ...
            watchDeepLinks: parseWatchDeepLinks(from: data)
        )
    }
