...
final class FirebaseAnalyticsLogger: AnalyticsLogging {
    ...
    func log(_ event: String, _ params: [String: Any] = [:]) {
        ...
        ProductFunnel.forward(sanitizedName)
    }
}

/// Ponte verso il funnel prodotto (`productMetrics/{giorno}` su Firestore).
///
/// PERCHE' ESISTE — Firebase Analytics vede tutto, ma la dashboard prodotto no:
/// quella legge `productMetrics`, che viene scritto dalla callable
/// `logProductEvent`, finora chiamata **solo dalla PWA**. Con una base utenti
/// quasi tutta iOS il risultato era un funnel vuoto: `onboarding_started` a zero
/// per giornate intere con iscritti veri dentro (misurato il 2026-08-24).
///
/// Sta qui e non nei singoli call site perche' gli eventi sono gia' loggati in
/// giro per l'app: cosi' nessuna feature viene toccata.
enum ProductFunnel {
    /// Deve restare allineata a `PRODUCT_EVENT_WHITELIST` in
    /// `functions/index.js`: un nome fuori lista e' un no-op silenzioso lato
    /// server, quindi aggiungerne uno qui senza aggiungerlo li' non conta niente.
    static let forwardedEvents: Set<String> = [
        AnalyticsEvent.onboardingStarted,
        AnalyticsEvent.onboardingCompleted,
        AnalyticsEvent.importStarted,
    ]

    static func forward(_ event: String) {
        guard forwardedEvents.contains(event) else { return }
        Task.detached(priority: .background) {
            do {
                _ = try await CloudFunctionsCaller.call(name: "logProductEvent", data: ["event": event])
            } catch {
                // Un contatore mancato non e' un problema di chi usa l'app:
                // non si propaga e non si mostra.
                SilentFailure.record(error, context: "ProductFunnel.\(event)")
            }
        }
    }
}
