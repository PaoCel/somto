...
final class SessionStore {
    ...
    /// Account Firebase valido ma ancora privo di accesso applicativo: il
    /// profilo viene materializzato solo dopo la verifica email server-side.
    private(set) var requiresEmailVerification = false
    /// Account storico ancora ammesso durante la migrazione: accesso consentito,
    /// con un invito dismissibile a verificare l'email.
    private(set) var recommendsEmailVerification = false
    ...
    var isAuthenticated: Bool {
        !requiresEmailVerification && (firebaseUser != nil || appUser != nil)
    }
    ...
    func dismissEmailVerificationRecommendation() {
        recommendsEmailVerification = false
    }

    func sendRecommendedEmailVerification() async {
        do {
    ...
    private func loadProfile(for user: User) async throws {
        ...
        try await loadExistingProfile(for: user)
    }

    private func loadExistingProfile(for user: User) async throws {
        ...
        guard appUser != nil else {
            throw NSError(domain: "TwoWatch", code: 20, userInfo: [
                NSLocalizedDescriptionKey: String(localized: "Profilo non disponibile. Riprova.")
            ])
        }
    ...
    private func consumeAuthChange(_ user: User?) async {
        ...
        if authRepository.isUnverifiedPasswordUser(user) {
            let mustHold: Bool
            var lifecycleState: SignupLifecycleState?
            if authRepository.shouldHoldForEmailVerification(user) {
                ...
                do {
                    ...
                    mustHold = state.requiresEmailVerification
                } catch {
                    // Compatibilità per gli account storici mentre le nuove
                    // callable non sono ancora disponibili: solo un profilo
                    // già esistente può entrare, mai un nuovo auth-only.
                    let hasLegacyProfile = (try? await userRepository.fetchUser(uid: user.uid)) != nil
                    mustHold = !hasLegacyProfile
                    recommendsEmailVerification = hasLegacyProfile
                }
            }
            if mustHold {
                appUser = nil
            ...
            if let lifecycleState {
                recommendsEmailVerification = lifecycleState.recommendsEmailVerification
            }
        } else {
            recommendsEmailVerification = false
        }
        ...
        do {
            if user.isEmailVerified && user.providerData.contains(where: { $0.providerID == "password" }) {
                do {
                    let state = try await authRepository.signupState()
                    if state.requiresServerCompletion {
                        _ = try await authRepository.completeVerifiedSignup()
                    }
                    try await loadExistingProfile(for: user)
                } catch {
                    if try await userRepository.fetchUser(uid: user.uid) != nil {
                        try await loadExistingProfile(for: user)
                    } else {
                        throw error
                    }
