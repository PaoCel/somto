...
struct AuthView: View {
    ...
    private var canSubmit: Bool {
        ...
        if isSignUp {
            guard viewModel.confirmPassword.isEmpty == false,
                  viewModel.hasConfirmedMinimumAge else { return false }
        }
        return true
    }
    ...
    var body: some View {
                ...
                ScrollView {
                    VStack(spacing: 22) {
                        ...
                        GlassCard(padding: 20) {
                            if viewModel.isAwaitingEmailVerification {
                                verificationContent
                            } else {
                                VStack(spacing: 18) {
                                    modePicker
                        ...
                        if !viewModel.isAwaitingEmailVerification
                            && (viewModel.canUseAppleSignIn || viewModel.canUseGoogleSignIn) {
                            socialSignInSection
                        }
        ...
        .task {
            await viewModel.resumePendingSignupIfNeeded()
        }
    }
    ...
    private var communityTermsBlock: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if isSignUp {
                Toggle(isOn: $viewModel.hasConfirmedMinimumAge) {
                    Text("Confermo di avere almeno 14 anni")
                        .font(.subheadline.weight(.semibold))
                        .foregroundStyle(TwoWatchTheme.textPrimary)
                }
                .tint(TwoWatchTheme.brandPrimary)
        ...
        VStack(spacing: 18) {
            ...
            VStack(spacing: 7) {
                Text("Controlla la tua email")
                    ...
                    .foregroundStyle(TwoWatchTheme.textPrimary)
                Text("Abbiamo inviato un link a \(viewModel.verificationEmail). Aprilo, poi torna qui per continuare.")
                    .font(.subheadline)
                    .foregroundStyle(TwoWatchTheme.textSecondary)
                    .multilineTextAlignment(.center)
            }
            ...
            Button {
                Task {
                    if await viewModel.checkVerification() {
                        await session.retryProfileLoad()
                        shell.isAuthPresented = false
                    }
                }
            } label: {
                if viewModel.isLoading {
                    ...
                    Text("Ho verificato l'email")
                }
            }
            .buttonStyle(PrimaryButtonStyle())
            .disabled(viewModel.isLoading)

            Button("Invia di nuovo") {
