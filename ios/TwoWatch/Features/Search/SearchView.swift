...
final class SearchViewModel {
    ...
    /// Solo per "Tendenze" nello stato vuoto: la riga stava in Home, ma e'
    /// scoperta generica, non personale — qui da' un contenuto alla ricerca
    /// prima ancora di digitare.
    ...
    init(titleRepository: TitleRepository, userRepository: UserRepository, homeRepository: HomeRepository? = nil) {
        self.titleRepository = titleRepository
        self.userRepository = userRepository
        self.homeRepository = homeRepository
    }

    /// Fallisce in silenzio: e' un suggerimento, non un risultato.
    func loadTrending() async {
        guard trending.isEmpty, let homeRepository else { return }
        do {
            trending = try await homeRepository.fetchTrendingTitles()
        } catch {
            SilentFailure.record(error, context: "Search.trending")
        }
    }

    /// Un genere toccato dallo stato vuoto: cambia scope e cerca subito.
    func applyGenre(_ name: String) {
        scope = .genres
        query = name
    }
...
struct SearchView: View {
    ...
    var body: some View {
        // Il campo sta in fondo, vicino al pollice, e la tastiera lo spinge
        // su da sola: `safeAreaInset` tiene i risultati sempre sopra al
        // campo, senza padding a mano.
        resultsList
        ...
        .task {
            // La tastiera si apre da sola: aprire la ricerca e dover toccare
            // una seconda volta il campo era il difetto principale del vecchio
            // foglio con `.searchable`. Il piccolo ritardo lascia finire la
            // transizione del foglio, altrimenti il fuoco viene scartato.
            guard !disablesAutomaticLoading else { return }
    ...
    /// Campo di ricerca ancorato in basso: vetro, lente, testo e un solo tasto
    /// a destra che pulisce quando c'è del testo e chiude quando non ce n'è.
    private var queryField: some View {
        @Bindable var viewModel = viewModel
        return HStack(spacing: SomtoSpacing.ml) {
        ...
        .background(
            Capsule()
                ...
                .overlay(Capsule().strokeBorder(TwoWatchTheme.border, lineWidth: 1))
                .somtoElevation(.medium)
        )
        .padding(.horizontal, SomtoSpacing.xxl)
        .padding(.bottom, SomtoSpacing.ml)
    }

    @ViewBuilder
    private var trailingFieldButton: some View {
    ...
    private func idleSuggestions(for scope: SearchScope, helperMessage: LocalizedStringKey) -> some View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if scope == .titles, !viewModel.trending.isEmpty {
                SearchSuggestionStrip(title: "Tendenze", icon: "flame.fill") {
                    LazyVGrid(columns: titleGridColumns, spacing: 14) {
                        ForEach(viewModel.trending.prefix(6)) { title in
                            titleResultCell(title)
                        }
                    }
                }

                SearchSuggestionStrip(title: "Generi", icon: SearchScope.genres.systemImage) {
                    FlowChips(items: SearchScope.genres.suggestions) { genre in
                        Button {
                            viewModel.applyGenre(genre)
                        } label: {
                            suggestionChipLabel(genre)
                        }
                        .buttonStyle(.plain)
                        .accessibilityHint(Text("Filtra per genere"))
                    }
                }
            }
