...
struct TitlesImportView: View {
    ...
    private var headerSubtitle: String {
        switch selectedSource {
        case .tvTime:
            return String(localized: "Porta su Somto i film e le serie che hai visto, i voti e la watchlist.")
        case .netflix:
    ...
    private var netflixSteps: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            importStepRow(number: 2, text: String(localized: "Seleziona in alto il profilo giusto, poi in fondo alla pagina tocca \u{201c}Scarica tutto\u{201d}."))
    ...
    private var traktDisconnectedSteps: some View {
        VStack(alignment: .leading, spacing: 12) {
            importStepRow(number: 1, text: String(localized: "Tocca \u{201c}Connetti Trakt\u{201d} qui sotto: ti mostriamo un codice."))
            ...
            importStepRow(number: 3, text: String(localized: "Torna qui: appena l'account risulta collegato, avvia l'import."))
        }
    }
    ...
    private var tvTimePickerCard: some View {
        GlassCard {
            // Nessuno step su come ottenere l'export: il tool self-service di
            // TV Time e' irraggiungibile da luglio 2026, quindi dire
            // "richiedilo" manda le persone contro un muro. Qui si carica e
            // basta, come per le altre fonti.
            VStack(alignment: .leading, spacing: 16) {
    ...
    private func confirmationBodyText(_ job: TitlesImportJob) -> String {
        var text = String(localized: "\(job.matchedCount) titoli sono già nel tuo profilo.")
        if job.unresolvedCount > 0 {
            text += String(localized: " \(job.unresolvedCount) voci non siamo riusciti ad abbinarle in automatico.")
        }
        return text
    }
    ...
    private func statusSubtitle(_ job: TitlesImportJob) -> String {
        ...
        case "awaiting_confirmation":
            return String(localized: "\(job.matchedCount) titoli aggiornati. Alcune voci richiedono un controllo.")
        case "manual_processing":
        ...
        case "completed":
            return String(localized: "\(job.matchedCount) titoli aggiornati nella tua libreria.")
        case "failed":
