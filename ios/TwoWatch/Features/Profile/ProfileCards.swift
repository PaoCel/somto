...
struct ProfileActivityCard: View {
    ...
    private func discussionLinkLabel(_ review: ProfileReviewEntry) -> String {
        ...
        case ("episode", .some(let s), .some(let e)):
            return String(localized: "Vai alla discussione di S\(s)E\(e)")
        case ("season", .some(let s), _):
            return String(localized: "Vai alla discussione della stagione \(s)")
        default:
            return String(localized: "Vai alla discussione")
        }
    }
