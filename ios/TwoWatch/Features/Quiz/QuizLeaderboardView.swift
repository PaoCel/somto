...
struct QuizLeaderboardView: View {
    ...
    /// Solo sulla classifica settimanale: prima diceva "si aggiorna ogni
    /// lunedì alle 00:00", che descriveva un job che non esiste — il bucket
    /// settimanale si azzera al primo tentativo della settimana nuova
    /// (`QuizRepository.submitAttempt`, chiave ISO), non a un orario.
    @ViewBuilder
    private var footer: some View {
        if scope == .weekly {
            HStack(spacing: 6) {
                ...
                Text("Classifica della settimana in corso")
                    .font(.caption2)
            }
