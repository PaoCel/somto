...
final class QuizTitleCatalogStore {
    ...
    /// Franchise giocabili (`quizMeta/sagas`), gia' ordinate dall'aggregato.
    /// Vuote quando il doc non esiste: la sezione "Saghe" non compare.
    private(set) var sagas: [QuizSaga] = []
    ...
    /// Stessa ricerca sui nomi delle saghe.
    var filteredSagas: [QuizSaga] {
        let q = query.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !q.isEmpty else { return [] }
        return sagas.filter {
    ...
    func load() async {
        ...
        do {
            ...
            // Accessorio: senza saghe il picker resta quello di prima, quindi
            // un errore qui non deve far fallire il caricamento dei titoli.
            do {
                sagas = try await container.quizRepository.fetchPlayableSagas()
            } catch {
                SilentFailure.record(error, context: "QuizTitleCatalog.sagas")
            }
            state = .loaded
...
struct QuizTitlePickerView: View {
    ...
    /// Selezione saga. `nil` = questo host non gioca le saghe (il composer
    /// sfida manda un solo `titleId` al backend): la sezione non compare.
    var selectedSagaId: Binding<String?>?
    ...
    private var searchResults: some View {
        sagaSection(store.filteredSagas)
        if store.filteredThemes.isEmpty && store.filteredSagas.isEmpty {
            VStack(spacing: 10) {
    ...
    /// Stessa cella e stessa griglia delle righe titolo: una saga e' un
    /// gruppo di titoli, non un oggetto di UI diverso. Il poster e' quello del
    /// primo titolo della saga (l'aggregato li ordina per numero di domande).
    @ViewBuilder
    private func sagaSection(_ sagas: [QuizSaga]) -> some View {
        if let selectedSagaId, !sagas.isEmpty {
            VStack(alignment: .leading, spacing: 10) {
                ...
                LazyVGrid(columns: columns, spacing: 14) {
                    ForEach(sagas) { saga in
                        let cover = saga.titleIds.first ?? saga.sagaId
                        QuizPosterCell(
                            theme: QuizTheme(
                                titleId: cover,
                                title: saga.name,
                                mediaType: saga.mediaType,
                                questionCount: saga.questionCount
                            ),
                            posterURL: store.posters[cover],
                            isSelected: selectedSagaId.wrappedValue == saga.sagaId,
                            kindLabel: String(localized: "Saga")
                        ) {
                            UIImpactFeedbackGenerator(style: .light).impactOccurred()
                            withAnimation(.easeOut(duration: 0.18)) {
                                selectedSagaId.wrappedValue = saga.sagaId
                                // Titolo e saga sono alternativi: la partita
                                // pesca da uno solo dei due.
                                selectedTitleId = nil
                            }
                            onSelect?()
                        }
                        .task {
                            if store.posters[cover] == nil {
                                await store.loadPosters(for: [cover])
                            }
                        }
                    }
                }
            ...
            ForEach(themes) { theme in
                QuizPosterCell(
                    ...
                    withAnimation(.easeOut(duration: 0.18)) {
                        ...
                        selectedSagaId?.wrappedValue = nil
                    }
                    onSelect?()
                }
                .task {
...
struct QuizPosterCell: View {
    ...
    let posterURL: URL?
    let isSelected: Bool
    /// Cosa e' questa cella per VoiceOver. Default: film o serie, dal
    /// `mediaType`. Le saghe passano "Saga", che e' piu' vero di "Film".
    ...
    var body: some View {
        ...
        .accessibilityLabel("\(theme.title). \(kindLabel ?? (theme.mediaType == .tv ? String(localized: "Serie TV") : String(localized: "Film"))).\(isSelected ? " Selezionato." : "")")
