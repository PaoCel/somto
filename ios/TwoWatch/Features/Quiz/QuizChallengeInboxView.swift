...
struct QuizChallengeInboxView: View {
    ...
    /// Invito esterno per cui e' aperta la conferma di annullamento.
    @State private var cancelTarget: QuizChallenge?
    @State private var isCancelling = false
    @State private var showCancelledBanner = false
    /// Errore di un'azione sulla riga: non deve svuotare la lista come
    /// `errorMessage`, che e' lo stato di errore del caricamento.
    ...
    var body: some View {
        ...
        .confirmationDialog(
            "Annullare l'invito? Il link smetterà di funzionare.",
            isPresented: Binding(
            ...
            Button("Annulla invito", role: .destructive) {
                Task { await cancelInvite(target) }
            }
            Button("Annulla", role: .cancel) { cancelTarget = nil }
        }
        .alert(
            actionError ?? "",
            isPresented: Binding(
        ...
        .animation(.spring(response: 0.35, dampingFraction: 0.78), value: showCancelledBanner)
        .sensoryFeedback(.success, trigger: showCancelledBanner)
        .task { await load() }
        .refreshable { await load() }
    }

    /// Conferma breve dopo l'annullamento: la riga sparisce, e senza un segno
    /// esplicito sembrerebbe sparita da sola.
    private var cancelledBanner: some View {
        Label("Invito annullato", systemImage: "checkmark.circle.fill")
            .font(.subheadline.weight(.bold))
            .foregroundStyle(.white)
            .padding(.horizontal, 16)
            .padding(.vertical, 10)
            .background(TwoWatchTheme.success.opacity(0.92), in: Capsule(style: .continuous))
                ...
                LazyVStack(spacing: 12) {
                    ForEach(list, id: \.id) { challenge in
                        QuizChallengeRow(
                            ...
                            session: session,
                            shell: shell,
                            onCancelInvite: { cancelTarget = challenge }
                        )
                    }
                    inviteFriendCard
                }
    ...
    /// Annulla l'invito lato server e toglie la riga: il link muore, quindi
    /// lasciarla in lista sarebbe una bugia.
    private func cancelInvite(_ challenge: QuizChallenge) async {
        guard !isCancelling else { return }
        isCancelling = true
        defer { isCancelling = false }
        cancelTarget = nil
        do {
            try await container.quizRepository.cancelExternalInvite(challengeId: challenge.challengeId)
            sent.removeAll { $0.challengeId == challenge.challengeId }
            received.removeAll { $0.challengeId == challenge.challengeId }
...
private struct QuizChallengeRow: View {
    ...
    let session: SessionStore
    let shell: AppShellStore
    /// Solo sugli inviti esterni ancora in attesa: chiede al contenitore di
    /// aprire la conferma. L'annullamento vero lo fa lui, che possiede la lista.
    ...
    /// Un invito esterno ancora in attesa e' l'unica riga su cui si puo'
    /// agire: il link si ricondivide e l'invito si annulla.
    private var showsInviteActions: Bool {
        onCancelInvite != nil && challenge.isAwaitingExternalSignup
    }

    private var inviteURL: URL? {
        challenge.inviteUrl.flatMap { URL(string: $0) }
    }
    ...
    var body: some View {
        ...
        .contextMenu {
            if showsInviteActions {
                if let inviteURL {
                    ShareLink(item: inviteURL, message: Text(QuizChallengeComposerView.shareMessage)) {
                ...
                Button(role: .destructive) {
                    ...
                    Label("Annulla invito", systemImage: "trash")
                }
            }
        }
    }
    ...
    private func rowBody(interactive: Bool) -> some View {
        VStack(spacing: 12) {
            rowSummary(interactive: interactive)
            if showsInviteActions {
                inviteActions
            }
        }
    ...
    /// Le azioni stanno nella riga, non solo nello swipe: uno swipe non si
    /// scopre, e questo e' l'unico posto da cui il link si ritrova.
    @ViewBuilder
    private var inviteActions: some View {
        HStack(spacing: 8) {
            if let inviteURL {
                ShareLink(item: inviteURL, message: Text(QuizChallengeComposerView.shareMessage)) {
                    Label("Condividi invito", systemImage: "square.and.arrow.up")
    ...
    private func rowSummary(interactive: Bool) -> some View {
        HStack(spacing: 12) {
            opponentAvatar
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                // Su cosa si gioca: oggi la riga non lo dice mai, ma su una
                // saga il nome del franchise e' meta' della sfida.
                if let sagaName = challenge.sagaName, !sagaName.isEmpty {
                    Label(sagaName, systemImage: "film.stack")
        ...
        // Combinato qui e non su tutta la riga: cosi' i bottoni dell'invito
        // restano elementi VoiceOver a se'.
        .accessibilityElement(children: .combine)
    ...
    private var avatarFallback: some View {
        ZStack {
            ...
            // Solo un nome vero diventa un'iniziale: "Invito del 6 set" darebbe
            // una "I" uguale per tutti.
            if challenge.hasOpponentName, let initial = challenge.opponentLabel.first {
    ...
    private var accessibilityLabel: String {
        let saga = (challenge.sagaName?.isEmpty == false) ? ", \(challenge.sagaName ?? "")" : ""
        return "\(isReceived ? "Sfida da" : "Sfida a") \(challenge.opponentLabel)\(saga), \(challenge.numQuestions) domande, \(QuizState.label(for: challenge, isReceived: isReceived, hasPlayed: hasPlayed))"
    }
