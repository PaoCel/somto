...
struct QuizHomeView: View {
    ...
    /// Il setup e' spinto da qui e non da un `NavigationLink`: a partita finita
    /// "Chiudi" deve poter smontare setup e player in un colpo solo e
    /// riportare all'hub.
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $isSetupPresented) {
            QuizGameSetupView(
                container: container,
                session: session,
                shell: shell,
                onFinishedGame: {
                    ...
                    // L'hub resta montato mentre si gioca: senza questo
                    // ricarico mostrerebbe le statistiche di prima della partita.
    ...
    /// Statistiche non caricate: prima `loadingError` veniva scritta e mai
    /// letta, quindi l'hub mostrava zeri indistinguibili da "non hai mai
    /// giocato". Riga compatta con Riprova, il resto della schermata resta
    /// utilizzabile.
    @ViewBuilder
    private var loadingErrorBanner: some View {
        if let loadingError {
            HStack(spacing: 10) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(TwoWatchTheme.warning.opacity(0.4), lineWidth: 1)
            )
