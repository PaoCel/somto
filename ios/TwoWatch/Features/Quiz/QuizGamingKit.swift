...
struct QuizStreakChip: View {
    ...
    /// `LocalizedStringKey` e non `String`: `Text(String)` non passa dal
    /// catalogo, quindi la scritta restava in italiano anche con l'app in
    /// inglese e diceva "1 giorni di fila" (il plurale sta nelle variations
    /// di `%lld giorni di fila` in Localizable.xcstrings).
...
struct QuizDailyBonusCard: View {
    ...
    private var subtitle: String {
        ...
        return String(localized: "Gioca \(remaining) partite per attivare il bonus.")
    }
