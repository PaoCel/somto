...
final class QuizChallengeComposerViewModel {
    ...
    /// Alternativa al titolo singolo: un'intera saga (`quizMeta/sagas`). I due
    /// sono esclusivi, ci pensa il picker a spegnere l'altro.
    ...
    /// `preselectedTitleId`: la sfida nasce gia' puntata su un titolo (CTA
    /// "Sfida un amico" nel risultato di una partita su quel titolo). Chi
    /// apre il composer dalle Sfide non lo passa e parte da "Casuale".
    init(container: AppContainer, session: SessionStore, preselectedTitleId: String? = nil) {
        ...
        if let preselectedTitleId {
            self.questionMode = .specific
            self.selectedTitleId = preselectedTitleId
        }
    }
    ...
    /// Titoli da cui pescare le domande: vuoto in modalita' casuale, tutti i
    /// titoli del franchise quando la scelta e' una saga.
    var sourceTitleIds: [String] {
        guard questionMode == .specific else { return [] }
        if let selectedSaga { return selectedSaga.titleIds }
        return selectedTitleId.map { [$0] } ?? []
    }

    /// Friend challenge is sendable only when the source is decided: random is
    /// always valid, a specific title (o una saga) needs a selection.
    var canSendFriend: Bool {
        selectedFriend != nil && (questionMode == .random || !sourceTitleIds.isEmpty)
    }
    ...
    func loadFriends() async {
        ...
        do {
            // Il grafo sociale e' "chi segui" (fase 1 della dismissione
            // amicizie): la lista amici legacy resta solo per chi l'aveva
            // gia'. Prima chi segui, poi gli amici accettati non ancora in lista.
            async let followingList = container.userRepository.listFollowing(userID: uid)
            async let legacyFriends = container.userRepository.listAcceptedFriends(userID: uid)
            let (following, accepted) = try await (followingList, legacyFriends)
            var seen = Set(following.map(\.id))
            friends = following + accepted.filter { seen.insert($0.id).inserted }
        } catch {
    ...
    func send() async {
        ...
        do {
            let saga = selectedSaga
            let titleIds = sourceTitleIds
...
struct QuizChallengeComposerView: View {
    let container: AppContainer
    let session: SessionStore
    /// Titolo su cui la sfida parte gia' impostata (vedi il view model).
    var preselectedTitleId: String? = nil

    @State private var viewModel: QuizChallengeComposerViewModel?
    @State private var showSuccessBanner = false
    @Environment(\.dismiss) private var dismiss

    /// Message used by the share sheet for external invites. Anche l'inbox
    /// ricondivide lo stesso link, quindi lo stesso testo vive qui una volta sola.
    static let shareMessage = String(localized: "Ti ho sfidato su Somto Quiz 🎬🔥 Registrati e giochiamo una sfida su film e serie!")
    ...
    var body: some View {
        ...
        .task {
            if viewModel == nil {
                let vm = QuizChallengeComposerViewModel(
                    container: container,
                    session: session,
    ...
    private func friendSendBar(vm: QuizChallengeComposerViewModel) -> some View {
        if vm.path == .friend, vm.selectedFriend != nil {
            Button {
                ...
                primaryLabel(
                    isBusy: vm.isSending,
                    busyText: String(localized: "Invio…"),
                    idleIcon: "paperplane.fill",
                    idleText: sendBarTitle(vm: vm)
                )
            }
            .buttonStyle(PrimaryButtonStyle())
            .disabled(vm.isSending || !vm.canSendFriend)
            .opacity(vm.canSendFriend ? 1 : 0.5)
            .accessibilityLabel(vm.isSending ? String(localized: "Invio sfida in corso") : sendBarTitle(vm: vm))
            .padding(.horizontal, 16)
            .padding(.top, 10)
            .padding(.bottom, 12)
    ...
    /// Cosa dice il bottone di invio: su una scelta precisa dice *su cosa* si
    /// sfida — titolo o saga, stessa frase — cosi' la barra conferma la
    /// selezione senza doverla ricercare a monte.
    private func sendBarTitle(vm: QuizChallengeComposerViewModel) -> String {
        guard vm.questionMode == .specific else { return String(localized: "Invia sfida") }
        if let saga = vm.selectedSaga { return String(localized: "Sfida su \(saga.name)") }
        if let theme = vm.selectedTheme { return String(localized: "Sfida su \(theme.title)") }
        // Titolo scelto ma catalogo non ancora in memoria (sfida che nasce
        // gia' puntata): il nome manca, la sfida invece parte.
        return vm.canSendFriend ? String(localized: "Invia sfida") : String(localized: "Scegli un titolo")
    }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if vm.isLoadingFriends {
                ...
                EmptyStateView(
                    ...
                    message: "Puoi comunque sfidare chiunque: crea un link d'invito e giocate insieme.",
    ...
    private func externalPathBody(vm: QuizChallengeComposerViewModel) -> some View {
        ...
        // Il nome sta in cima perche' e' l'unica cosa che rende riconoscibile
        // l'invito nella lista: senza, resta identificato dalla sola data.
        ...
        if let result = vm.inviteResult {
            ...
            Button {
                ...
                primaryLabel(
                    isBusy: vm.isSending,
                    busyText: String(localized: "Creo l'invito…"),
                    idleIcon: "link.badge.plus",
                    idleText: String(localized: "Crea invito")
                )
            }
            .buttonStyle(PrimaryButtonStyle())
