...
final class TitleDetailViewModel {
    ...
    /// Titoli della stessa saga (`title.collectionId`), ordinati per anno.
    /// Vuoto per titoli senza collection o con una sola voce approvata (la
    /// sezione Saga si nasconde da sola in quel caso).
    var sagaTitles: [Title] = []
    /// `featureSums` del tasteProfile utente, caricato una volta sola per
    /// apertura scheda (owner read, fallimento silenzioso): alimenta
    /// l'affinita' gusti di `RelatedTitlesRanking`. `nil` per i guest o se il
    /// profilo non esiste ancora.
    @ObservationIgnored private(set) var tasteFeatureSums: TasteFeatureSums?
    var trailerURL: URL?
    ...
    /// Titoli della saga (`title.metadata.collectionId`, solo film): `[]`
    /// senza collection, con una sola voce approvata (la saga sarebbe solo
    /// il titolo stesso) o su qualunque errore — la sezione Saga si nasconde
    /// da sola in questi casi.
    private func loadSagaTitlesIfNeeded(for title: Title) async -> [Title] {
        guard let collectionID = title.metadata.collectionId else { return [] }
        do {
            let saga = try await titleRepository.fetchSagaTitles(collectionID: collectionID, excludingTitleID: title.id)
            return saga.count > 1 ? saga : []
        } catch {
            SilentFailure.record(error, context: "TitleDetail.saga")
            return []
        }
    }

    /// `featureSums` del tasteProfile utente, caricato una volta per
    /// apertura scheda. Guest o errore → `nil`, i correlati degradano al
    /// solo punteggio collaborativo + popolarità (come sul web).
    private func loadTasteFeatureSumsIfNeeded(currentUserID: String?) async -> TasteFeatureSums? {
        guard let currentUserID, !currentUserID.isEmpty else { return nil }
        do {
            return try await titleRepository.fetchTasteFeatureSums(userID: currentUserID)
        } catch {
            SilentFailure.record(error, context: "TitleDetail.tasteProfile")
            return nil
        }
    }

    /// Applica il risultato di `enrichTitleAssets` sia alle proprieta' di
    /// presentazione sia al modello in memoria: RatingPostComposerSheet,
    /// EpisodeSeenSheet e il pick personaggio leggono da `self.title` via
    /// `fetchTitleCharacterCandidates(title:)`, non da `castCredits`. Senza
    /// questo passaggio il cast restava "non disponibile" nel composer voto
    /// finche' la scheda non veniva riaperta.
    private func applyEnrichedAssets(_ enriched: TitleEnrichmentResult, needsTrailer: Bool, needsCast: Bool) {
        let gotTrailer = needsTrailer && enriched.trailerURL != nil
        let gotCast = needsCast && !enriched.castWithCharacters.isEmpty
        if gotTrailer, let url = enriched.trailerURL {
            self.trailerURL = url
        }
        if gotCast {
            castCredits = mergeCharacters(into: castCredits, with: enriched.castWithCharacters)
        }
        if let currentTitle = self.title, gotTrailer || gotCast {
            self.title = currentTitle.withEnrichedAssets(
                trailerURL: enriched.trailerURL,
                castWithCharacters: enriched.castWithCharacters
            )
        }
    }
    ...
    private func loadDeferredDetailData(for title: Title, currentUserID: String?, generation: Int) async {
        ...
        // Saga + tasteProfile determinano gli input di "Titoli correlati"
        // (exclude gia' mostrati in Saga, affinita' gusti): vanno risolti
        // prima di lanciare la fetch correlati, quindi in parallelo tra loro
        // ma prima del resto della batch qui sotto.
        async let sagaTask = loadSagaTitlesIfNeeded(for: title)
        async let featureSumsTask = loadTasteFeatureSumsIfNeeded(currentUserID: currentUserID)
        let sagaResults = await sagaTask
        let featureSums = await featureSumsTask
        if isCurrentGeneration(generation) {
            sagaTitles = sagaResults
            tasteFeatureSums = featureSums
        }
        let sagaExcludeIDs = Set(sagaResults.map(\.id))

        async let providersTask = titleRepository.fetchProviders(for: titleID, region: watchProvidersRegion())
        async let relatedTask = titleRepository.fetchRelatedTitles(
            for: title,
            currentUserID: currentUserID,
            featureSums: featureSums,
            excludeIDs: sagaExcludeIDs
        )
        ...
        if isCurrentGeneration(generation) {
            self.trailerURL = fetchedTrailerURL.flatMap { $0 }
        ...
        if (needsTrailer || needsCast),
           let tmdbId = title.metadata.tmdbId, tmdbId > 0 {
            ...
            if let enriched = enrichment, isCurrentGeneration(generation) {
                applyEnrichedAssets(enriched, needsTrailer: needsTrailer, needsCast: needsCast)
            }
        }
