...
/// Numero di correlati mostrati nella corsia orizzontale prima di dover
/// aprire "Vedi tutte". Il resto (fino a 40, vedi `TitleRelatedGridSheet`)
/// resta a un tap di distanza.
private let relatedCarouselPreviewCount = 8

struct TitleRelatedCarouselSection: View {
    /// Titolo della pagina corrente: serve a `TitleRelatedGridSheet` per
    /// creare la proposta di correlato (`titleId` sorgente) e per il titolo
    /// del foglio di ricerca.
    let pageTitle: Title
    ...
    @State private var isRelatedSheetPresented = false
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                if relatedTitles.isEmpty {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 14) {
                            ForEach(relatedTitles.prefix(relatedCarouselPreviewCount)) { related in
                                NavigationLink {
        ...
        .sheet(isPresented: $isRelatedSheetPresented) {
            TitleRelatedGridSheet(
                pageTitle: pageTitle,
...
/// Riga "Saga": film della stessa collection TMDB (`title.metadata.collectionId`),
/// ordinati per anno. Il titolo della pagina corrente compare evidenziato e
/// non è un link — sei già qui. Il chiamante nasconde la sezione quando
/// `sagaTitles.count <= 1` (nessuna saga da mostrare).
struct TitleSagaSection: View {
    let pageTitleID: String
    let sagaTitles: [Title]
    let subtitle: String?
    let container: AppContainer
    let session: SessionStore
    let shell: AppShellStore
    ...
    var body: some View {
        TitleSectionCard {
            VStack(alignment: .leading, spacing: 14) {
                TitleSectionHeader(title: "Saga", subtitle: subtitle)

                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 14) {
                        ForEach(sagaTitles) { sagaTitle in
                            if sagaTitle.id == pageTitleID {
                                TitleRelatedCard(title: sagaTitle, isCurrent: true)
                            } else {
                                NavigationLink {
                                    TitleDetailView(container: container, session: session, shell: shell, titleID: sagaTitle.id)
                                } label: {
                                    TitleRelatedCard(title: sagaTitle)
                                }
                                .buttonStyle(.plain)
                            }
                        }
