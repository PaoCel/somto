...
struct TitleDetailView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .confirmationDialog(
                ...
                quickRatingDialogButtons
            } message: {
                quickRatingDialogMessage
            }
    ...
    private func toggleWatchlist() {
        ...
        // Solo un titolo FINITO è escluso da "Da vedere" (stessa regola del
        // backend, `toggle_watchlist` in functions/lib/titleStates.js, e del
        // web). Una serie iniziata ci rientra: prima bastava un episodio per
        // vedersi rispondere "Titolo già visto".
        if let personalState = viewModel.personalState,
           personalState.isCompleted,
    ...
    /// Fuori dal body: con quattro bottoni dentro `confirmationDialog` il
    /// type-checker non chiudeva piu' l'espressione in tempo utile.
    @ViewBuilder
    private var quickRatingDialogButtons: some View {
        // Post in un tocco: il voto e' l'azione frequente che gia'
        // esiste, il post e' quella che manca (4 autori in 30 giorni).
        // Testo gia' scritto, titolo attaccato, zero digitazione.
        Button("Pubblica il voto") {
            ...
            pendingQuickSeasonNumber = nil
            if let rating {
                publishQuickRatingPost(rating: rating, season: season)
            }
        }
    ...
    private var quickRatingDialogMessage: some View {
    ...
    /// Pubblica un post pubblico col voto appena dato, senza passare dal
    /// composer. Riusa la creazione post dei client (`PostsRepository`):
    /// niente backend nuovo, niente rules nuove.
    private func publishQuickRatingPost(rating: Double, season: Int?) {
        guard let uid = session.firebaseUser?.uid, let title = viewModel.title else { return }
        let score = RatingDisplayFormat.halfStep(rating)
        let text: String
        if let season {
            text = String(localized: "\(title.name), stagione \(season): \(score)/10")
        } else {
            text = String(localized: "\(title.name): \(score)/10")
        }
        Task {
            do {
                _ = try await container.postsRepository.createPost(
                    authorUID: uid,
    ...
    var body: some View {
        VStack(spacing: isCompactWidth ? 20 : 24) {
            ...
            VStack(spacing: 18) {
                ...
                // Il salto verso la piattaforma sta QUI, sopra le tab: e' cio'
                // che si viene a fare aprendo una serie che si sta guardando, e
                // in fondo alla Panoramica non lo trovava nessuno.
                TitleWatchNowButton(title: title, providers: viewModel.providers)
                ...
                Group {
                    ...
                    case .overview:
                        VStack(spacing: 18) {
                            ...
                            if viewModel.sagaTitles.count > 1 {
                                TitleSagaSection(
                                    pageTitleID: title.id,
                                    sagaTitles: viewModel.sagaTitles,
                                    subtitle: title.collectionName,
                                    container: container,
                                    session: session,
                                    shell: shell
                                )
