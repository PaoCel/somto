...
struct TitleSeriesSeasonsSection: View {
    ...
    @State private var isShowingEpisodeJump = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $isShowingEpisodeJump) {
            EpisodeJumpSheet(
                seasons: seasons,
                initialSeason: activeSeason?.seasonNumber ?? defaultSeasonNumber,
                initialEpisode: personalState?.seriesProgress?.lastWatchedEpisodeNumber ?? 1
            ) { seasonNumber, episodeNumber in
                selectedSeasonNumber = seasonNumber
                onSelectEpisodeProgress(seasonNumber, episodeNumber)
            }
        }
    }

    // MARK: - Salta a un episodio

    /// Come si segna il punto in cui sei: prima non lo diceva niente. Il tasto
    /// ✓ della riga e' un'icona muta, e chi sta a meta' della quinta stagione
    /// doveva scorrere fin li' per capirlo.
    @ViewBuilder
    private func episodeJumpRow(season: TitleSeason) -> some View {
        if episodeCount > 0 {
            VStack(alignment: .leading, spacing: 8) {
                Button {
                    isShowingEpisodeJump = true
                } label: {
                    HStack(spacing: 6) {
                        Image(systemName: "arrow.right.to.line")
                            .font(.system(size: 11, weight: .bold))
                        Text("Salta a un episodio")
                ...
                Text("Tocca la spunta dell'episodio dove sei arrivato: i precedenti diventano visti.")
                    .font(.caption)
...
/// Foglio "Dove sei arrivato?": stagione + episodio in due ruote, e un tasto
/// che sposta il segnalibro. Fa la stessa cosa del ✓ sulla riga episodio, ma
/// senza dover scorrere fino a quella riga — che su una serie da otto stagioni
/// e' il motivo per cui nessuno lo trovava.
private struct EpisodeJumpSheet: View {
    let seasons: [TitleSeason]
    ...
    init(
        ...
        _seasonNumber = State(initialValue: initialSeason)
        _episodeNumber = State(initialValue: max(1, initialEpisode))
    }

    /// Solo stagioni con episodi: gli "speciali" a zero non hanno niente da
    /// scegliere e romperebbero il range della ruota episodi.
    private var selectableSeasons: [TitleSeason] {
        let withEpisodes = seasons.filter { $0.episodeCount > 0 }
        return withEpisodes.isEmpty ? seasons : withEpisodes
    }

    /// Mai zero: `ForEach(1...0)` e' un crash, non una lista vuota.
    private var episodeCount: Int {
        let count = selectableSeasons.first { $0.seasonNumber == seasonNumber }?.episodeCount ?? 0
        return max(1, count)
    }

    private func seasonLabel(_ season: TitleSeason) -> String {
        if let name = season.name?.trimmingCharacters(in: .whitespacesAndNewlines),
           !name.isEmpty,
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    Picker("Stagione", selection: $seasonNumber) {
                        ForEach(selectableSeasons) { season in
                            Text(seasonLabel(season)).tag(season.seasonNumber)
                        }
                    }

                    Picker("Episodio", selection: $episodeNumber) {
                        ForEach(1...episodeCount, id: \.self) { number in
                            Text("Episodio \(number)").tag(number)
                        }
                    }
                } footer: {
                    Text("Segniamo visti tutti gli episodi fino a qui, comprese le stagioni precedenti.")
                }
            ...
            .navigationTitle("Dove sei arrivato?")
            ...
            // Cambiando stagione l'episodio scelto puo' finire fuori range:
            // si riporta dentro invece di mandare un numero che non esiste.
            .onChange(of: seasonNumber) { _, _ in
                episodeNumber = min(max(1, episodeNumber), episodeCount)
            }
        }
