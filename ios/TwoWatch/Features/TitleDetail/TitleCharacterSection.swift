    ...
    var body: some View {
        Group {
            if isUnlocked {
                ...
                // Ricarica anche quando l'arricchimento lazy popola il cast
                // dopo la prima apertura: keyed solo sull'uid restava vuota.
                .task(id: "\(session.firebaseUser?.uid ?? "")|\(title.castWithCharacters.count)") { await load() }
