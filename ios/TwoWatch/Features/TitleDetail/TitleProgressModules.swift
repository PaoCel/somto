...
struct SomtoSeriesProgressModule: View {
    ...
    private var metaRow: some View {
        HStack(spacing: 6) {
            ...
            Spacer(minLength: 4)
            // Senza totali attendibili la percentuale sarebbe uno 0% bugiardo
            // accanto a episodi realmente segnati: meglio non mostrarla.
            if let percentText {
                Text(percentText)
    ...
    private var mainStatementRow: some View {
        if let last = lastSE {
            ...
            // Il conteggio episodi vive qui e non solo nel ramo con S·E:
            // quando le stagioni del titolo non sono note la card deve
            // comunque muoversi a ogni "+1 episodio", altrimenti sembra rotta.
            HStack(alignment: .firstTextBaseline, spacing: 6) {
                Text(emptyHeadline)
    ...
    private var percentText: String? {
        if watchedCount > 0 && totalCount <= 0 && progressFraction <= 0 { return nil }
