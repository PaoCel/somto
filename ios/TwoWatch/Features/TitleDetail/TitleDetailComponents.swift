...
struct TitlePersonCard: View {
    ...
    /// Altezze fisse riservate per nome, pastiglia e personaggio/ruolo: senza,
    /// nomi/personaggi corti (1 riga) contro lunghi (2 righe) e la pastiglia
    /// opzionale (solo su alcune card) producono card di altezza diversa
    /// nella `HStack` di `TitleCreditsRow`, visibilmente disallineate.
    private static let nameBlockHeight: CGFloat = 36
    private static let badgeBlockHeight: CGFloat = 22
    private static let roleBlockHeight: CGFloat = 32
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(person.name)
                ...
                .frame(height: Self.nameBlockHeight, alignment: .top)
            ...
            // L'altezza è riservata anche quando non c'è nessuna delle due,
            // così le card senza pastiglia restano allineate alle altre.
            Group {
            ...
            .frame(height: Self.badgeBlockHeight, alignment: .topLeading)
            ...
            .multilineTextAlignment(.leading)
            .frame(height: Self.roleBlockHeight, alignment: .top)
        }
        .frame(width: 104, alignment: .leading)
        .padding(12)
        .background(TwoWatchTheme.panelStrong, in: RoundedRectangle(cornerRadius: 22, style: .continuous))
...
struct TitleRelatedCard: View {
    let title: Title
    /// Riga Saga: il titolo della pagina corrente compare nell'elenco ma non
    /// è un link (sei già qui) ed è evidenziato per orientarsi nella serie di
    /// film. `false` per i correlati normali.
    ...
    var body: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 24, style: .continuous)
                .stroke(isCurrent ? TwoWatchTheme.accent : TwoWatchTheme.border, lineWidth: isCurrent ? 2 : 1)
        )
