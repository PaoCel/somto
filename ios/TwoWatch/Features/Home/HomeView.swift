...
/// View model della testa personale della Home: hero "Cosa vuoi guardare
/// oggi?", "Continua a guardare", "Per te", uscite e "Novità per te". Il feed
/// sociale sta nella stessa schermata, sotto (`HomeStreamSection`), ma ha il
/// suo ViewModel (`CommunityViewModel`). "Tendenze" vive nella ricerca.
@Observable
@MainActor
final class HomeViewModel {
    ...
    func reload(userID: String?) async {
        ...
        do {
            ...
            // "Tendenze" non sta piu' qui: vive nello stato vuoto della ricerca.
            ...
            if let userID {
                ...
                async let forYouTask = homeRepository.fetchForYou()
                let (upcoming, launchpad, forYouTitles) = try await (
                    upcomingTask,
                    launchpadTask,
                    forYouTask
                )
                ...
                upcomingReleases = await upcomingTask
                hero = []
                continueWatching = []
                newForYou = []
                forYou = []
...
struct HomeView: View {
    ...
    /// Consiglio di verifica email: prima era un overlay ancorato al top dello
    /// ZStack di `RootView`, dove finiva sotto l'header flottante e copriva i
    /// titoli di ogni schermata. Ora e' una card della Home come le altre.
    @State private var emailVerification: HomeEmailVerificationBanner.State = .idle
    ...
    /// Il feed e le persone da seguire: la seconda meta' della Home
    /// (`HomeStreamSection`), stessi ViewModel della vecchia tab Community.
    @State private var community: CommunityViewModel
    @State private var people: PeopleToFollowViewModel
    /// Il post "Importa i tuoi dati" resta solo a chi non ha mai importato.
    @State private var hasImportHistory = false
    ...
    init(container: AppContainer, session: SessionStore, shell: AppShellStore) {
        ...
        _community = State(initialValue: Self.makeCommunityViewModel(container))
        _people = State(initialValue: Self.makePeopleViewModel(container))
    }

    private static func makeCommunityViewModel(_ container: AppContainer) -> CommunityViewModel {
        CommunityViewModel(
    ...
    private static func makePeopleViewModel(_ container: AppContainer) -> PeopleToFollowViewModel {
        PeopleToFollowViewModel(
            repository: container.peopleSuggestionsRepository,
        ...
        ScrollViewReader { scrollProxy in
            ScrollView {
                VStack(spacing: 18) {
                    ...
                    if showsEmailVerificationBanner {
                        HomeEmailVerificationBanner(
                            state: emailVerification,
                            onSendLink: { Task { await sendEmailVerificationLink() } },
                            onDismiss: {
                                withAnimation(.easeOut(duration: 0.2)) {
                                    emailVerification = .idle
                                    session.dismissEmailVerificationRecommendation()
                                }
                            }
                        )
            ...
            .task(id: session.firebaseUser?.uid) {
                guard !disablesAutomaticLoading else { return }
                await community.load(userID: session.firebaseUser?.uid)
            }
            ...
            .task(id: session.firebaseUser?.uid) {
                guard !disablesAutomaticLoading else { return }
                await people.loadIfNeeded(userID: session.firebaseUser?.uid)
            }
            .refreshable {
                guard !disablesAutomaticLoading else { return }
                await reloadAll()
    ...
    /// Le due meta' della Home si ricaricano insieme, non in fila.
    private func reloadAll() async {
        let userID = session.firebaseUser?.uid
        async let personal: Void = viewModel.reload(userID: userID)
        async let social: Void = community.reload(userID: userID)
        _ = await (personal, social)
    }

    /// Stesse condizioni dell'overlay che stava in `RootView`, piu' la coda per
    /// far leggere la conferma: il flag di sessione cade appena il link parte,
    /// e senza questo la card sparirebbe prima di dire che e' partito.
    private var showsEmailVerificationBanner: Bool {
        if case .sent = emailVerification { return true }
        return session.recommendsEmailVerification && !session.isLoading
    }

    /// Il flag di sessione si spegne solo a invio riuscito: se fallisce
    /// l'errore e' gia' nell'alert di `RootView` e la card resta richiedibile.
    private func sendEmailVerificationLink() async {
        emailVerification = .sending
        await session.sendRecommendedEmailVerification()
        withAnimation(.easeOut(duration: 0.2)) {
            emailVerification = session.recommendsEmailVerification ? .idle : .sent
        }
    }
    ...
    private func refreshImportBanner() async {
        ...
        hasImportHistory = active != nil || done != nil
    ...
    private var launchpadContent: some View {
        ...
        // Da qui in giu' la Home e' il flusso: composer a una riga e poi i
        // post, con "Per te", uscite e novita' inserite dentro
        // (`HomeStreamSection`). Le tue cose stanno sopra, in testa.
        if session.isAuthenticated {
            if isComposerExpanded {
                HomeComposerCard(container: container, session: session, shell: shell) { _ in
                    await community.reload(userID: session.firebaseUser?.uid)
                    isComposerExpanded = false
                }
                .padding(.horizontal, 14)
                .transition(.move(edge: .top).combined(with: .opacity))
            } else {
                HomeComposerRow(session: session) {
                    withAnimation(.easeInOut(duration: 0.22)) { isComposerExpanded = true }
                }
                .padding(.horizontal, 14)
            }
        }
    ...
    private var resumeLabel: String {
        if let season = state.seriesProgress?.lastWatchedSeasonNumber,
           let episode = state.seriesProgress?.lastWatchedEpisodeNumber {
            return String(localized: "Sei a S\(season)·E\(episode)")
        }
