...
struct FeedActivityCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // "Vai a guardarlo" senza passare dalla scheda: un post su un titolo
            // che sta su una piattaforma e' gia' la risposta a "cosa guardo
            // stasera", e la scheda nel mezzo e' un tap che non serve. Compare
            // solo quando sappiamo aprire un'app (link diretto o ricerca): con
            // il solo ripiego TMDB il bottone non c'e', e la card resta com'era.
            if let linkedTitle {
                TitleWatchNowButton(title: linkedTitle, providers: nil, style: .compact)
            }
    ...
    private var socialTaskID: String {
        let id = socialPostID ?? activity.id
        return "\(id)-\(shell.socialToken(forPostID: id))"
    }
