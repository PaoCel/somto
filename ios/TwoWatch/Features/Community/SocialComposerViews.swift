...
struct HomeComposerCard: View {
    ...
    private func submit() async {
        ...
        guard !rawText.isEmpty else {
            errorMessage = String(localized: "Scrivi qualcosa prima di pubblicare.")
            return
        }
        guard rawText.count <= composer.characterLimit else {
            errorMessage = String(localized: "Il post può avere massimo \(composer.characterLimit) caratteri.")
            return
        }
