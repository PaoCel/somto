        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            if viewModel.isLoading && viewModel.comments.isEmpty {
                ...
                LazyVStack(spacing: 0) {
                    ...
                    let visible = viewModel.comments.filter { session.blockedUserIDs.contains($0.uid) == false }
                    ForEach(Array(visible.enumerated()), id: \.element.id) { index, comment in
                        ...
                        // Stessa riga del dettaglio post e del foglio commenti:
                        // la conversazione non cambia forma a seconda di dove
                        // la apri.
                        PostCommentRow(
                            comment: comment,
                            viewModel: viewModel,
                            ...
                            shell: shell,
                            onReply: startReply,
                            moderationSource: "ios_feed_comments"
                        )
                    }
                }
            }
