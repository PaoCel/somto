...
final class ThreadDetailViewModel {
    ...
    func acceptCommunitySafety(currentUserID: String) async -> Bool {
        ...
        do {
            ...
            successMessage = String(localized: "Termini community accettati. Ora puoi usare la chat.")
            return true
        } catch {
            errorMessage = UserFacingError.message(for: error)
            return false
    ...
    func reportThread(from currentUserID: String) async {
        ...
        do {
            try await userRepository.submitModerationReport(
                ...
                type: "thread",
                targetId: threadID,
                reason: "Discussione segnalata per verifica moderazione.", // i18n-ok: valore canonico salvato per la moderazione
                metadata: [
                    "threadId": threadID,
                    "source": "ios_thread_header"
                ]
            )
            successMessage = String(localized: "Segnalazione inviata al team di moderazione.")
        } catch {
            errorMessage = UserFacingError.message(for: error)
        }
    }
    ...
    func reportMessage(_ message: ThreadMessage, from currentUserID: String) async {
        ...
        do {
            try await userRepository.submitModerationReport(
                ...
                type: "thread",
                targetId: threadID,
                reason: "Messaggio segnalato per contenuto o comportamento inappropriato.", // i18n-ok: valore canonico salvato per la moderazione
                metadata: [
                    "threadId": threadID,
                    "messageId": message.id,
            ...
            successMessage = String(localized: "Messaggio segnalato correttamente.")
        } catch {
            errorMessage = UserFacingError.message(for: error)
        }
    }
    ...
    func blockUser(_ targetUserID: String, from currentUserID: String) async {
        ...
        do {
            ...
            applyMessageVisibility()
            successMessage = String(localized: "Utente bloccato. I suoi messaggi non saranno più mostrati.")
        } catch {
            errorMessage = UserFacingError.message(for: error)
        }
    }
