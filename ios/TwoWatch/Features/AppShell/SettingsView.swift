...
struct SettingsView: View {
    ...
    /// Stato dell'email, non un consiglio: la riga resta finche' l'account e'
    /// email+password e non verificato. Il nudge di sessione
    /// (`recommendsEmailVerification`, card in Home) si spegne al primo invio,
    /// qui invece l'utente deve poterlo ritrovare quando lo cerca.
    private var isEmailUnverified: Bool {
        guard let user = session.firebaseUser else { return false }
        return container.authRepository.isUnverifiedPasswordUser(user)
    }
    ...
    private var accountSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if isEmailUnverified {
                Button {
                    Task { await sendEmailVerification() }
                } label: {
                    rowLabel(
                        icon: "envelope.badge.shield.half.filled",
                        title: String(localized: "Email non verificata"),
                        subtitle: isSendingEmailVerification
                            ? String(localized: "Invio…")
                            : String(localized: "Invia link"),
    ...
    /// Attribuzione TMDB. I loro termini API chiedono il logo **e** la frase,
    /// non una delle due: finora c'era solo una riga dentro la privacy del
    /// sito, e su iOS niente.
    private var dataSourcesSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            sectionTitle("Dati di film e serie")
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("Somto usa TMDB e le API di TMDB, ma non è approvato, certificato o altrimenti avallato da TMDB.")
                    .font(.caption)
                    .foregroundStyle(TwoWatchTheme.textSecondary)
    ...
    /// Stesso invio della card in Home: a buon fine spegne anche il consiglio
    /// di sessione, se no la Home continuerebbe a chiedere un link gia' partito.
    private func sendEmailVerification() async {
        isSendingEmailVerification = true
        defer { isSendingEmailVerification = false }
        do {
            try await container.authRepository.sendSignupVerification()
            session.dismissEmailVerificationRecommendation()
            infoMessage = String(localized: "Link inviato. Controlla anche lo spam.")
        } catch {
    ...
    private func sendPasswordReset() async {
        ...
        do {
            ...
            infoMessage = String(localized: "Ti abbiamo inviato un'email a \(email) con il link per cambiare la password.")
        } catch {
            errorMessage = AuthenticationRepository.friendlyMessage(for: error)
        }
    }
