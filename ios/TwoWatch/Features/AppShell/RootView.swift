...
struct RootView: View {
    ...
    /// Pre-prompt push rimandato: l'onboarding atterra su una scheda titolo e
    /// il prompt non puo' aprirsi sopra un cover gia' presentato (§
    /// `offerPushPromptIfNeeded`). Riparte alla chiusura della scheda.
    @State private var pendingPushPromptAfterOnboarding = false
    ...
    var body: some View {
        ...
        // Foglio, non overlay: si abbassa con lo swipe e ti riporta dov'eri
        // senza passare dalla tab bar. Il campo sta in fondo al foglio e la
        // tastiera si apre da sola (`SearchView`).
        .sheet(isPresented: $shell.isSearchPresented) {
            ...
            .presentationBackground { TwoWatchBackground() }
        }
        ...
        .onChange(of: shell.activePresentedDestination == nil) { _, isClear in
            // Secondo tempo del pre-prompt: l'onboarding lo rimanda perche' al
            // momento della chiusura c'e' gia' la scheda titolo sopra. Qui la
            // scheda e' stata chiusa e la Home e' di nuovo libera.
            guard isClear, pendingPushPromptAfterOnboarding else { return }
            pendingPushPromptAfterOnboarding = false
            offerPushPromptIfNeeded(after: 400_000_000)
        }
    ...
    private func offerPushPromptIfNeeded(after nanoseconds: UInt64) {
        ...
        Task { @MainActor in
            ...
            // (`landOnComments`, 250ms) e questo controllo scatta a 400ms: la
            // scheda c'e' sempre gia'. Prima qui si usciva e basta, quindi dal
            // 6 agosto (b7b73d4) il pre-prompt non e' piu' comparso a nessun
            // nuovo iscritto — zero token push su ~30 registrazioni. Non e' un
            // prompt da non mostrare, e' solo troppo presto: lo rimandiamo
            // alla chiusura della scheda.
            guard shell.activePresentedDestination == nil else {
                pendingPushPromptAfterOnboarding = true
                return
            }
...
struct AppTabShellView: View {
    ...
    var body: some View {
        ...
        Group {
            if #available(iOS 26.0, *) {
                // Da iOS 26 il tasto Cerca e' `Tab(role: .search)`: il sistema
                // lo stacca a destra della tab bar. Toccarlo apre il foglio di
                // ricerca, non una tab.
                TabView(selection: $shell.selectedTab) {
                    Tab("Home", systemImage: "house.fill", value: AppTab.home) {
                        homeTab(chromeVisible: chromeVisible)
                    }
                    Tab("Watchlist", systemImage: "bookmark.fill", value: AppTab.watchlist) {
                        watchlistTab(chromeVisible: chromeVisible)
                    }
                    Tab("Quiz", systemImage: "questionmark.circle.fill", value: AppTab.quiz) {
                        quizTab(chromeVisible: chromeVisible)
                    }
                    Tab("Profilo", systemImage: "person.crop.circle.fill", value: AppTab.profile) {
                        profileTab(chromeVisible: chromeVisible)
                    }
                    // Contenuto vuoto: la selezione viene rimessa subito sulla
                    // tab di prima e si apre il foglio (`AppShellStore.selectedTab`).
                    Tab("Cerca", systemImage: "magnifyingglass", value: AppTab.search, role: .search) {
                        Color.clear
                    }
                }
            } else {
                // iOS 17/18: tab bar classica, ricerca = lente sopra la barra
                // (`FloatingSearchButton`) che apre lo stesso foglio.
                TabView(selection: $shell.selectedTab) {
                    homeTab(chromeVisible: chromeVisible)
    ...
    private func homeTab(chromeVisible: Bool) -> some View {
        NavigationStack {
    ...
    private func watchlistTab(chromeVisible: Bool) -> some View {
        NavigationStack {
    ...
    private func quizTab(chromeVisible: Bool) -> some View {
        NavigationStack {
    ...
    private func profileTab(chromeVisible: Bool) -> some View {
        NavigationStack {
