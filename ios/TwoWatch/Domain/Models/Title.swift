...
struct Title: Identifiable, Hashable {
    ...
    let keywords: [String]
    let collectionName: String?
    /// Id di `titles` correlati curati (`titles.related`), scritti da admin/
    /// trusted o auto-approvati a 3 proposte utente distinte
    /// (`titleRelatedSuggestions`). Non filtrati per `status`: il chiamante
    /// (`TitleRepository.fetchRelatedTitles`) scarta i non-approvati dopo il
    /// fetch batch, stesso schema del web (`getRelatedTitlesUnified`).
    let related: [String]
    ...
    /// denormalizzati su `titles/{id}.watchDeepLinks` (mappa name→url) dallo
    /// stesso modulo che li risolve per la scheda titolo
    /// (`functions/modules/streamingLinks.js`).
    ///
    /// PERCHE' ANCHE QUI e non solo nella risposta della callable: il widget non
    /// chiama nessuna callable, legge lo snapshot costruito dai `Title` che
    /// l'app ha gia' in mano. Finche' questo campo non veniva letto, il tasto
    /// play del widget ripiegava SEMPRE sulla ricerca — anche sui titoli per cui
    /// il link esatto esisteva gia' in Firestore.
    let watchDeepLinks: [String: URL]
    ...
    init(
        ...
        self.watchDeepLinks = watchDeepLinks
    }
    ...
    /// Copia con `trailerURL`/`castWithCharacters` aggiornati dopo l'arricchimento
    /// lazy in `TitleDetailViewModel.loadDeferredDetailData`. Senza questa copia il
    /// modello in memoria restava con `castWithCharacters` vuoto anche a fetch
    /// riuscita: i flussi di voto che leggono `title` invece di `castCredits`
    /// (`RatingPostComposerSheet`, `EpisodeSeenSheet`, il pick personaggio) via
    /// `fetchTitleCharacterCandidates(title:)` continuavano a mostrare "Cast non
    /// disponibile" finche' la scheda non veniva riaperta. Un parametro `nil`/vuoto
    /// lascia invariato il valore corrente.
    func withEnrichedAssets(trailerURL newTrailerURL: URL?, castWithCharacters newCast: [TitleCastMember]) -> Title {
        Title(
            id: id,
            name: name,
            nameLower: nameLower,
            ...
            updatedAt: updatedAt,
            tmdbNextRefreshAt: tmdbNextRefreshAt,
            trailerURL: newTrailerURL ?? trailerURL,
            castWithCharacters: newCast.isEmpty ? castWithCharacters : newCast,
            watchProviderNames: watchProviderNames,
            watchProviderLogos: watchProviderLogos,
            watchDeepLinks: watchDeepLinks
        )
    }
}
