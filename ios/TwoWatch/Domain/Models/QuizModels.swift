...
/// Franchise giocabile: piu' titoli con quiz raggruppati (Harry Potter, MCU,
/// Star Wars...). Viene dall'aggregato pubblico `quizMeta/sagas`, scritto dal
/// backend (`functions/modules/quizSagas.js`): il client non lo ricalcola.
struct QuizSaga: Identifiable, Hashable {
    var id: String { sagaId }
    /// Slug leggibile, e' anche la chiave del deep link web `?saga=`.
    let sagaId: String
    let name: String
    let mediaType: QuizMediaType
    /// Titoli da cui pescare le domande, ordinati come nell'aggregato.
    let titleIds: [String]
    /// Domande giocabili in tutta la saga.
    let questionCount: Int
}
...
struct QuizChallenge: Identifiable, Codable, Hashable {
    ...
    /// Link d'invito, scritto dal server alla creazione. `nil` sugli inviti
    /// creati prima: li' il link si puo' solo ricreare, non ricondividere.
    var inviteUrl: String? = nil

    // MARK: Saga

    /// Sfida su un'intera saga: `sharedTitleIds` porta gia' tutti i titoli del
    /// franchise, questi due servono solo a poter dire *su cosa* si gioca.
    /// `nil` sulle sfide su titolo singolo e su tutte quelle create prima.
    var sagaId: String? = nil
    var sagaName: String? = nil
    ...
    /// Vero solo quando l'avversario ha un nome vero: quello reale o quello
    /// scritto da chi invita. L'etichetta di ripiego non e' un nome, e chi la
    /// usa come tale (l'iniziale nell'avatar) deve saperlo.
    var hasOpponentName: Bool {
        if let opponentDisplayName, !opponentDisplayName.isEmpty { return true }
        if let opponentPlaceholderName, !opponentPlaceholderName.isEmpty { return true }
        return false
    }
    ...
    var opponentLabel: String {
        ...
        // Senza nome, "Invito esterno" faceva di tre inviti in attesa tre
        // righe identiche: la data di creazione e' l'unica cosa che li
        // distingue finche' non si registra qualcuno.
        if isAwaitingExternalSignup {
            let day = createdAt.formatted(.dateTime.day().month(.abbreviated))
            return String(localized: "Invito del \(day)")
        }
