...
protocol TitleRepositoryProtocol: AnyObject, Sendable {
    ...
    /// Correlati unificati (curati + collaborativi + fallback genere), gia'
    /// ordinati da `RelatedTitlesRanking`. Vedi `TitleRepository.fetchRelatedTitles`
    /// per la spec completa delle letture e il mirror web.
    func fetchRelatedTitles(
        for title: Title,
        currentUserID: String?,
        featureSums: TasteFeatureSums?,
    ...
    /// `featureSums` del tasteProfile dell'utente, per l'affinita' gusti dei
    /// correlati. `nil` se assente o su qualunque errore.
    func fetchTasteFeatureSums(userID: String) async throws -> TasteFeatureSums?
    /// Crea `titleRelatedSuggestions/{uid}__{titleId}__{suggestedTitleId}`;
    /// auto-approvata server-side alla terza proposta utente distinta.
...
extension TitleRepositoryProtocol {
    ...
    /// Default coerente col mirror web (`RELATED_CANDIDATES_MAX = 40`).
    func fetchRelatedTitles(
        for title: Title,
        currentUserID: String?,
