...
struct WatchlistWidgetSnapshot: Codable, Sendable, Hashable {
    struct Row: Codable, Sendable, Hashable, Identifiable {
        ...
        /// Lo STESSO provider, ma col nome che TMDB gli da' per esteso.
        ///
        /// `providerName` e' gia' collassato per essere mostrato ("HBO Max"),
        /// e cosi' facendo perde l'unica traccia del fatto che quel marchio si
        /// guarda dentro Prime Video: il suffisso "Amazon Channel". Senza,
        /// 837 titoli del catalogo (misurati l'8/9/2026) finivano sulla pagina
        /// TMDB invece che nell'app dove si guardano davvero.
        ///
        /// Opzionale perche' i riassunti scritti prima non ce l'hanno: in quel
        /// caso si ricade su `providerName`, cioe' sul comportamento di prima.
        let providerSearchName: String?
        /// Il link che porta a guardarlo, quando e' quello esatto.
        ///
        /// Lo riempie `WatchlistWidgetSnapshotBuilder` da `titles.watchDeepLinks`,
        /// cioe' dai link che il server risolve su Wikidata: aprono la scheda del
        /// titolo dentro l'app della piattaforma. Copertura misurata su prod
        /// l'8/9/2026: 1121 titoli su 2825 con provider italiani.
        ///
        /// `nil` = per quella piattaforma non abbiamo l'id (Prime Video non ne ha
        /// di validi per l'Italia, vedi `StreamingDestination`): il widget
        /// ripiega sulla ricerca dentro la stessa app.
        let watchUrl: URL?
...
enum WatchlistWidgetSnapshotStore {
    ...
    /// Vero quando il file c'e' ma non si riesce a leggere: succede solo prima
    /// del primo sblocco dopo un riavvio. Il widget lo usa per ritentare
    /// presto invece di aspettare il giro successivo.
    static var isPresentButUnreadable: Bool {
    ...
    static func write(_ snapshot: WatchlistWidgetSnapshot) {
        ...
        // `.completeFileProtectionUntilFirstUserAuthentication`: il widget si
        // aggiorna anche a telefono bloccato (WidgetKit lo rinfresca quando
        // vuole, spesso di notte), e questa e' l'unica classe che resta
        // leggibile da bloccato dopo il primo sblocco. `UnlessOpen`, usata fino
        // al 2026-09-06, rendeva il file illeggibile proprio in quei giri:
        // `read()` tornava nil e il widget mostrava "Apri Somto" anche se il
        // riassunto c'era. Stessa classe del keychain del bottone
        // (`AfterFirstUnlock`, in WidgetAuthBridge).
    ...
    static func applyProgress(
        ...
        let rows = current.rows.map { row -> WatchlistWidgetSnapshot.Row in
            ...
            return WatchlistWidgetSnapshot.Row(
                ...
                providerSearchName: row.providerSearchName,
...
enum WatchlistWidgetPendingStore {
    ...
    /// Lo stato da DISEGNARE, che non e' sempre quello scritto: un `inFlight`
    /// piu' vecchio della finestra non e' piu' in volo (il processo del widget
    /// puo' morire senza chiudere il giro), e disegnarlo lascerebbe il bottone
    /// a girare per sempre.
    static func displayState(
        for titleId: String,
        now: Date = Date()
    ) -> WatchlistWidgetPendingEntry.State? {
        guard let entry = read()[titleId] else { return nil }
        if entry.state == .inFlight, now.timeIntervalSince(entry.at) >= inFlightWindow { return nil }
        return entry.state
    }
