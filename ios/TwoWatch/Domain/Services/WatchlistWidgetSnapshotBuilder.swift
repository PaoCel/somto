...
enum WatchlistWidgetSnapshotBuilder {
    ...
    private static func row(
        ...
        return WatchlistWidgetSnapshot.Row(
            ...
            // Il nome per esteso della stessa piattaforma: e' quello che il
            // widget passa al resolver, perche' e' li' che sopravvive il
            // suffisso "Amazon Channel".
            providerSearchName: providerName.flatMap { name in
                title.watchProviderNames.first {
                    StreamingProviderRanking.baseName($0).lowercased() == name.lowercased()
                }
            },
            // Il link diretto al titolo dentro la piattaforma, quando il server
            // l'ha risolto (Wikidata -> `titles.watchDeepLinks`). Se manca resta
            // `nil` e il widget ripiega sulla ricerca dentro l'app della
            // piattaforma, come faceva prima.
            //
            // SOLO IL LINK DELLA PIATTAFORMA MOSTRATA — non il migliore fra
            // tutte: il widget fa vedere un logo solo, e mandare su Disney+ chi
            // ha toccato play sotto il logo Netflix e' peggio di una ricerca.
            watchUrl: directWatchLink(for: providerName, in: title),
    ...
    /// Il link diretto per la piattaforma che il widget mostra.
    ///
    /// I nomi sono testo di TMDB e il server li salva con la sua forma canonica
    /// ("Disney Plus", non "Disney+"): si prova il nome cosi' com'e', poi quello
    /// collassato da `StreamingProviderRanking`, poi un confronto lasco su tutte
    /// le chiavi. La stessa scala che fa `StreamingDestinationResolver`, che qui
    /// non si puo' chiamare: quello risolve gia' il ripiego, e il ripiego lo
    /// deve calcolare il widget al momento del disegno (il nome del titolo puo'
    /// essere cambiato nel frattempo).
    static func directWatchLink(for providerName: String?, in title: Title) -> URL? {
        guard let providerName, !providerName.isEmpty, !title.watchDeepLinks.isEmpty else { return nil }
        if let exact = title.watchDeepLinks[providerName] { return exact }
        let base = StreamingProviderRanking.baseName(providerName).lowercased()
        return title.watchDeepLinks.first { StreamingProviderRanking.baseName($0.key).lowercased() == base }?.value
    }
