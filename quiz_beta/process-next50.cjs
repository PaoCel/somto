 ...
 * process-next50.cjs
 *
 * Merge dei file generati dal fan-out (quiz_beta/next50/q_<rank>.json, ognuno un
 ...
 *
 * Output (in quiz_beta/):
 *  - quiz_questions_next50_import_ready.json   ({ questions: [...] }, pronto per import)
 *  - quiz_questions_next50_medium_review.json  (confidence==medium)
 *  - quiz_questions_next50_heavy_spoiler_review.json (spoilerLevel==heavy)
 *  - _next50_cleanup_stats.json
 *
 * Nessuna scrittura su DB. Run: node quiz_beta/process-next50.cjs
 */
const fs = require('fs');
const path = require('path');

const DIR = path.resolve(__dirname, 'next50');
const TITLES = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'quiz_next50_titles.json'), 'utf8'));
...
const importReady = {
  generatedAt: stamp,
  source: 'Somto quiz batch acquisizione (next 50 titles, rank 81..130)',
...
W('quiz_questions_next50_import_ready.json', importReady);
W('quiz_questions_next50_medium_review.json', { generatedAt: stamp, count: mediumReview.length, questions: mediumReview });
W('quiz_questions_next50_heavy_spoiler_review.json', { generatedAt: stamp, count: heavySpoiler.length, questions: heavySpoiler });
const report = {
...
W('_next50_cleanup_stats.json', report);
