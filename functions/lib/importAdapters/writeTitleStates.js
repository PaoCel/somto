...
function buildSeriesImportState(currentState, title, rows, options = {}) {
  ...
  // Netflix non etichetta con "Stagione N" le serie che ne hanno una sola
  // ("House of Guinness: Il nome dell'episodio"), quindi nessuna riga porta un
  // numero di stagione e il badge del profilo restava senza stagione anche a
  // serie interamente vista. Quando il catalogo dice che di stagioni ce n'e'
  // UNA, quale sia non e' un'inferenza: e' la 1. Sulle serie multi-stagione
  // non si indovina — li' la stagione si potrebbe ricavare solo risolvendo il
  // NOME dell'episodio contro TMDB (vedi docs/PENDING.md).
  const singleSeasonFallback = toPositiveInt(totals.totalSeasonCount) === 1 ? 1 : null;
  const lastWatchedSeasonNumber = toPositiveInt(lastRow?.seasonNumber)
    || maxSeason
    || singleSeasonFallback;
  const next = buildNextTitleState(currentState, {
