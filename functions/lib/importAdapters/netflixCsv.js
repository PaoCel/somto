...
// Marker di stagione "brandizzato": Netflix etichetta alcune serie con il
// PROPRIO nome piu' il numero di stagione invece di "Stagione N"
// ("Stranger Things: Stranger Things 4: Capitolo quattro: Caro Billy",
// "Emily in Paris: Emily in Paris 3: ..."). Senza questo la riga cadeva nel
// ramo `ambiguous`: stagione null, `seasonsCompletedCount` fermo a 0 e nessun
// numero di stagione sul badge del profilo, anche a serie interamente vista
// (caso reale: import Netflix 2026-09-08, 38 righe di Stranger Things su 156).
// Il confronto e' sul nome serie del segmento 0, quindi non puo' scambiare per
// stagione un episodio che comincia per numero.
function matchBrandedSeasonSegment(segment, seriesName) {
  const series = String(seriesName || "").trim();
  if (!series) return null;
  const escaped = series.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const m = String(segment || "").trim().match(new RegExp(`^${escaped}\\s+(\\d{1,2})$`, "i"));
  if (!m) return null;
  const n = Number(m[1]);
  return n > 0 ? n : null;
}
...
function parseNetflixTitleCell(rawTitle) {
  ...
  // --- "<Nome serie> N" come marker di stagione (vedi matchBrandedSeasonSegment) ---
  const brandedSeason = matchBrandedSeasonSegment(rest[0], seriesName);
  if (brandedSeason != null && rest.length >= 2) {
    const after = rest.slice(1);
    const episodeSegments = stripTrailingPartSuffixFromSegments(after);
    if (episodeSegments.length === 0) {
      const parteEpisode = matchLoneParteEpisodeSegment(after[after.length - 1]);
      return {
        ...
        seriesNameGuess: seriesName,
        movieNameGuess: null,
        seasonNumber: brandedSeason,
        episodeNumber: parteEpisode,
        episodeNameGuess: parteEpisode != null ? null : (after.join(": ") || null),
      };
    }
    const epNumber = episodeSegments.length === 1
      ? matchChapterOrEpisodeNumberSegment(episodeSegments[0])
      : null;
    return {
      kind: "tv_episode",
      seriesNameGuess: seriesName,
