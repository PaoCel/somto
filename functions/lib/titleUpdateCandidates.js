...
function pickRegionalMovieRelease(payload, region = "IT") {
  ...
  // COSA CONTA COME "USCITA IN ITALIA": sala (3), sala limitata (2), digitale
  // (4), TV (6). In quest'ordine, perche' e' l'ordine in cui una persona puo'
  // davvero vedere il film.
  //
  // Fuori restano di proposito due tipi TMDB:
  //  * 1 anteprima — una proiezione di festival non e' un'uscita. Il 2026-08-29
  //    tre film in cartellone a Venezia (Primetime, Father Joe, Possible Love)
  //    erano stati annunciati come "arriva dal 5 settembre": a quella data non
  //    erano vedibili da nessuna parte.
  //  * 5 supporto fisico — un Blu-ray non e' l'uscita di un film.
  //
  // Un film che in Italia ha SOLO una di queste due date non ha una data
  // italiana: cade sul fallback globale, diventa `missing_it_release_date` e
  // resta in bozza. E' la stessa dottrina del widget: una data da verificare
  // non e' una data.
  const priorities = new Map([[3, 0], [2, 1], [4, 2], [6, 3]]);
