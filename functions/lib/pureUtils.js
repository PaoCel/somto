...
// I contatori engagement (`users/{uid}.engagement.*`) sono stati scritti a lungo
// con `batch.set()` e chiavi puntate: in `set()` il punto non e' un percorso, e'
// parte del nome del campo, quindi il valore finiva in un campo letterale di
// primo livello ("engagement.lastNudgeAt") e la lettura annidata non lo trovava
// mai. Questi due leggono l'annidato e, se manca, il campo letterale, cosi' i
// doc vecchi continuano a valere finche' non vengono riscritti.
function engagementMillis(userData, key) {
  const nested = userData?.engagement?.[key];
  const legacy = userData?.[`engagement.${key}`];
  return toMillis(nested) || toMillis(legacy);
}

function engagementNumber(userData, key) {
  const nested = userData?.engagement?.[key];
  const legacy = userData?.[`engagement.${key}`];
  return Number(nested ?? legacy ?? 0) || 0;
}
