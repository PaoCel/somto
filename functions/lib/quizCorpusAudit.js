...
const STRICT_MISSING_ACCENT_RE = /\b(?:perche|puo|piu|gia|cosi|citta)\b|\bqual\s+e\b/iu;
...
function answerLengths(question) {
  if (!Array.isArray(question?.answers) || question.answers.length !== 4) return null;
  return question.answers.map((answer) => text(answer).length);
}

function strongLengthTell(question) {
  const lengths = answerLengths(question);
  const correctAnswerIndex = question?.correctAnswerIndex;
  if (!lengths || !Number.isInteger(correctAnswerIndex) || correctAnswerIndex < 0 || correctAnswerIndex > 3) return null;

  const correctLength = lengths[correctAnswerIndex];
  const longest = Math.max(...lengths);
  const shortest = Math.min(...lengths);
  const uniqueLongest = lengths.filter((length) => length === longest).length === 1;
  if (!uniqueLongest || correctLength !== longest || correctLength < shortest * 1.5) return null;

  return { lengths, correctLength, shortestLength: shortest };
}

function hasStrictMissingAccent(question) {
  const visibleTexts = [question?.questionText, ...(Array.isArray(question?.answers) ? question.answers : []), question?.explanation];
  return visibleTexts.some((value) => STRICT_MISSING_ACCENT_RE.test(text(value)));
}
