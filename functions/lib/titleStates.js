...
function seasonRowsWithEpisodes(totals) {
  return safeArray(totals?.seasons).filter((row) => toPositiveInt(row?.episodes) > 0);
}

// Mappa un contatore cumulativo di episodi (1-based) sulla coppia
// (stagione, episodio) camminando le righe reali di `meta.seasons`.
//
// Prima esisteva solo la divisione per `episodesPerSeason`, che è valorizzato
// SOLO quando tutte le stagioni hanno la stessa lunghezza: su una serie
// normale (Breaking Bad, Squid Game…) restava null e quindi
// `lastWatchedSeasonNumber` restava null per sempre. Conseguenza a valle:
// niente "Sei a S1 · E3" sulla card e nessun foglio post-episodio su iOS
// (pretende season > 0 && episode > 0), cioè il tasto "+1 episodio"
// sembrava non fare niente.
function locateSeriesPosition(cumulativeEpisodes, totals) {
  const count = toPositiveInt(cumulativeEpisodes);
  if (count <= 0) return null;

  const rows = seasonRowsWithEpisodes(totals);
  if (rows.length) {
    let remaining = count;
    let seasonsCompleted = 0;
    for (const row of rows) {
      const episodes = toPositiveInt(row.episodes);
      if (remaining <= episodes) {
        return {
          season: toPositiveInt(row.season),
          episode: remaining,
          seasonsCompleted: remaining === episodes ? seasonsCompleted + 1 : seasonsCompleted,
        };
      }
      remaining -= episodes;
      seasonsCompleted += 1;
    }
    // Contatore oltre l'ultima stagione conosciuta (TMDB non ancora
    // aggiornato): resta ancorato alla fine dell'ultima riga nota.
    const last = rows[rows.length - 1];
    return {
      season: toPositiveInt(last.season),
      episode: toPositiveInt(last.episodes),
      seasonsCompleted,
    };
  }

  const perSeason = toPositiveInt(totals?.episodesPerSeason);
  if (perSeason > 0) {
    return {
      season: Math.max(1, Math.ceil(count / perSeason)),
      episode: Math.max(1, ((count - 1) % perSeason) + 1),
      seasonsCompleted: Math.floor(count / perSeason),
    };
  }
...
// Episodi cumulativi coperti dalle prime `seasonsCompleted` stagioni.
function episodesThroughSeasons(seasonsCompleted, totals) {
  const count = toPositiveInt(seasonsCompleted);
  if (count <= 0) return 0;
  const rows = seasonRowsWithEpisodes(totals);
  if (rows.length) {
    return rows.slice(0, count).reduce((sum, row) => sum + toPositiveInt(row.episodes), 0);
  }
  const perSeason = toPositiveInt(totals?.episodesPerSeason);
  return perSeason ? count * perSeason : 0;
}

// Numero di stagione + lunghezza della n-esima stagione (1-based).
function seasonRowAt(seasonsCompleted, totals) {
  const count = toPositiveInt(seasonsCompleted);
  if (count <= 0) return null;
  const rows = seasonRowsWithEpisodes(totals);
  if (!rows.length) return null;
  const row = rows[Math.min(count, rows.length) - 1];
  return { season: toPositiveInt(row.season), episodes: toPositiveInt(row.episodes) };
}

// Ultimo episodio dell'ultima stagione: la coordinata giusta per una serie
// completata. `episodesPerSeason` da solo sbagliava (o restava null) su tutte
// le serie con stagioni di lunghezza diversa.
function lastEpisodeOfLastSeason(totals) {
  const rows = seasonRowsWithEpisodes(totals);
  if (rows.length) return toPositiveInt(rows[rows.length - 1].episodes) || null;
  return toPositiveInt(totals?.episodesPerSeason) || null;
}
  ...
  if (mediaType === "tv") {
    ...
    if (baseState === "rated" || baseState === "completed_unrated") {
      seriesProgress = {
        ...
        lastWatchedEpisodeNumber: lastEpisodeOfLastSeason(totals) || seriesProgress.lastWatchedEpisodeNumber,
        percentComplete: 1,
      };
    }
  }
...
function buildCompletedSeriesProgress(currentProgress, totals, now) {
  ...
  return {
    ...
    lastWatchedEpisodeNumber: lastEpisodeOfLastSeason(totals) || progress.lastWatchedEpisodeNumber,
    lastWatchedAt: now,
    percentComplete: 1,
  };
}
...
function buildInProgressSeriesState(currentState, action, title, options = {}) {
  ...
  if (action.type === "mark_series_episode") {
    ...
    progress.episodesWatchedCount = totals.totalEpisodeCount
      ...
      : nextEpisodes;
    const located = locateSeriesPosition(progress.episodesWatchedCount, totals);
    if (located) {
      progress.seasonsCompletedCount = Math.max(progress.seasonsCompletedCount, located.seasonsCompleted);
      progress.lastWatchedSeasonNumber = located.season || null;
      progress.lastWatchedEpisodeNumber = located.episode || null;
    } else {
      // Nessuna informazione sulle stagioni: si tiene il contatore grezzo,
      // senza inventare una stagione.
      progress.lastWatchedEpisodeNumber = progress.episodesWatchedCount || null;
    }
    Object.assign(
      progress,
      buildEpisodeIdentity(progress.lastWatchedSeasonNumber, progress.lastWatchedEpisodeNumber)
    );
    ...
    progress.seasonsCompletedCount = totals.totalSeasonCount
      ...
      : nextSeasons;
    const covered = episodesThroughSeasons(progress.seasonsCompletedCount, totals);
    if (covered) {
      progress.episodesWatchedCount = Math.max(progress.episodesWatchedCount, covered);
      if (totals.totalEpisodeCount) {
        progress.episodesWatchedCount = Math.min(totals.totalEpisodeCount, progress.episodesWatchedCount);
      }
    }
    const row = seasonRowAt(progress.seasonsCompletedCount, totals);
    const lastSeasonNumber = row?.season || progress.seasonsCompletedCount || null;
    const lastEpisodeNumber = row?.episodes || totals.episodesPerSeason || null;
