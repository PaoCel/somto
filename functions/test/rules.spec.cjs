...
describe('Notification push outcome (pushDelivery)', () => {
  // Il campo lo scrive solo pushOnNotificationCreate (Admin SDK). Il client
  // deve continuare a segnare letto — web con updateDoc, iOS con
  // setData(merge) — ma non puo' riscrivere l'esito che la guardia conta.
  ...
  const seeded = () => ({
    ...
    pushDelivery: {
      ...
      deadTokens: 0,
      errorCodes: [],
      at: new Date('2026-09-10T08:00:01Z'),
    },
  });
  const legacy = () => {
    const row = seeded();
    delete row.pushDelivery;
    return row;
  };

  it('owner marks read from web and iOS with the outcome on the doc', async () => {
    await seedDoc(path, seeded());
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    await assertSucceeds(updateDoc(doc(aliceDb, path), { read: true }));
    await assertSucceeds(setDoc(doc(aliceDb, path), { read: false }, { merge: true }));
  });

  it('owner marks read on an older notification without the outcome', async () => {
    await seedDoc(path, legacy());
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    await assertSucceeds(updateDoc(doc(aliceDb, path), { read: true }));
  });

  it('owner cannot rewrite or add the outcome', async () => {
    await seedDocs([[path, seeded()], ['users/alice/notifications/n2', legacy()]]);
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    await assertFails(updateDoc(doc(aliceDb, path), { 'pushDelivery.status': 'delivered' }));
    await assertFails(updateDoc(doc(aliceDb, 'users/alice/notifications/n2'), {
      read: true,
      pushDelivery: { status: 'delivered' },
    }));
  });
});
...
describe('Signup lifecycle server-only', () => {
  it('pending, attribution e deletion lifecycle sono deny-all per ogni client', async () => {
    await seedDocs([
      ['users/adm', { displayName: 'Adm', displayNameLower: 'adm', isAdmin: true }],
      ['pendingSignups/alice', { uid: 'alice', status: 'pending', requestedDisplayName: 'Alice' }],
      ['accountDeletionRequests/alice', { uid: 'alice', status: 'completed', source: 'self-serve' }],
      ['users/alice/_system/signupAttribution', { schemaVersion: 1, surface: 'web' }],
    ]);
    const contexts = [
      testEnv.authenticatedContext('alice').firestore(),
      testEnv.authenticatedContext('bob').firestore(),
      testEnv.authenticatedContext('adm').firestore(),
      testEnv.unauthenticatedContext().firestore(),
    ];

    for (const db of contexts) {
      await assertFails(getDoc(doc(db, 'pendingSignups/alice')));
      await assertFails(getDoc(doc(db, 'accountDeletionRequests/alice')));
      await assertFails(getDoc(doc(db, 'users/alice/_system/signupAttribution')));
      await assertFails(setDoc(doc(db, 'pendingSignups/alice'), { status: 'completed' }, { merge: true }));
      await assertFails(setDoc(doc(db, 'accountDeletionRequests/alice'), { status: 'failed' }, { merge: true }));
      await assertFails(setDoc(doc(db, 'users/alice/_system/signupAttribution'), { surface: 'ios' }, { merge: true }));
    }
  });
});
...
describe('Suggerimenti server (users/{uid}/suggestions)', () => {
  it('il proprietario legge la propria cache, nessun client la scrive', async () => {
    await seedDoc('users/alice/suggestions/people', {
      people: [{ uid: 'bob', displayName: 'Bob', sharedCount: 3 }],
      computedAt: new Date(),
    });

    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    await assertSucceeds(getDoc(doc(aliceDb, 'users/alice/suggestions/people')));
    await assertFails(setDoc(doc(aliceDb, 'users/alice/suggestions/people'), {
      people: [{ uid: 'chi_voglio_io', displayName: 'Fake', sharedCount: 99 }],
    }));
  });

  it('i suggerimenti di un altro utente non si leggono', async () => {
    await seedDoc('users/alice/suggestions/people', { people: [], computedAt: new Date() });
    const bobDb = testEnv.authenticatedContext('bob').firestore();
    await assertFails(getDoc(doc(bobDb, 'users/alice/suggestions/people')));
  });
});
...
// Suggerimenti community di titoli collegati (CONTRACT 1). Id composto
// uid__titleId__suggestedTitleId, create solo status:"pending" e createdAt
// server, update/delete solo admin.
describe('titleRelatedSuggestions (community: titoli collegati)', () => {
  const suggestionId = 'alice__title_a__title_b';
  const validPayload = () => ({
    uid: 'alice',
    titleId: 'title_a',
    suggestedTitleId: 'title_b',
    status: 'pending',
    createdAt: serverTimestamp(),
  });

  it('il proponente puo\' creare con id composto e status pending', async () => {
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    await assertSucceeds(setDoc(doc(aliceDb, `titleRelatedSuggestions/${suggestionId}`), validPayload()));
  });

  it('creare a nome di un altro uid e\' negato', async () => {
    const bobDb = testEnv.authenticatedContext('bob').firestore();
    await assertFails(setDoc(doc(bobDb, `titleRelatedSuggestions/${suggestionId}`), validPayload()));
  });

  it('uno status diverso da pending in create e\' negato', async () => {
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    await assertFails(setDoc(doc(aliceDb, `titleRelatedSuggestions/${suggestionId}`), {
      ...validPayload(),
      status: 'approved',
    }));
  });

  it('titleId uguale a suggestedTitleId e\' negato', async () => {
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    const id = 'alice__title_a__title_a';
    await assertFails(setDoc(doc(aliceDb, `titleRelatedSuggestions/${id}`), {
      ...validPayload(),
      titleId: 'title_a',
      suggestedTitleId: 'title_a',
    }));
  });

  it('un id che non rispetta la chiave composta e\' negato', async () => {
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    await assertFails(setDoc(doc(aliceDb, 'titleRelatedSuggestions/alice__wrong__id'), validPayload()));
  });

  it('createdAt diverso da request.time e\' negato', async () => {
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    await assertFails(setDoc(doc(aliceDb, `titleRelatedSuggestions/${suggestionId}`), {
      ...validPayload(),
  ...
  it('un campo extra fuori dalla hasOnly e\' negato', async () => {
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    await assertFails(setDoc(doc(aliceDb, `titleRelatedSuggestions/${suggestionId}`), {
      ...validPayload(),
      reviewedBy: 'alice',
  ...
  it('il proponente legge il proprio suggerimento, un altro utente no, admin si\'', async () => {
    await seedDocs([
      [`titleRelatedSuggestions/${suggestionId}`, { uid: 'alice', titleId: 'title_a', suggestedTitleId: 'title_b', status: 'pending' }],
      ['users/adm', { displayName: 'Adm', displayNameLower: 'adm', isAdmin: true }],
    ]);
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    const bobDb = testEnv.authenticatedContext('bob').firestore();
    const admDb = testEnv.authenticatedContext('adm').firestore();
    await assertSucceeds(getDoc(doc(aliceDb, `titleRelatedSuggestions/${suggestionId}`)));
    await assertFails(getDoc(doc(bobDb, `titleRelatedSuggestions/${suggestionId}`)));
    await assertSucceeds(getDoc(doc(admDb, `titleRelatedSuggestions/${suggestionId}`)));
  });

  it('il proponente non puo\' aggiornare o cancellare il proprio suggerimento; admin si\'', async () => {
    await seedDocs([
      [`titleRelatedSuggestions/${suggestionId}`, { uid: 'alice', titleId: 'title_a', suggestedTitleId: 'title_b', status: 'pending' }],
      ['users/adm', { displayName: 'Adm', displayNameLower: 'adm', isAdmin: true }],
    ]);
    const aliceDb = testEnv.authenticatedContext('alice').firestore();
    const admDb = testEnv.authenticatedContext('adm').firestore();
    await assertFails(updateDoc(doc(aliceDb, `titleRelatedSuggestions/${suggestionId}`), { status: 'approved' }));
    await assertFails(deleteDoc(doc(aliceDb, `titleRelatedSuggestions/${suggestionId}`)));
    await assertSucceeds(updateDoc(doc(admDb, `titleRelatedSuggestions/${suggestionId}`), { status: 'approved', reviewedBy: 'adm' }));
  });
});
