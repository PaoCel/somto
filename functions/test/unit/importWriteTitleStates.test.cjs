...
// --- serie a UNA stagione senza marker Netflix ---
// Regressione: Netflix non scrive "Stagione 1" per le serie mono-stagione, e
// il badge del profilo restava senza stagione. 3.607 righe negli import in
// prod al 2026-09-08.

test("una serie con una sola stagione prende la stagione 1 anche senza marker", () => {
  const title = {
    id: "s1",
    type: "tv",
    meta: { durationEpisode: 50, seasonsCount: 1, episodesPerSeason: 8, seasons: [{ season: 1, episodes: 8 }] },
  };
  const rows = [
    { kind: "ambiguous", seasonNumber: null, episodeNumber: null, episodeNameGuess: "Primo", watchedDate: new Date("2026-01-01") },
    { kind: "ambiguous", seasonNumber: null, episodeNumber: null, episodeNameGuess: "Secondo", watchedDate: new Date("2026-01-02") },
  ];
  const next = buildSeriesImportState(null, title, rows, { source: "netflix_csv" });
  assert.equal(next.seriesProgress.episodesWatchedCount, 2);
  assert.equal(next.seriesProgress.lastWatchedSeasonNumber, 1);
});

test("una serie multi-stagione senza marker non si inventa la stagione", () => {
  const title = {
    id: "s2",
    type: "tv",
    meta: { durationEpisode: 50, seasonsCount: 5, episodesPerSeason: 10, seasons: [{ season: 1, episodes: 10 }, { season: 2, episodes: 10 }, { season: 3, episodes: 10 }, { season: 4, episodes: 10 }, { season: 5, episodes: 10 }] },
  };
  const rows = [
    { kind: "ambiguous", seasonNumber: null, episodeNumber: null, episodeNameGuess: "Uno", watchedDate: new Date("2026-01-01") },
  ];
  const next = buildSeriesImportState(null, title, rows, { source: "netflix_csv" });
  assert.equal(next.seriesProgress.episodesWatchedCount, 1);
  assert.equal(next.seriesProgress.lastWatchedSeasonNumber, null);
