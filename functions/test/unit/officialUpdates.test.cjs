...
test("draftToPublishInput conserva autore e pubblico per affinita'", () => {
  const input = draftToPublishInput("serie-2", {
    title: "T",
    text: "Body",
    updateType: "new_season",
    linkedTitleIds: ["a"],
    authorUid: "page_serie",
    tasteAudienceLimit: 25,
    status: "draft",
  });

  assert.equal(input.authorUid, "page_serie");
  assert.equal(input.tasteAudienceLimit, 25);
});
...
// Un post ufficiale puo' uscire da Somto o da una pagina tematica. La pagina
// deve esistere ed essere marcata come tale: senza il controllo basterebbe
// passare l'uid di un utente vero per pubblicare a suo nome.
function fakeUsersDb(users = {}, followers = {}) {
  return {
    collection: () => ({
      doc: (uid) => ({
        get: async () => ({ exists: Boolean(users[uid]), data: () => users[uid] }),
        set: async () => {},
        collection: () => ({
          limit: () => ({
            get: async () => ({ docs: (followers[uid] || []).map((id) => ({ id })) }),
          }),
        }),
      }),
    }),
...
test("l'autore di default resta Somto", async () => {
  const publisher = await resolvePublisher({ db: fakeUsersDb(), admin: fakeAdmin, authorUid: "" });
  assert.deepEqual(publisher, { uid: SOMTO_OFFICIAL_UID, name: "Somto" });
  assert.equal(normalizeOfficialUpdateInput({
    title: "T", text: "x", linkedTitleIds: ["a"],
  }).authorUid, SOMTO_OFFICIAL_UID);
});

test("una pagina registrata puo' firmare il post", async () => {
  const db = fakeUsersDb({ page_crime: { accountType: PAGE_ACCOUNT_TYPE, displayName: "Crime" } });
  const publisher = await resolvePublisher({ db, admin: fakeAdmin, authorUid: "page_crime" });
  assert.deepEqual(publisher, { uid: "page_crime", name: "Crime" });
});

test("un utente vero non puo' essere usato come autore", async () => {
  const db = fakeUsersDb({ paolo: { displayName: "Paolo" } });
  await assert.rejects(
    () => resolvePublisher({ db, admin: fakeAdmin, authorUid: "paolo" }),
    /non e' una pagina editoriale/
  );
});

test("un autore inesistente viene rifiutato", async () => {
  await assert.rejects(
    () => resolvePublisher({ db: fakeUsersDb(), admin: fakeAdmin, authorUid: "page_fantasma" }),
    /non e' una pagina editoriale/
  );
});

test("i follower della pagina entrano nel fan-out, quelli di Somto no", async () => {
  const db = fakeUsersDb({}, { page_crime: ["u1", "u2", "page_crime"] });
  assert.deepEqual(await collectPageFollowerUids(db, "page_crime"), ["u1", "u2"]);
  assert.deepEqual(await collectPageFollowerUids(db, SOMTO_OFFICIAL_UID), []);
});

test("una lettura fallita dei follower non blocca la pubblicazione", async () => {
  const db = {
    collection: () => ({
      doc: () => ({ collection: () => ({ limit: () => ({ get: async () => { throw new Error("giu'"); } }) }) }),
    }),
  };
  assert.deepEqual(await collectPageFollowerUids(db, "page_crime"), []);
});

// Anteprima (dryRun) con l'autore ufficiale: niente follower di pagina, niente
// pubblico per affinita' (tasteAudienceLimit non passato = spento), quindi
// l'unica audience e' chi ha il titolo in libreria.
function previewDb(interestedUids = []) {
  return {
    collectionGroup(name) {
      assert.equal(name, "titleStates");
      return {
        where: () => ({
          limit: () => ({
            async get() {
              return {
                docs: interestedUids.map((uid) => ({
                  data: () => ({ titleId: "tmdb_tv_1", state: "in_progress" }),
    ...
    collection: () => ({ doc: () => ({ async set() {} }) }),
  };
}

async function preview(interestedUids) {
  const { publishOfficialUpdate } = require("../../lib/officialUpdates");
  return publishOfficialUpdate({
    db: previewDb(interestedUids),
    admin: schedulerAdminMock,
    requestedByUid: "admin1",
    dryRun: true,
    input: {
      title: "Spin-off annunciato",
      text: "Testo",
      status: "published",
      updateType: "announcement",
      linkedTitleIds: ["tmdb_tv_1"],
    },
  });
}

test("l'anteprima avvisa quando il post non arriva a nessuno", async () => {
  const result = await preview([]);
  assert.equal(result.recipientCount, 0);
  const warning = result.warnings.find((w) => w.code === "audience_empty");
  assert.ok(warning, "avviso di audience vuota");
  assert.match(warning.message, /Nessuno riceve/);
});

test("l'anteprima avvisa quando i destinatari sono pochi, senza bloccare", async () => {
  const result = await preview(["alice", "bob"]);
  assert.equal(result.recipientCount, 2);
  const warning = result.warnings.find((w) => w.code === "audience_tiny");
  assert.ok(warning, "avviso di audience piccola");
  assert.equal(warning.notifyReach, 2);
  // Informa, non blocca: la pubblicazione resta possibile.
  assert.equal(result.status, "published");
});

test("con abbastanza destinatari non si avvisa niente", async () => {
  const result = await preview(["a", "b", "c", "d", "e", "f"]);
  assert.equal(result.recipientCount, 6);
