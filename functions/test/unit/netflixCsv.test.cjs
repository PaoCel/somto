...
// --- marker di stagione "brandizzato" ("<Nome serie> N") ---
// Regressione import 2026-09-08: 38 righe di Stranger Things su 156 finivano
// `ambiguous` con stagione null, lasciando seasonsCompletedCount a 0 e il
// badge del profilo senza stagione.

test("parseNetflixTitleCell riconosce '<Serie> N' come marker di stagione", () => {
  const parsed = parseNetflixTitleCell("Stranger Things: Stranger Things 4: Capitolo quattro: Caro Billy");
  assert.equal(parsed.kind, "tv_episode");
  assert.equal(parsed.seriesNameGuess, "Stranger Things");
  assert.equal(parsed.seasonNumber, 4);
  assert.equal(parsed.episodeNumber, null);
  assert.equal(parsed.episodeNameGuess, "Capitolo quattro: Caro Billy");
});

test("parseNetflixTitleCell legge il numero episodio sotto un marker brandizzato", () => {
  const parsed = parseNetflixTitleCell("Emily in Paris: Emily in Paris 3: Episodio 5");
  assert.equal(parsed.kind, "tv_episode");
  assert.equal(parsed.seasonNumber, 3);
  assert.equal(parsed.episodeNumber, 5);
  assert.equal(parsed.episodeNameGuess, null);
});

test("parseNetflixTitleCell non scambia per stagione un sottotitolo numerico a 2 segmenti", () => {
  // Senza un terzo segmento non c'e' episodio: resta ambiguo (puo' essere un film).
  const parsed = parseNetflixTitleCell("Ocean's: Ocean's 8");
  assert.equal(parsed.kind, "ambiguous");
  assert.equal(parsed.seasonNumber, null);
});

test("parseNetflixTitleCell lascia ambigua una riga senza marker di stagione", () => {
  const parsed = parseNetflixTitleCell("Stranger Things: Capitolo tre: Luci natalizie");
  assert.equal(parsed.kind, "ambiguous");
  assert.equal(parsed.seasonNumber, null);
  assert.equal(parsed.episodeNameGuess, "Capitolo tre: Luci natalizie");
});

test("parseNetflixTitleCell continua a preferire 'Stagione N' esplicito", () => {
  const parsed = parseNetflixTitleCell("Squid Game: Stagione 1: Un, due, tre, stella");
  assert.equal(parsed.kind, "tv_episode");
  assert.equal(parsed.seasonNumber, 1);
  assert.equal(parsed.episodeNameGuess, "Un, due, tre, stella");
});
