...
test("la premiere di stagione e' un'uscita, gli altri episodi no", () => {
  assert.equal(isSeasonPremiere(event({ eventType: "new_episode", season: 2, episode: 1 })), true);
  assert.equal(isSeasonPremiere(event({ eventType: "new_episode", season: 1, episode: 1 })), true);
  assert.equal(isSeasonPremiere(event({ eventType: "new_episode", season: 2, episode: 3 })), false);
  assert.equal(isSeasonPremiere(event({ eventType: "trailer", episode: 1 })), false);
});

test("a sweep per episodio spenta passano solo le premiere", async () => {
  const today = new Date(NOW_MS + (2 * 60 * 60 * 1000));
  const rows = [
    { id: "premiere", data: event({ eventType: "new_episode", season: 2, episode: 1, effectiveAt: today }) },
    { id: "puntata", data: event({ eventType: "new_episode", season: 2, episode: 4, effectiveAt: today }) },
  ];
  const db = makeEventsDb(rows);
  const seen = [];
  const report = await fanOutDueTitleUpdates({
    ...
    nowMs: NOW_MS,
    premieresOnly: true,
    fanOut: async ({ eventId }) => {
      seen.push(eventId);
      return { skipped: false, written: 1 };
    },
  });

  assert.deepEqual(seen, ["premiere"]);
  assert.equal(report.skipped, 1);
  assert.equal(report.premieresOnly, true);
  assert.deepEqual([...db.written.keys()], ["premiere"]);
});

test("la premiere si annuncia come inizio di stagione", () => {
  const day = 24 * 60 * 60 * 1000;
  const premiere = (offsetMs) => event({
    eventType: "new_episode",
    season: 2,
    episode: 1,
    effectiveAt: new Date(NOW_MS + offsetMs),
  });
  const today = buildTitleUpdateMessageByLocale(premiere(2 * 60 * 60 * 1000), "Ted Lasso", NOW_MS);
  assert.equal(today["it-IT"], "Oggi inizia la stagione 2 di Ted Lasso.");
  assert.equal(today["en-US"], "Season 2 of Ted Lasso starts today.");
  assert.equal(buildTitleUpdateMessageByLocale(premiere(-3 * day), "Ted Lasso", NOW_MS)["it-IT"], "È iniziata la stagione 2 di Ted Lasso.");
  assert.match(buildTitleUpdateMessageByLocale(premiere(5 * day), "Ted Lasso", NOW_MS)["it-IT"], /^La stagione 2 di Ted Lasso inizia il /);

  // Una serie nuova resta sul copy dell'episodio: "uscito/uscita" dipende dal titolo.
  const debut = event({ eventType: "new_episode", season: 1, episode: 1, effectiveAt: new Date(NOW_MS + (2 * 60 * 60 * 1000)) });
  assert.equal(buildTitleUpdateMessageByLocale(debut, "Ted Lasso", NOW_MS)["it-IT"], "Oggi nuovo episodio di Ted Lasso.");
});
...
// ---------------------------------------------------------------------------
// Fanout collegato (CONTRACT 3: `event.linkedTitleIds`, saga)
// ---------------------------------------------------------------------------
...
function linkedEvent(overrides = {}) {
  return event({
    ...
    mediaType: "movie",
    eventType: "trailer",
    linkedTitleIds: ["iron-man-2", "iron-man-1"],
    ...overrides,
  });
}

// A differenza di `makeDb` sopra (che assume un solo titleId, "ted-lasso"),
// questo fake tiene stati/preferenze indicizzati per titolo: il fanout
// collegato interroga collectionGroup su piu' titleId nella stessa run.
function makeLinkedDb({ statesByTitle = {}, prefsByTitle = {}, titles = {} } = {}) {
  const store = new Map(Object.entries(titles).map(([id, data]) => [`titles/${id}`, data]));
  return {
    store,
    collection(name) { return collectionFor(name, store); },
    collectionGroup(name) {
      ...
      return {
        where(field, op, value) {
          assert.deepEqual([field, op], ["titleId", "=="]);
          const docs = (name === "titleStates" ? statesByTitle[value] : prefsByTitle[value]) || [];
...
test("LINKED_FANOUT_EVENT_TYPES copre annuncio/trailer/uscita, non i nuovi episodi", () => {
  assert.equal(LINKED_FANOUT_EVENT_TYPES.has("trailer"), true);
  assert.equal(LINKED_FANOUT_EVENT_TYPES.has("teaser"), true);
  assert.equal(LINKED_FANOUT_EVENT_TYPES.has("release_date"), true);
  assert.equal(LINKED_FANOUT_EVENT_TYPES.has("new_episode"), false);
});

test("un destinatario collegato scende di un passo di rilevanza, un follow esplicito no", () => {
  assert.equal(
    titleUpdateRelevance({ state: "in_progress" }, event({ eventType: "new_episode" }), null, { linked: true }),
    "medium"
  );
  assert.equal(
    titleUpdateRelevance({ generalWatchlist: true }, event({ eventType: "release_date" }), null, { linked: true }),
    "low"
  );
  assert.equal(titleUpdateRelevance({}, event(), "follow", { linked: true }), "high");
  // Senza `linked`, il comportamento di sempre resta invariato.
  assert.equal(titleUpdateRelevance({ state: "in_progress" }, event({ eventType: "new_episode" })), "high");
});

test("il messaggio collegato aggiunge una frase sulla saga, senza cambiare quello normale", () => {
  const base = buildTitleUpdateMessageByLocale(event(), "Iron Man 2", NOW_MS);
  const linked = buildTitleUpdateMessageByLocale(event(), "Iron Man 2", NOW_MS, { titleName: "Iron Man" });
  assert.equal(linked["it-IT"].startsWith(base["it-IT"]), true);
  assert.match(linked["it-IT"], /Dalla stessa saga di Iron Man\.$/);
  assert.match(linked["en-US"], /From the same saga as Iron Man\.$/);
  // Senza contesto collegato, il messaggio e' quello di sempre.
  assert.deepEqual(buildTitleUpdateMessageByLocale(event(), "Iron Man 2", NOW_MS, null), base);
});

test("fanout collegato: raggiunge chi segue un altro titolo della saga", async () => {
  const db = makeLinkedDb({
    statesByTitle: { "iron-man-2": [] },
    prefsByTitle: { "iron-man-1": [preferenceDoc("erin", "follow")] },
    titles: { "iron-man-2": { name: "Iron Man 2" }, "iron-man-1": { name: "Iron Man" } },
  });
  const result = await fanOutTitleUpdate({ db, admin, eventId: "e1", before: null, after: linkedEvent(), nowMs: NOW_MS });

  assert.equal(result.written, 0); // nessuno segue il titolo primario in questo scenario
  assert.equal(result.linkedEligible, 1);
  assert.equal(result.linkedWritten, 1);
  const doc = db.store.get("users/erin/notifications/title_update_e1");
  assert.equal(doc.data.linkedFromTitleId, "iron-man-2");
  assert.equal(doc.data.titleId, "iron-man-2");
  assert.match(doc.data.messageByLocale["it-IT"], /Dalla stessa saga di Iron Man\.$/);
});

test("fanout collegato: chi e' gia' stato notificato sul titolo primario non viene riconteggiato", async () => {
  const db = makeLinkedDb({
    statesByTitle: { "iron-man-2": [stateDoc("carol", { state: "seen_unrated", generalWatchlist: true })] },
    prefsByTitle: { "iron-man-1": [preferenceDoc("carol", "follow")] },
    titles: { "iron-man-2": { name: "Iron Man 2" }, "iron-man-1": { name: "Iron Man" } },
  });
  const result = await fanOutTitleUpdate({
    db, admin, eventId: "e2", before: null, after: linkedEvent({ eventType: "release_date" }), nowMs: NOW_MS,
  });

  assert.equal(result.written, 1);
  assert.equal(result.linkedWritten, 0);
  assert.equal(db.store.has("users/carol/notifications/title_update_e2"), true);
});

test("fanout collegato: un eventType fuori dalla allowlist non tocca i titoli collegati", async () => {
  const db = makeLinkedDb({
    statesByTitle: { "iron-man-2": [] },
    prefsByTitle: { "iron-man-1": [preferenceDoc("erin", "follow")] },
    titles: { "iron-man-2": { name: "Iron Man 2" } },
  });
  const result = await fanOutTitleUpdate({
    db, admin, eventId: "e3", before: null,
    after: linkedEvent({ eventType: "new_episode", mediaType: "tv" }),
    nowMs: NOW_MS,
  });

  assert.equal(result.linkedScanned, 0);
  assert.equal(result.linkedWritten, 0);
});

test("fanout collegato: senza linkedTitleIds i nuovi campi restano a zero", async () => {
  const db = makeDb();
  const result = await fanOutTitleUpdate({ db, admin, eventId: "e4", before: null, after: event(), nowMs: NOW_MS });
  assert.equal(result.linkedEligible, 0);
  assert.equal(result.linkedWritten, 0);
  assert.equal(result.linkedScanned, 0);
});
...
// --- CTA verso la conversazione (2026-08-19) ---
// 42 post ufficiali in un mese avevano prodotto 2 commenti: la notifica
// portava alla scheda titolo, dove non si risponde a nessuno.
test("con un post di uscita la notifica porta alla conversazione", () => {
  const doc = buildTitleUpdateNotificationDocument({
    admin,
    uid: "u1",
    eventId: "tmdb_release_tv_97546_s4_e1",
    event: { titleId: "ted-lasso", eventType: "new_episode" },
    titleName: "Ted Lasso",
    nowMs: NOW_MS,
    conversationPostId: "official_uscita-tmdb-release-tv-97546-s4-e1",
  });
  assert.equal(doc.data.ctaUrl, "/community.html?post=official_uscita-tmdb-release-tv-97546-s4-e1");
  assert.equal(doc.data.postId, "official_uscita-tmdb-release-tv-97546-s4-e1");
});

test("senza post la notifica resta sulla scheda titolo", () => {
  const doc = buildTitleUpdateNotificationDocument({
    admin,
    uid: "u1",
    eventId: "evt-1",
  ...
  assert.equal(doc.data.ctaUrl, "/title.html?id=ted-lasso&focus=updates&event=evt-1");
  assert.equal("postId" in doc.data, false);
});

test("il post si risolve solo se esiste ancora", async () => {
  const existing = new Set(["official_uscita-evt-vivo"]);
  const db = {
    collection: () => ({
      doc: (id) => ({ get: async () => ({ exists: existing.has(id) }) }),
    }),
  };
  assert.equal(await resolveConversationPostId(db, "evt_vivo"), "official_uscita-evt-vivo");
  assert.equal(await resolveConversationPostId(db, "evt_ritirato"), "");
});

test("un errore di lettura non blocca la notifica", async () => {
  const db = { collection: () => ({ doc: () => ({ get: async () => { throw new Error("indisponibile"); } }) }) };
  assert.equal(await resolveConversationPostId(db, "evt_x"), "");
});

test("un episodio infrasettimanale ricade sul post della premiere", async () => {
  const existing = new Set(["official_uscita-tmdb-release-tv-97546-s4-e1"]);
  const db = { collection: () => ({ doc: (id) => ({ get: async () => ({ exists: existing.has(id) }) }) }) };
  const event = { mediaType: "tv", tmdbId: 97546, season: 4, episode: 3 };
  assert.equal(
    await resolveConversationPostId(db, "tmdb_release_tv_97546_s4_e3", event),
    "official_uscita-tmdb-release-tv-97546-s4-e1"
  );
});

test("il primo episodio non cerca un ripiego", () => {
  assert.equal(seasonPremierePostId({ mediaType: "tv", tmdbId: 97546, season: 4, episode: 1 }), "");
  assert.equal(seasonPremierePostId({ mediaType: "movie", tmdbId: 123 }), "");
  assert.equal(seasonPremierePostId({ mediaType: "tv", tmdbId: 97546, season: 0, episode: 5 }), "");
});

test("senza nemmeno il post della premiere si resta sulla scheda", async () => {
  const db = { collection: () => ({ doc: () => ({ get: async () => ({ exists: false }) }) }) };
  const event = { mediaType: "tv", tmdbId: 97546, season: 4, episode: 3 };
  assert.equal(await resolveConversationPostId(db, "tmdb_release_tv_97546_s4_e3", event), "");
});
