...
// --- Coordinate S·E su stagioni di lunghezza diversa -------------------------
// Regressione 2026-08-19 ("Muertos S.r.l."): il tasto "+1 episodio" ricavava
// stagione ed episodio dividendo per `episodesPerSeason`, valorizzato solo
// quando TUTTE le stagioni hanno la stessa lunghezza. Sulle altre serie
// `lastWatchedSeasonNumber` restava null: niente "Sei a S1 · E3" sulla card,
// niente foglio post-episodio su iOS (pretende season > 0 && episode > 0).

const unevenSeries = {
  id: "tv-uneven",
  type: "tv",
  name: "Serie a stagioni diverse",
  meta: {
    durationEpisode: 45,
    seasonsCount: 3,
    seasons: [
...
test("mark_series_episode ricava S·E camminando le stagioni reali", () => {
  const now = new Date("2026-08-19T10:00:00Z");
  const next = buildNextTitleState({
    titleId: "tv-uneven",
    mediaType: "tv",
    state: "in_progress",
    seriesProgress: { episodesWatchedCount: 7, seasonsCompletedCount: 1 },
  }, { type: "mark_series_episode", source: "unit_test" }, unevenSeries, { now });

  assert.equal(next.seriesProgress.episodesWatchedCount, 8);
  assert.equal(next.seriesProgress.lastWatchedSeasonNumber, 2);
  assert.equal(next.seriesProgress.lastWatchedEpisodeNumber, 1);
  assert.equal(next.seriesProgress.lastWatchedEpisodeId, "s2e1");
});

test("l'ultimo episodio di una stagione chiude la stagione, non la successiva", () => {
  const now = new Date("2026-08-19T10:00:00Z");
  const next = buildNextTitleState({
    titleId: "tv-uneven",
    mediaType: "tv",
    state: "in_progress",
    seriesProgress: { episodesWatchedCount: 6, seasonsCompletedCount: 0 },
  }, { type: "mark_series_episode", source: "unit_test" }, unevenSeries, { now });

  assert.equal(next.seriesProgress.lastWatchedSeasonNumber, 1);
  assert.equal(next.seriesProgress.lastWatchedEpisodeNumber, 7);
  assert.equal(next.seriesProgress.seasonsCompletedCount, 1);
});

test("mark_series_season conta gli episodi veri della stagione", () => {
  const now = new Date("2026-08-19T10:00:00Z");
  const next = buildNextTitleState({
    titleId: "tv-uneven",
    mediaType: "tv",
    state: "in_progress",
    seriesProgress: { episodesWatchedCount: 7, seasonsCompletedCount: 1 },
  }, { type: "mark_series_season", source: "unit_test" }, unevenSeries, { now });

  assert.equal(next.seriesProgress.seasonsCompletedCount, 2);
  assert.equal(next.seriesProgress.episodesWatchedCount, 20);
  assert.equal(next.seriesProgress.lastWatchedSeasonNumber, 2);
  assert.equal(next.seriesProgress.lastWatchedEpisodeNumber, 13);
});

test("completando la serie la coordinata è l'ultimo episodio dell'ultima stagione", () => {
  const now = new Date("2026-08-19T10:00:00Z");
  const next = buildNextTitleState({
    titleId: "tv-uneven",
    mediaType: "tv",
    state: "in_progress",
    seriesProgress: { episodesWatchedCount: 24, seasonsCompletedCount: 2 },
  }, { type: "mark_series_episode", source: "unit_test" }, unevenSeries, { now });

  assert.equal(next.state, "completed_unrated");
  assert.equal(next.seriesProgress.episodesWatchedCount, 25);
  assert.equal(next.seriesProgress.lastWatchedSeasonNumber, 3);
  assert.equal(next.seriesProgress.lastWatchedEpisodeNumber, 5);
  assert.equal(next.seriesProgress.percentComplete, 1);
});

test("senza righe stagione si tiene il contatore, senza inventare la stagione", () => {
  const now = new Date("2026-08-19T10:00:00Z");
  const bare = {
    id: "tv-bare",
    type: "tv",
    name: "Serie senza stagioni note",
    meta: { durationEpisode: 30, seasonsCount: 4 },
  };
  const next = buildNextTitleState(null, {
    type: "mark_series_episode",
    source: "unit_test",
