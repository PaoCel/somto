...
test("un post utente non editoriale non e' servibile (404)", () => {
  ...
  // Anche marcandolo a mano come editoriale: senza slug editoriale non passa.
...
// --- chi puo' firmare: redazione o pagina editoriale, niente altro ---------
//
// Il gate e' in due pezzi: `isServableOfficialPost` guarda la forma del post,
// `resolveServableAuthor` guarda chi lo firma con una lettura vera su `users`.
// Il secondo esiste perche' dal 2026-08 gli aggiornamenti possono uscire anche
// dalle pagine editoriali (page_uscite, page_serie...), e un prefisso nell'uid
// non e' una prova.
...
test("l'autore: somto_official e le pagine passano, il resto e' 404", async () => {
  const { _resolveServableAuthor: resolveServableAuthor } = require("../../modules/officialUpdatePage");
  const db = fakeUsersDb({
    page_uscite: { accountType: "page", displayName: "Uscite al cinema" },
    "uid-utente-vero": { accountType: "user", displayName: "Paolo" },
  });

  // La redazione non ha bisogno di lettura: e' l'autore di default.
  assert.deepEqual(
    await resolveServableAuthor(db, "somto_official"),
    { uid: "somto_official", name: "Somto", isPage: false }
  );
  assert.equal((await resolveServableAuthor(db, "")).uid, "somto_official");

  // Una pagina editoriale registrata firma con il suo nome.
  const page = await resolveServableAuthor(db, "page_uscite");
  assert.equal(page.uid, "page_uscite");
  assert.equal(page.name, "Uscite al cinema");
  assert.equal(page.isPage, true);

  // Un utente vero, anche se il post si dichiarasse editoriale: 404.
  assert.equal(await resolveServableAuthor(db, "uid-utente-vero"), null);

  // Un uid che *sembra* una pagina ma non esiste su `users`: 404.
  assert.equal(await resolveServableAuthor(db, "page_inventata"), null);
});
