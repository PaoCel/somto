...
async function main() {
  ...
  const result = await publishOfficialUpdate({
    ...
    input: {
      ...
      // Gia' supportati dalla console admin, mancavano solo qui.
      // --authorUid: pagina editoriale che firma il post (default Somto).
      // --tasteAudienceLimit: estende il pubblico a chi ha affinita' col titolo
      //   (0 = spento). Serve sulle uscite, dove il pubblico per libreria e'
      //   vuoto per costruzione.
      authorUid: args.authorUid || args["author-uid"] || undefined,
      tasteAudienceLimit: (args.tasteAudienceLimit || args["taste-audience-limit"]) !== undefined
        ? Number(args.tasteAudienceLimit || args["taste-audience-limit"])
        : undefined,
    },
  });
