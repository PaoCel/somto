...
const { registerRelatedSuggestions } = require("./modules/relatedSuggestions");
const { registerTitleUpdates } = require("./modules/titleUpdates");
const { registerYouTubeTrailers } = require("./modules/youtubeTrailers");
const { buildEditorialPatch } = require("./lib/titleUpdateEditorial");
const {
  MAX_SEED_TITLES: PEOPLE_MAX_SEED_TITLES,
  MAX_RATERS_PER_TITLE: PEOPLE_MAX_RATERS_PER_TITLE,
  MAX_SUGGESTIONS: PEOPLE_MAX_SUGGESTIONS,
  pickSeedTitles,
  MAX_REASON_TITLES: PEOPLE_MAX_REASON_TITLES,
  scoreCandidates: scorePeopleCandidates,
} = require("./lib/peopleSuggestions");
const { registerPushCoverage } = require("./modules/pushCoverage");
const { registerNotificationHealth } = require("./modules/notificationHealth");
...
function fetchTmdbUpcomingCandidatesForType(...args) {
  return getTmdbModule().fetchTmdbUpcomingCandidatesForType(...args);
}
...
const SIGNUP_VERIFICATION_ENFORCEMENT_PARAM = defineString("SIGNUP_VERIFICATION_ENFORCEMENT", { default: "false" });
// Data ISO entro cui gli account password gia' esistenti devono verificare
// l'email. Vuota = nessuna scadenza, comportamento morbido di oggi.
const EMAIL_VERIFICATION_GRACE_UNTIL_PARAM = defineString("EMAIL_VERIFICATION_GRACE_UNTIL", { default: "" });
const PENDING_SIGNUP_CLEANUP_ENABLED_PARAM = defineString("PENDING_SIGNUP_CLEANUP_ENABLED", { default: "false" });
const PENDING_SIGNUP_CLEANUP_DRY_RUN_PARAM = defineString("PENDING_SIGNUP_CLEANUP_DRY_RUN", { default: "true" });
const PENDING_SIGNUP_CLEANUP_MAX_ABSOLUTE_PARAM = defineString("PENDING_SIGNUP_CLEANUP_MAX_ABSOLUTE", { default: "25" });
const PENDING_SIGNUP_CLEANUP_MAX_RATIO_PARAM = defineString("PENDING_SIGNUP_CLEANUP_MAX_RATIO", { default: "0.02" });
const PENDING_SIGNUP_CLEANUP_EXCLUDED_UIDS_PARAM = defineString("PENDING_SIGNUP_CLEANUP_EXCLUDED_UIDS", { default: "" });
...
function verificationGraceUntilMs() {
  try {
    return parseVerificationGraceUntil(EMAIL_VERIFICATION_GRACE_UNTIL_PARAM.value());
  } catch (_) {
    return parseVerificationGraceUntil(process.env.EMAIL_VERIFICATION_GRACE_UNTIL);
  }
}

function isSignupVerificationEnforced() {
  try {
    return String(SIGNUP_VERIFICATION_ENFORCEMENT_PARAM.value() || "false").toLowerCase() === "true";
  } catch (_) {
    return String(process.env.SIGNUP_VERIFICATION_ENFORCEMENT || "false").toLowerCase() === "true";
  }
}

function stringParamValue(param, envName, fallback) {
  try {
    return String(param.value() ?? fallback);
  } catch (_) {
    return String(process.env[envName] ?? fallback);
  }
}

function boolParamValue(param, envName, fallback) {
  return stringParamValue(param, envName, fallback ? "true" : "false").toLowerCase() === "true";
}

const ADMIN_UIDS_BOOT = readAdminUids({ allowParamValue: false });

// Campanella admin sull'attivita' reale degli utenti (post, commenti, voti,
// follow, libreria, liste, account cancellati). Non e' un export di Cloud
// Functions: e' un helper chiamato dai trigger che gia' girano.
const { recordAdminActivity } = registerAdminActivity({
  admin,
  logger,
  resolveAdminUids: getAdminUids,
});
...
Object.assign(exports, registerYouTubeTrailers({ admin, logger }));
Object.assign(exports, registerPushCoverage({ admin, logger, getAdminUids }));
Object.assign(exports, registerNotificationHealth({ admin, logger, getAdminUids }));
...
// Suggerimenti community di titoli collegati: trigger di auto-approvazione
// (soglia proponenti distinti) + callable di revisione admin.
Object.assign(exports, registerRelatedSuggestions({
  functions,
  admin,
  logger,
  isAdminCaller,
...
function buildAuthPublicUserDoc(userRecord = {}) {
  ...
  return {
    ...
    // Niente lastActiveAt al provisioning: scriverlo qui faceva risultare
    // "attivo negli ultimi 7 giorni" un account creato via API e mai aperto —
    // esattamente il caso gcpmap_*. Lo scrivono i client alla prima apertura
    // vera (users.api.js, UserRepository.swift), quindi il campo esiste solo
    // se qualcuno ha davvero usato l'app.
...
// App Check e' predisposto sui client (recaptchaV3 sul web, App Attest su iOS)
// ma non e' configurato sul progetto: `public/firebaseConfig.js` ha
// `appCheck.enabled=false` e siteKey vuota. Con `enforceAppCheck: true` le tre
// callable del signup rispondono `unauthenticated` a un web che non puo'
// produrre il token, e la registrazione password resta rotta come lo e' oggi
// col 404. L'enforcement torna attivo mettendo SIGNUP_APP_CHECK=on nelle env
// delle Functions, dopo aver registrato reCAPTCHA v3 e App Attest: nessun
// altro cambio di codice. Restano attivi Auth obbligatoria e rate limit.
// Vedi docs/SIGNUP_SECURITY_AND_OBSERVABILITY_PLAN.md.
const SIGNUP_APP_CHECK_REQUIRED = process.env.SIGNUP_APP_CHECK === "on";
const SIGNUP_CALLABLE_RUNTIME = { enforceAppCheck: SIGNUP_APP_CHECK_REQUIRED };

function assertSignupCallableContext(context) {
  const uid = String(context.auth?.uid || "").trim();
  if (!uid) throw new functions.https.HttpsError("unauthenticated", "Autenticazione richiesta.");
  // Stesso interruttore del runWith: senza App Check configurato sul progetto
  // il web non ha token da mandare e il gate bloccherebbe ogni registrazione.
  if (SIGNUP_APP_CHECK_REQUIRED && !context.app) {
    throw new functions.https.HttpsError("failed-precondition", "App Check richiesto.");
  }
  return uid;
}

function signupProviderFromAuthUser(authUser) {
  const providers = providerIdsForAuthUser(authUser);
...
function pendingSignupExpiryTimestamp() {
  const expiresMs = Date.now() + PENDING_SIGNUP_RETENTION_HOURS * 60 * 60 * 1000;
  return admin.firestore.Timestamp.fromMillis(expiresMs);
}

exports.registerPendingSignup = functions
  .runWith(SIGNUP_CALLABLE_RUNTIME)
  .region("europe-west1")
  .https.onCall(async (data, context) => {
    const uid = assertSignupCallableContext(context);
    const db = admin.firestore();
    await enforceCallableRateLimit(db, uid, "registerPendingSignup", {
      windowSeconds: 60,
      maxInWindow: 4,
      dailyMax: 20,
    });
    await enforceGuestCallableRateLimit(
      db,
      context?.rawRequest?.ip,
      "registerPendingSignup",
      { windowSeconds: 60, maxInWindow: 20, dailyMax: 300 }
    );

    const authUser = await admin.auth().getUser(uid);
    const providerIds = providerIdsForAuthUser(authUser);
    if (!providerIds.includes("password") || authUser.emailVerified === true) {
      throw new functions.https.HttpsError("failed-precondition", "Account non idoneo alla verifica email.");
    }
    const requestedName = sanitizeDisplayName(data?.displayName || authUser.displayName || "");
    if (!requestedName) throw new functions.https.HttpsError("invalid-argument", "Nome pubblico non valido.");
    if (data?.ageConfirmed !== true || data?.communitySafetyAccepted !== true) {
      throw new functions.https.HttpsError("failed-precondition", "Consensi di registrazione mancanti.");
    }

    const attribution = sanitizeSignupAttribution(data?.attribution);
    const pendingRef = db.collection("pendingSignups").doc(uid);
    const now = admin.firestore.FieldValue.serverTimestamp();
    ...
    await db.runTransaction(async (tx) => {
      ...
      pendingCreated = !snap.exists || current.status !== "pending";
      tx.set(pendingRef, {
        schemaVersion: 1,
        uid,
        provider: "password",
        status: "pending",
        requestedDisplayName: requestedName.displayName,
        ageConfirmed: true,
        communitySafetyAcceptedAt: current.communitySafetyAcceptedAt || now,
        communitySafetyAcceptedSource: ["web_signup", "ios_signup"].includes(data?.communitySafetyAcceptedSource)
          ? data.communitySafetyAcceptedSource
          : "web_signup",
        createdAt: current.createdAt || now,
        updatedAt: now,
        expiresAt: pendingSignupExpiryTimestamp(),
    ...
    if (pendingCreated) {
      await bumpDailyMetric("signup_pending_registered", 1).catch((error) => {
        logger.warn("[signup] pending metric unavailable", { errorCode: String(error?.code || "unknown") });
      });
    }

    logger.info("[signup] pending registered", {
      uid,
      provider: "password",
      surface: attribution.surface,
      reservedEmailDomain: isReservedEmailDomain(authUser.email || ""),
    });
    return { ok: true, status: "pending", expiresInHours: PENDING_SIGNUP_RETENTION_HOURS };
  });

async function reserveSignupDisplayName(tx, db, uid, desiredName) {
  const checked = sanitizeDisplayName(desiredName);
  if (!checked) throw new functions.https.HttpsError("invalid-argument", "Nome pubblico non valido.");

  const candidates = [];
  for (let index = 0; index < 14; index += 1) {
    const suffix = index ? String(index + 1) : "";
    const stem = checked.displayName.slice(0, 24 - suffix.length).replace(/[._-]+$/g, "");
    const displayName = `${stem}${suffix}`;
    candidates.push({ displayName, displayNameLower: displayName.toLowerCase() });
  }
  const refs = candidates.map((candidate) => db.collection("usernames").doc(candidate.displayNameLower));
  const snaps = [];
  for (const ref of refs) {
    // Tutte le letture avvengono prima delle scritture della transazione.
    // eslint-disable-next-line no-await-in-loop
    snaps.push(await tx.get(ref));
  }
  const index = snaps.findIndex((snap) => !snap.exists || snap.data()?.uid === uid);
  if (index < 0) throw new functions.https.HttpsError("already-exists", "Nome pubblico non disponibile.");
  return { candidate: candidates[index], ref: refs[index], snap: snaps[index] };
}

async function removeLegacyUnverifiedClaim(authUser) {
  if (authUser?.customClaims?.legacyEmailUnverified !== true) return false;
  const claims = claimsWithoutLegacyBypass(authUser.customClaims);
  await admin.auth().setCustomUserClaims(authUser.uid, claims);
  return true;
}

exports.completeVerifiedSignup = functions
  .runWith(SIGNUP_CALLABLE_RUNTIME)
  .region("europe-west1")
  .https.onCall(async (data, context) => {
    const uid = assertSignupCallableContext(context);
    if (context.auth?.token?.email_verified !== true) {
      throw new functions.https.HttpsError("permission-denied", "Aggiorna la sessione dopo la verifica email.");
    }
    const db = admin.firestore();
    await enforceCallableRateLimit(db, uid, "completeVerifiedSignup", {
      windowSeconds: 30,
      maxInWindow: 5,
      dailyMax: 30,
    });
    ...
    const authUser = await admin.auth().getUser(uid);
    if (!isApprovedVerifiedProvider(authUser)) {
      logger.warn("[signup] verified provider rejected", {
        uid,
        providerIds: providerIdsForAuthUser(authUser),
      });
      throw new functions.https.HttpsError("permission-denied", "Account non verificato.");
    }
    const provider = signupProviderFromAuthUser(authUser);
    const userRef = db.collection("users").doc(uid);
    const privateRef = db.collection("usersPrivate").doc(uid);
    const pendingRef = db.collection("pendingSignups").doc(uid);
    const attributionRef = userRef.collection("_system").doc("signupAttribution");
    const now = admin.firestore.FieldValue.serverTimestamp();
    let created = false;
    ...
    await db.runTransaction(async (tx) => {
      const [userSnap, pendingSnap] = await Promise.all([tx.get(userRef), tx.get(pendingRef)]);
      const pending = pendingSnap.data() || {};
      pendingCompleted = pending.status === "pending";
      if (provider === "password" && !userSnap.exists && pending.status !== "pending") {
        throw new functions.https.HttpsError("failed-precondition", "Registrazione in attesa non trovata.");
      }

      if (userSnap.exists) {
        selectedDisplayName = String(userSnap.data()?.displayName || authUser.displayName || "User");
        if (pendingSnap.exists) {
          tx.set(pendingRef, { status: "completed", completedAt: now, updatedAt: now }, { merge: true });
        }
        const storedAttribution = sanitizeSignupAttribution(pending.attribution || data?.attribution);
      ...
      const desiredName = pending.requestedDisplayName || data?.displayName || authUser.displayName || displayNameForAuthRecord(authUser);
      const reserved = await reserveSignupDisplayName(tx, db, uid, desiredName);
      selectedDisplayName = reserved.candidate.displayName;
      const publicDoc = buildAuthPublicUserDoc(authUser);
      const privateDoc = buildAuthPrivateUserDoc(authUser);
      tx.set(userRef, {
        ...publicDoc,
        displayName: reserved.candidate.displayName,
        displayNameLower: reserved.candidate.displayNameLower,
        communitySafetyAcceptedAt: pending.communitySafetyAcceptedAt || now,
        communitySafetyVersion: 1,
        communitySafetyAcceptedSource: pending.communitySafetyAcceptedSource || data?.communitySafetyAcceptedSource || "web_signup",
      });
      const ageConfirmed = pending.ageConfirmed === true || data?.ageConfirmed === true;
      tx.set(privateRef, {
        ...privateDoc,
        ageConfirmed,
        ...(ageConfirmed ? { ageConfirmedAt: now } : {}),
      }, { merge: true });
      tx.set(reserved.ref, {
        uid,
        displayName: reserved.candidate.displayName,
        displayNameLower: reserved.candidate.displayNameLower,
        createdAt: reserved.snap.exists ? (reserved.snap.data()?.createdAt || now) : now,
        updatedAt: now,
      }, { merge: true });
      const storedAttribution = sanitizeSignupAttribution(pending.attribution || data?.attribution);
      tx.set(attributionRef, {
    ...
    await removeLegacyUnverifiedClaim(authUser);
    if (pendingCompleted) {
      await bumpDailyMetric("signup_verified_completed", 1).catch((error) => {
        logger.warn("[signup] completion metric unavailable", { errorCode: String(error?.code || "unknown") });
      });
    }
    const supportResult = await ensureSupportThreadForUser(db, uid);
    logger.info("[signup] verified signup completed", {
      uid,
      provider,
      created,
      supportThreadCreated: supportResult?.created === true,
    });
    return {
      ok: true,
      status: "active",
...
exports.getSignupState = functions
  .runWith(SIGNUP_CALLABLE_RUNTIME)
  .region("europe-west1")
  .https.onCall(async (_data, context) => {
    const uid = assertSignupCallableContext(context);
    const db = admin.firestore();
    const [authUser, userSnap, pendingSnap, deletionSnap] = await Promise.all([
    ...
    const providerIds = providerIdsForAuthUser(authUser);
    const baseStatus = classifySignupLifecycle({
      authPresent: true,
      profilePresent: userSnap.exists,
      providerIds,
      emailVerified: authUser.emailVerified === true,
      pendingStatus: pendingSnap.data()?.status || null,
      deletionStatus: deletionSnap.data()?.status || null,
    });
    // Scaduta la finestra morbida, lo stesso account viene raccontato come
    // "da verificare": i client gia' distribuiti lo capiscono da soli e
    // mostrano la schermata di verifica al prossimo ingresso.
    const graceExpired = isVerificationGraceExpired({
      lifecycleStatus: baseStatus,
      providerIds,
      emailVerified: authUser.emailVerified === true,
      graceUntilMs: verificationGraceUntilMs(),
    });
    const lifecycleStatus = graceExpired ? "pending_verification" : baseStatus;
    return {
      ok: true,
      lifecycleStatus,
      providerIds,
      emailVerified: authUser.emailVerified === true,
      // Dopo la scadenza il claim legacy non vale piu' come lasciapassare:
      // si risponde con la policy effettiva, non con il claim grezzo.
      legacyEmailUnverified: graceExpired
        ? false
        : authUser.customClaims?.legacyEmailUnverified === true,
      profilePresent: userSnap.exists,
      pendingStatus: pendingSnap.data()?.status || null,
      verificationGraceUntil: verificationGraceUntilMs() || null,
    };
  });

async function listAllAuthUsersForPendingCleanup() {
  const users = [];
  let pageToken;
  do {
    // listUsers e' paginata e read-only. Non usare deleteUsers in seguito:
    // il bulk delete non emette i trigger Auth onDelete necessari al cleanup.
    // eslint-disable-next-line no-await-in-loop
...
exports.cleanupExpiredPendingSignups = functions
  .runWith({ timeoutSeconds: 540, memory: "512MB" })
  .region("europe-west1")
  .pubsub.schedule("17 3 * * *")
  .timeZone("Europe/Rome")
  .onRun(async () => {
    const enabled = boolParamValue(
      PENDING_SIGNUP_CLEANUP_ENABLED_PARAM,
      "PENDING_SIGNUP_CLEANUP_ENABLED",
      false
    );
    const dryRun = boolParamValue(
      PENDING_SIGNUP_CLEANUP_DRY_RUN_PARAM,
      "PENDING_SIGNUP_CLEANUP_DRY_RUN",
      true
    );
    if (!enabled) {
      logger.info("[signup-cleanup] disabled", { enabled: false, dryRun });
      return { enabled: false, dryRun, candidateCount: 0, deletedCount: 0 };
    }

    const db = admin.firestore();
    const [accounts, adminProfiles] = await Promise.all([
      listAllAuthUsersForPendingCleanup(),
      db.collection("users").where("isAdmin", "==", true).get(),
    ]);
    const adminUids = new Set([
      ...getAdminUids(),
      ...adminProfiles.docs.map((docSnap) => docSnap.id),
    ]);
    const excludedUids = stringParamValue(
      PENDING_SIGNUP_CLEANUP_EXCLUDED_UIDS_PARAM,
      "PENDING_SIGNUP_CLEANUP_EXCLUDED_UIDS",
      ""
    ).split(",").map((value) => value.trim()).filter(Boolean);
    const plan = pendingSignupCleanupPlan(accounts, {
      adminUids,
      excludedUids,
      maxAbsolute: Number(stringParamValue(
        PENDING_SIGNUP_CLEANUP_MAX_ABSOLUTE_PARAM,
        "PENDING_SIGNUP_CLEANUP_MAX_ABSOLUTE",
        "25"
      )),
      maxRatio: Number(stringParamValue(
        PENDING_SIGNUP_CLEANUP_MAX_RATIO_PARAM,
        "PENDING_SIGNUP_CLEANUP_MAX_RATIO",
        "0.02"
      )),
    });
    const summary = {
      ...
      candidateRatio: Number(plan.candidateRatio.toFixed(6)),
      skipped: plan.skipped,
      circuitBreakerOpen: plan.circuitBreakerOpen,
      circuitBreakerReason: plan.circuitBreakerReason,
    };
    if (plan.circuitBreakerOpen) {
      logger.error("[signup-cleanup] circuit breaker open", summary);
      throw new Error(`pending signup cleanup blocked: ${plan.circuitBreakerReason}`);
    }
    if (dryRun) {
      logger.info("[signup-cleanup] dry run", summary);
      return { ...summary, deletedCount: 0 };
    }

    let deletedCount = 0;
    let failedCount = 0;
    for (const candidate of plan.candidates) {
      const deletionRef = db.collection("accountDeletionRequests").doc(candidate.uid);
      try {
        // Il marker precede Auth.deleteUser: il trigger onDelete può così
        // distinguere una scadenza automatica da una cancellazione Auth diretta.
        // Non contiene email o altri identificatori oltre all'UID tecnico.
        // eslint-disable-next-line no-await-in-loop
        await deletionRef.set({
          uid: candidate.uid,
          source: "pending-expiry",
          status: "awaiting-auth-delete",
          requestedAt: admin.firestore.FieldValue.serverTimestamp(),
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        }, { merge: true });
        // Individuale di proposito: mantiene il trigger Auth onDelete.
        // eslint-disable-next-line no-await-in-loop
    ...
    const result = { ...summary, deletedCount, failedCount };
    if (failedCount) {
      logger.error("[signup-cleanup] partial failure", result);
      throw new Error(`pending signup cleanup partial failure: ${failedCount}`);
    }
    logger.info("[signup-cleanup] completed", result);
    return result;
  });
...
exports.ensureUserDocsOnAuthCreate = functions
  ...
  .onCreate(async (userRecord) => {
    ...
    const providerIds = providerIdsForAuthUser(userRecord);
    if (isSignupVerificationEnforced()
      && providerIds.includes("password")
      && userRecord.emailVerified !== true) {
      logger.info("[ensureUserDocsOnAuthCreate] unverified password skipped", { uid });
      return null;
    }
...
exports.sendInactivityNudges = functions
  ...
  .onRun(async () => {
    ...
    for (const docSnap of usersSnap.docs) {
      ...
      const lastNudgeAtMs = engagementMillis(u, "lastNudgeAt");
      const lastActiveAtMs = toMillis(u.lastActiveAt);
      // Rientrata dopo l'ultimo richiamo? La scala riparte da zero.
      const returnedSinceLastNudge = !lastNudgeAtMs
        || (!!lastActiveAtMs && lastActiveAtMs > lastNudgeAtMs);
      const unanswered = returnedSinceLastNudge ? 0 : engagementNumber(u, "nudgeCount");

      if (unanswered >= INACTIVITY_NUDGE_MAX_UNANSWERED) {
        skippedExhausted++;
        continue;
      }
      ...
      batch.set(
        docSnap.ref,
        {
          engagement: {
            ...
            // Valore esplicito, non increment(): il conteggio vecchio sta nel
            // campo letterale e va ereditato, non ricominciato da capo.
            nudgeCount: returnedSinceLastNudge ? 1 : unanswered + 1,
          },
          "engagement.lastNudgeAt": admin.firestore.FieldValue.delete(),
          "engagement.nudgeCount": admin.firestore.FieldValue.delete(),
        },
        { merge: true }
      );
      writeOps++;
    ...
    logger.info(
      `[nudge] sent=${sentCount} skippedAdmins=${skippedAdmins} skippedCooldown=${skippedCooldown} skippedExhausted=${skippedExhausted} scanned=${usersSnap.size}`
    );
    return null;
  });
...
// ============================================
// Digest settimanale: una notifica, le novità sui tuoi titoli
// ============================================
// Sostituisce la sweep giornaliera dei nuovi episodi (spenta il 2026-08-25,
// vedi `titleUpdateDueSweepEnabled` in modules/titleUpdates.js): quelle erano
// 553 notifiche lette al 2%, queste sono una a settimana. Dry-run del
// 2026-08-25: 108 persone su 350, 237 novità, mediana 3 righe.
//
// Anteprima senza scrivere niente: `functions/scripts/preview-weekly-digest.js`.
const DIGEST_WINDOW_BACK_MS = 7 * 24 * 60 * 60 * 1000;
const DIGEST_WINDOW_FORWARD_MS = 14 * 24 * 60 * 60 * 1000;

exports.sendWeeklyDigest = functions
  .region("europe-west1")
  .runWith({ timeoutSeconds: 540, memory: "512MB" })
  .pubsub.schedule("0 18 * * 4")
  .timeZone("Europe/Rome")
  .onRun(async () => {
    const {
    ...
    const { hasGlobalTitleUpdateOptOut } = require("./lib/titleUpdateNotifications");

    const db = admin.firestore();
    const nowMs = Date.now();
    const weekKey = digestWeekKey(nowMs);
    const notificationId = digestNotificationId(weekKey);

    const eventsSnap = await db.collection("titleUpdateEvents")
      .where("effectiveAt", ">=", admin.firestore.Timestamp.fromMillis(nowMs - DIGEST_WINDOW_BACK_MS))
      .where("effectiveAt", "<=", admin.firestore.Timestamp.fromMillis(nowMs + DIGEST_WINDOW_FORWARD_MS))
    ...
    if (!events.length) {
      logger.info("[digest] nessuna novità nella finestra, niente da mandare", { weekKey });
      return null;
    }

    // I nomi dei titoli in blocco: un digest che dice "questo titolo" non lo
    // legge nessuno, e una lettura per riga sarebbe una lettura per persona.
    const titleNames = new Map();
    const titleIdList = [...titleIds];
    for (let i = 0; i < titleIdList.length; i += 250) {
      const refs = titleIdList.slice(i, i + 250).map((id) => db.collection("titles").doc(id));
      // eslint-disable-next-line no-await-in-loop
      const docs = await db.getAll(...refs);
      docs.forEach((docSnap) => {
        if (docSnap.exists) titleNames.set(docSnap.id, String(docSnap.data()?.name || ""));
      });
    }
    events.forEach((row) => { row.titleName = titleNames.get(row.data.titleId) || ""; });
    const eventIds = events.map((row) => row.id);

    const report = { weekKey, users: 0, written: 0, skippedInactive: 0, skippedOptOut: 0, skippedEmpty: 0, duplicates: 0, errors: 0 };
    let lastUser = null;
    ...
    for (;;) {
      ...
      for (const userDoc of usersSnap.docs) {
        ...
        if (u.isSynthetic === true || u.isDeleted === true) continue;
        report.users += 1;

        const lastActive = toMillis(u.lastActiveAt);
        // Chi manca da mesi non è pubblico da digest ricorrente: è pubblico da
        // un risveglio, che è un'altra cosa e si decide a mano.
        if (!lastActive || nowMs - lastActive > DEFAULT_MAX_INACTIVITY_MS) {
        ...
        try {
          // eslint-disable-next-line no-await-in-loop
          if (await hasGlobalTitleUpdateOptOut(db, userDoc.id)) {
          ...
          const prefs = {};
          for (let i = 0; i < titleIdList.length; i += 250) {
            const chunk = titleIdList.slice(i, i + 250);
            // eslint-disable-next-line no-await-in-loop
            const [stateDocs, prefDocs] = await Promise.all([
              db.getAll(...chunk.map((id) => userDoc.ref.collection("titleStates").doc(id))),
              db.getAll(...chunk.map((id) => userDoc.ref.collection("titleUpdatePrefs").doc(id))),
            ]);
            stateDocs.forEach((docSnap) => { if (docSnap.exists) states[docSnap.id] = docSnap.data() || {}; });
            prefDocs.forEach((docSnap) => { if (docSnap.exists) prefs[docSnap.id] = docSnap.data() || {}; });
          }
          ...
          // Quello che la sweep aveva già mandato singolarmente non si ripete:
          // vale per lo storico, visto che la sweep ora è spenta.
          const alreadyNotified = new Set();
          for (let i = 0; i < eventIds.length; i += 250) {
            const refs = eventIds.slice(i, i + 250)
              .map((id) => userDoc.ref.collection("notifications").doc(`title_update_${id}`));
            // eslint-disable-next-line no-await-in-loop
            const docs = await db.getAll(...refs);
            docs.forEach((docSnap) => {
              if (docSnap.exists) alreadyNotified.add(String(docSnap.id).replace(/^title_update_/, ""));
            });
          }
          ...
          const items = selectDigestItems({
            ...
            notifiedEventIds: alreadyNotified,
            nowMs,
          });
          if (!items.length) {
            report.skippedEmpty += 1;
          ...
          // `create` e non `set`: l'id è la settimana, quindi un run ripetuto
          // fallisce invece di mandare un secondo digest.
          // eslint-disable-next-line no-await-in-loop
          await userDoc.ref.collection("notifications").doc(notificationId).create(
            buildWeeklyDigestNotification({ admin, uid: userDoc.id, items, weekKey, nowMs })
          );
          report.written += 1;
        } catch (err) {
          if (String(err?.code) === "6" || String(err?.message || "").toLowerCase().includes("already exists")) {
            report.duplicates += 1;
            continue;
          }
          report.errors += 1;
          logger.warn("[digest] utente saltato", { uid: userDoc.id, error: String(err?.message || err).slice(0, 200) });
        }
      }

      if (usersSnap.size < 200) break;
    }

    logger.info("[digest] settimana chiusa", report);
    return null;
  });
...
// ============================================
// Da 7 a 21 giorni (2026-08-24): 2.780 promemoria scritti, letti al 2,1%.
// Stessa ragione del cooldown dei nudge, vedi INACTIVITY_NUDGE_COOLDOWN_MS.
const WATCHLIST_REMINDER_COOLDOWN_MS = 21 * 24 * 60 * 60 * 1000;
...
exports.sendWatchlistReminders = functions
  ...
  .onRun(async () => {
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      // Check cooldown
      const lastReminderMs = engagementMillis(u, "lastWatchlistReminderAt");
      ...
      batch.set(
        userDoc.ref,
        {
          ...
          "engagement.lastWatchlistReminderAt": admin.firestore.FieldValue.delete(),
        },
        { merge: true }
      );
      writeOps++;
...
exports.sendFriendActivityDigest = functions
  ...
  .onRun(async () => {
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      const lastDigestMs = engagementMillis(u, "lastFriendDigestAt");
      ...
      batch.set(
        userDoc.ref,
        {
          ...
          "engagement.lastFriendDigestAt": admin.firestore.FieldValue.delete(),
        },
        { merge: true }
      );
      writeOps++;
...
// ============================================
// I doc di tmdbCache pesano decine di KB l'uno (payload TMDB interi): un
// batch.commit() da 300 delete sfonda il limite di 10 MiB per transazione
// ("Transaction too big") e il cleanup falliva ogni notte, lasciando crescere
// la collection. BulkWriter spezza le scritture da solo, quindi qui si cancella
// a pagine piccole finche' c'e' tempo nel run.
const TMDB_CACHE_CLEANUP_PAGE = 500;
const TMDB_CACHE_CLEANUP_BUDGET_MS = 8 * 60 * 1000;
...
exports.cleanupTmdbCache = functions
  ...
  .onRun(async () => {
    ...
    try {
      while (Date.now() - startedAt < TMDB_CACHE_CLEANUP_BUDGET_MS) {
        let query = db.collection("tmdbCache")
          .where("expiresAtMs", "<=", cutoffMs)
          .orderBy("expiresAtMs")
          .limit(TMDB_CACHE_CLEANUP_PAGE);
        if (lastDoc) query = query.startAfter(lastDoc);

        // select mirato: legge solo il campo del cursore, non i payload interi
        // (senza il campo, startAfter(lastDoc) non puo' costruire il cursore).
        const snap = await query.select("expiresAtMs").get();
        if (snap.empty) break;

        snap.docs.forEach((docSnap) => { writer.delete(docSnap.ref); });
    ...
    logger.info(`[tmdb-cache] cleanup deleted=${deleted} elapsedMs=${Date.now() - startedAt}`);
    return null;
  });

const DAY_MS = 24 * 60 * 60 * 1000;
// I contatori engagement erano scritti con `batch.set()` e chiavi puntate
// ("engagement.lastNudgeAt"). In `set()` il punto NON e' un percorso: e' parte
// del nome del campo. Il valore finiva quindi in un campo letterale di primo
// livello e la lettura `u.engagement?.lastNudgeAt` non lo trovava mai: **i tre
// cooldown non hanno mai frenato niente**. E' questa la causa degli 11.116
// nudge (uno al giorno a testa, letti allo 0,5%), non un cooldown troppo corto.
// Solo `update()` interpreta i punti come percorso — vedi anonymizeQueryDocs
// in lib/accountDeletion.js, che infatti e' corretto.
// Dal 2026-08-24 si scrive annidato, si legge con fallback sul campo letterale
// (engagementMillis/engagementNumber in lib/pureUtils.js) e il campo letterale
// viene cancellato nello stesso write.
const { engagementMillis, engagementNumber } = pureUtils;

const INACTIVITY_NUDGE_AFTER_MS = 3 * DAY_MS;
// Cooldown alzato da 2 a 14 giorni il 2026-08-24. Misura che ha portato al
// cambio: 11.116 `engagement_nudge` scritte in tutto, **lette allo 0,5%**,
// ~275-320 al giorno fra nudge e promemoria watchlist, mediana 41 a testa e
// un massimo di 120 a una persona sola. Nel frattempo gli aggiornamenti
// editoriali (450 notifiche) sono letti al 5,1% e i messaggi nei thread al
// 36,7%: la campanella vale piu' di quanto la stavamo spendendo.
const INACTIVITY_NUDGE_COOLDOWN_MS = 14 * DAY_MS;
// Dopo N richiami senza che la persona rientri, si smette: se quattro nudge in
// due mesi non l'hanno riportata, il quinto non serve a niente. Il contatore
// riparte da zero appena torna attiva.
const INACTIVITY_NUDGE_MAX_UNANSWERED = 4;
...
// Corsia "in uscita" (2026-08-26). Il filtro `vote_count` della corsia storica
// misura il passato e non puo' vedere un titolo che deve ancora uscire: il 75%
// delle uscite dei 60 giorni successivi non era a catalogo. Qui si scopre per
// data futura, con la popolarita' come soglia e `vote_count` come seconda porta.
// Finestra a 45 giorni: lo scanner aggiornamenti fa il giro del catalogo in
// ~2,5 giorni, quindi anche l'ultimo arrivato viene scansionato (e produce
// l'evento di uscita notificabile) ben prima del debutto.
const TMDB_UPCOMING_AHEAD_DAYS = clamp(Number(process.env.TMDB_UPCOMING_AHEAD_DAYS || 45), 7, 120);
const TMDB_UPCOMING_MIN_POPULARITY = Math.max(0, Number(process.env.TMDB_UPCOMING_MIN_POPULARITY || 5));
const TMDB_UPCOMING_PAGES = clamp(Number(process.env.TMDB_UPCOMING_PAGES || 3), 1, 10);
const TMDB_UPCOMING_IMPORT_ENABLED = String(process.env.TMDB_UPCOMING_IMPORT_ENABLED || "true").trim().toLowerCase() !== "false";
...
exports.onRatingCreatedFeedEvent = functions
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      // Campanella admin: un voto e' l'azione piu' frequente dell'app, quindi
      // il modulo la raggruppa in finestre da 15 minuti.
      void recordAdminActivity({
        kind: "rating",
        actorUid,
        detail: {
          what: compactText(rating.titleName || rating.name, 80),
          titleId,
          ctaUrl: `/title.html?id=${encodeURIComponent(titleId)}`,
        },
      });
...
exports.onSeriesStartedFeedEvent = functions
  ...
  .onWrite(async (change, context) => {
    ...
    // Campanella admin: qui passa OGNI scrittura di titleState fatta a mano,
    // non solo le serie iniziate. Raggruppata a finestra dal modulo, se no un
    // utente che si ricostruisce la libreria a mano farebbe cento notifiche.
    if (!change.before.exists) {
      // Watchlist e libreria sono lo stesso documento visto da due lati: se il
      // titolo nasce solo "da vedere" e' watchlist, se nasce gia' iniziato o
      // visto e' libreria. La collezione `watchlist` e' una proiezione di
      // questa, quindi qui non si conta due volte.
      const isOnlyWatchlisted = Boolean(after.generalWatchlist)
        && !["in_progress", "completed_unrated", "rated", "seen_unrated"].includes(
          String(after.state || "").trim().toLowerCase()
        );
      void recordAdminActivity({
        kind: isOnlyWatchlisted ? "watchlist" : "library",
        actorUid: context.params.uid,
        detail: {
          what: compactText(after.titleName || after.name, 80),
...
exports.onPostCreatedFeedEvent = functions
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      // Campanella admin: un post e' raro abbastanza da meritare una riga sua.
      void recordAdminActivity({
        kind: "post",
        actorUid,
        eventId: postId,
        detail: {
          what: compactText(post.text, 120),
          postId,
          ctaUrl: `/community.html?post=${encodeURIComponent(postId)}`,
        },
      });
...
exports.onFollowCreatedFeedEvent = functions
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      // Campanella admin: chi segue chi e' il segnale di grafo che manca
      // altrove (gli analytics dicono quanti follow, non fra chi).
      void recordAdminActivity({
        kind: "follow",
        actorUid,
        eventId: `${actorUid}_${targetUid}`,
        detail: {
          what: await getDisplayNameForNotify(db, targetUid),
          ctaUrl: `/user.html?uid=${encodeURIComponent(targetUid)}`,
        },
      });
...
exports.onPostCommentCreatedFeedEvent = functions
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      // Campanella admin: i commenti sono il segnale di conversazione — quelli
      // che si vogliono vedere subito. Finestra corta (5') solo per non fare
      // dieci notifiche a chi risponde a raffica nello stesso thread.
      void recordAdminActivity({
        kind: "comment",
        actorUid,
        detail: {
          what: compactText(comment.text, 120),
          postId,
          ctaUrl: `/community.html?post=${encodeURIComponent(postId)}`,
        },
      });
  ...
  .https.onCall(async (data, context) => {
    ...
    if (wantCast && (castWithCharsMissing || castIdsSparse || directorIdsMissing)) {
      try {
        ...
        // Forma dei campi condivisa con scripts/import-tmdb-title.js
        // (lib/titleCastAssets.js): un titolo importato a mano deve nascere
        // con gli stessi castIds/directorIds che scriverebbe questa callable.
        const { cast, castIds: freshCastIds, directorIds: freshDirectorIds } =
          buildCastAssets(detailsRes?.data?.credits);
        ...
        if (castIdsSparse && freshCastIds.length > 0) {
          updates.castIds = admin.firestore.FieldValue.arrayUnion(...freshCastIds);
        }
        if (directorIdsMissing && freshDirectorIds.length > 0) {
          updates.directorIds = admin.firestore.FieldValue.arrayUnion(...freshDirectorIds);
        }
      } catch (err) {
  ...
  .https.onCall(async (data, context) => {
    ...
    if (cacheIsFresh) {
      ...
      try {
        ...
        if (titleData && titleData.watchProviderLogos === undefined) {
          ...
          await patchExistingTitle(db, titleId, { watchProviderLogos }, { context: "watchProviders.backfillLogos" });
        }
      } catch (e) {
    ...
    try {
      ...
      try {
        ...
        await patchExistingTitle(db, titleId, { watchProviderNames, watchProviderLogos }, { context: "watchProviders.denormalize" });
      } catch (e) {
...
// Stagioni chieste in un solo `append_to_response`. TMDB ne accetta 20, ma qui
// il tetto e' piu' basso di proposito: la risposta porta dentro l'overview di
// OGNI episodio e finisce nella cache TMDB su Firestore, dove un documento
// oltre 1 MB fallisce la scrittura (in silenzio, dentro il catch di
// writeFirestoreCacheEntry). Otto stagioni bastano ampiamente per la media —
// le serie piu' lunghe di cosi' tengono la durata episodio stabile.
const TMDB_MAX_APPENDED_SEASONS = 8;

// Arricchisce il payload `/tv/{id}` con i runtime REALI degli episodi, che
// TMDB espone solo dentro `/tv/{id}/season/{n}`. Senza di questi
// `resolveTvEpisodeRuntime` cade sul runtime del SINGOLO ultimo episodio —
// tipicamente un finale lungo il doppio — e quel numero moltiplica ogni
// episodio visto nelle ore del profilo (vedi lib/tmdbDurations.js).
//
// Costa UNA richiesta TMDB in piu' per serie, cachata come le altre e solo
// quando serve davvero: film esclusi, e serie che hanno gia' un
// `episode_run_time` credibile escluse a meno che il titolo non sia ancora
// senza durata (li' il valore reale vale la chiamata).
async function withTvEpisodeRuntimes(detailsPayload, { target, db, state, cacheScope }) {
  const details = asObject(detailsPayload);
  if (normalizeTmdbMediaType(target?.mediaType) !== "tv") return details;
  if (!toPositiveInt(target?.tmdbId)) return details;

  const seasonNumbers = safeArray(details.seasons)
    .map((row) => toPositiveInt(row?.season_number))
    .filter((n) => n > 0)
    .sort((a, b) => a - b)
    .slice(0, TMDB_MAX_APPENDED_SEASONS);
  if (!seasonNumbers.length) return details;

  const seasons = await fetchTmdbCachedJson(`/tv/${target.tmdbId}`, {
    language: "it-IT",
    append_to_response: seasonNumbers.map((n) => `season/${n}`).join(","),
  }, {
    db,
    state,
    cacheScope: cacheScope || "titleSeasonRuntimes_tv",
    // I runtime degli episodi gia' andati in onda non cambiano piu': TTL al
    // massimo consentito (fetchTmdbCachedJson clampa a 30 giorni) per non
    // ripagare la chiamata a ogni refresh notturno.
    ttlSeconds: 30 * 24 * 60 * 60,
    allowStaleOnError: true,
  }).catch((err) => {
    logger.warn("[tmdb] season runtimes fetch failed", {
  ...
  const payload = asObject(seasons?.data);
  const merged = { ...details };
  for (const key of Object.keys(payload)) {
    if (/^season\/\d+$/.test(key)) merged[key] = payload[key];
  }
  return merged;
}
...
// Il refresh vero e proprio, fuori dalla callable perche' lo usa anche lo
// sweeper notturno `refreshStaleTmdbTitles`: finche' l'unico innesco era
// l'apertura della scheda, un titolo che non apre nessuno non veniva
// rinfrescato mai. I termini API di TMDB non permettono di tenere in cache i
// loro dati oltre 6 mesi, e il nostro catalogo ha la coda lunga.
async function refreshTitleFromTmdbCore({ db, titleId, uid, force = false }) {
    const nowMs = Date.now();
    const titleRef = db.collection("titles").doc(titleId);
    ...
    if (txResult?.notFound) {
      return { ok: false, checked: false, updated: false, reason: "not_found" };
    }
    ...
    try {
      ...
      const detailsItWithRuntimes = await withTvEpisodeRuntimes(detailsIt?.data || {}, {
        target,
        db,
        state,
        cacheScope: "titleRefreshSeasonRuntimes_tv",
      });

      const { patch, changedFields } = buildTmdbTitleRefreshPatch({
        title: latestTitle,
        target,
        detailsIt: detailsItWithRuntimes,
...
exports.refreshTitleFromTmdb = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    const result = await refreshTitleFromTmdbCore({
      db,
      titleId,
      uid,
      force: data?.force === true && getAdminUids().includes(uid),
    });
...
// Quanti titoli tocca al massimo un giro, e quanto puo' durare: il tetto vero
// e' il budget, i numeri servono a non chiedere pagine enormi.
//
// Le misure del 2026-08-25, sul primo giro vero: 21.123 titoli in catalogo,
// 4.160 con `nextCheckAt`, quindi ~17k mai sincronizzati; 190 titoli in 72
// secondi. Coi tetti di partenza (150 + 40, una pagina sola) la prima
// copertura completa voleva piu' di un anno. Con questi, e quattro giri al
// giorno, il catalogo intero gira in un paio di settimane: dentro i 15 giorni
// che il refresh si prefigge, e molto dentro i 6 mesi che TMDB impone.
const TMDB_SWEEP_MAX_DUE = 400;
const TMDB_SWEEP_MAX_SEEDED = 400;
const TMDB_SWEEP_SCAN_PAGE = 300;
const TMDB_SWEEP_BUDGET_MS = 7 * 60 * 1000;
const TMDB_SWEEP_STATE_DOC = "systemJobs/tmdbRefreshSweep";

/// Rinfresca i titoli scaduti, e da' un `nextCheckAt` a quelli che non ce
/// l'hanno: senza il secondo passaggio i titoli piu' vecchi del sistema di
/// sync resterebbero invisibili alla query, che su un campo assente non
/// restituisce niente.
exports.refreshStaleTmdbTitles = require("firebase-functions/v2/scheduler").onSchedule(
  {
    region: "europe-west1",
    schedule: "40 */6 * * *",
    timeZone: "Europe/Rome",
  ...
  async () => {
    const db = admin.firestore();
    const startedAt = Date.now();
    const outOfBudget = () => Date.now() - startedAt > TMDB_SWEEP_BUDGET_MS;

    let refreshed = 0;
    let updated = 0;
    let seeded = 0;

    // 1) Scaduti: `nextCheckAt` nel passato, i piu' vecchi per primi.
    const dueSnap = await db.collection("titles")
      .where("tmdbSync.nextCheckAt", "<=", admin.firestore.Timestamp.fromMillis(Date.now()))
      .orderBy("tmdbSync.nextCheckAt")
      .limit(TMDB_SWEEP_MAX_DUE)
      .get();

    for (const docSnap of dueSnap.docs) {
      if (outOfBudget()) break;
      const res = await refreshTitleFromTmdbCore({ db, titleId: docSnap.id, uid: "scheduler" });
      if (res?.checked) refreshed += 1;
      if (res?.updated) updated += 1;
    }

    // 2) Mai sincronizzati: si scorre il catalogo per id con un cursore che
    // sopravvive fra un giro e l'altro. Finito il giro il cursore riparte da
    // capo, cosi' prende anche i titoli creati nel frattempo.
    const stateRef = db.doc(TMDB_SWEEP_STATE_DOC);
    let cursor = "";
    try {
      const stateSnap = await stateRef.get();
      cursor = safeString(stateSnap.data()?.scanCursor || "", 200);
    } catch (err) {
      logger.warn("[tmdb-sweep] stato non leggibile", {
        message: safeString(err?.message || String(err), 180),
      });
    }

    // Piu' pagine per giro: con 17k titoli da inizializzare, una pagina sola
    // al giorno non finiva prima dell'anno prossimo.
    while (!outOfBudget() && seeded < TMDB_SWEEP_MAX_SEEDED) {
      let scanQuery = db.collection("titles")
        .orderBy(admin.firestore.FieldPath.documentId())
        .limit(TMDB_SWEEP_SCAN_PAGE);
      if (cursor) scanQuery = scanQuery.startAfter(cursor);

      const scanSnap = await scanQuery.select("tmdbSync").get();

      if (scanSnap.empty) {
        // Giro finito: si riparte da capo, cosi' rientrano anche i titoli
      ...
      for (const docSnap of scanSnap.docs) {
        if (outOfBudget() || seeded >= TMDB_SWEEP_MAX_SEEDED) break;
        // Il cursore avanza sul doc VISTO, non a fine pagina: fermandosi a
        // meta' pagina per budget, il resto della pagina veniva saltato fino
        // al giro successivo del catalogo.
        cursor = docSnap.id;
        const sync = asObject(docSnap.data()?.tmdbSync);
        if (sync.nextCheckAt || sync.nextCheckAtMs || sync.syncDisabled === true) continue;
        const res = await refreshTitleFromTmdbCore({ db, titleId: docSnap.id, uid: "scheduler" });
        seeded += 1;
        if (res?.updated) updated += 1;
      }
...
const SERIES_PROGRESS_ACTIONS = new Set([
  "mark_series_episode",
  "mark_series_season",
  "mark_series_completed",
  "set_series_progress",
...
function actionNeedsWatchMetrics(action) {
  ...
  return type === "mark_movie_seen" || SERIES_PROGRESS_ACTIONS.has(type);
}
...
function titleHasRequiredWatchMetrics(title, action = {}) {
  ...
  // Ogni azione di progresso serie ha bisogno delle stagioni reali, non solo
  // della durata episodio: senza `totalEpisodeCount` la percentuale resta a 0
  // e `locateSeriesPosition` non riesce a dare S·E, quindi la card non si
  // muove e il foglio post-episodio non si apre (caso "Muertos S.r.l.",
  // titolo creato da ricerca TMDB con la sola `seasonsCount`).
  if (SERIES_PROGRESS_ACTIONS.has(actionType)) {
...
async function refreshTitleWatchMetricsIfNeeded({ db, titleId, action, requestedBy = "system" }) {
  ...
  const detailsItWithRuntimes = await withTvEpisodeRuntimes(detailsIt?.data || {}, {
    ...
    db,
    state,
    cacheScope: "titleMetricsSeasonRuntimes_tv",
  });
...
async function enrichSingleImportTitle(db, titleId, title, { state }) {
  ...
  await patchExistingTitle(db, titleId, {
    meta: metaPatch,
    updatedAt: admin.firestore.FieldValue.serverTimestamp(),
  }, { context: "importEnrich.durationMeta" });
  return "enriched";
}
...
exports.scheduledFirestoreExport = functions
  ...
  .timeZone("Europe/Rome")
  .onRun(async () => {
    // Backup in cantiere per costo (2026-08-24): l'export completo e' ~1,55M
    // doc = ~0,93 $ a run, e ogni run e' fatturato un read per documento.
    // Bucket, IAM e function restano pronti: per riaccendere basta la env
    // FIRESTORE_BACKUP_ENABLED=1 (functions/.env.gia-visto) + il job scheduler
    // `firebase-schedule-scheduledFirestoreExport-europe-west1` (in us-central1)
    // riportato a ENABLED. La guardia sta qui e non solo nello scheduler perche'
    // un `firebase deploy --only functions` riabilita il job da solo.
    if (process.env.FIRESTORE_BACKUP_ENABLED !== "1") {
      logger.info("[firestoreBackup] disattivato (FIRESTORE_BACKUP_ENABLED != 1), nessun export");
      return null;
    }
...
exports.computeProductMetricsSnapshot = functions
  ...
  .onRun(async () => {
    ...
    // Ritorno per coorte di iscrizione, derivato dai dati e non dagli eventi
    // client: `lastActiveAt - createdAt >= N giorni` significa "e' tornata
    // almeno una volta dopo N giorni". Serve perche' la telemetria di funnel
    // vive solo sul web mentre la base utenti e' quasi tutta iOS: cosi' il
    // numero vale per tutti e vale anche all'indietro.
    // Finestre larghe una settimana: le coorti giornaliere qui sono da 1-3
    // persone e una sola persona sposterebbe la percentuale di 50 punti.
    const retention = {
      d1: { cohort: 0, returned: 0 },
      d7: { cohort: 0, returned: 0 },
      d30: { cohort: 0, returned: 0 },
    };
    ...
    for (;;) {
      ...
      for (const d of snap.docs) {
        ...
        if (watched === 0) neverActivated += 1;
        if (created) {
          const sinceSignup = active - created;
          // Ogni coorte guarda solo chi ha avuto il tempo di tornare: chi si e'
          // iscritto ieri non puo' figurare come "non tornato dopo 7 giorni".
          if (created < now - 3 * DAY && created >= now - 10 * DAY) {
            retention.d1.cohort += 1;
            if (sinceSignup >= DAY) retention.d1.returned += 1;
          }
          if (created < now - 14 * DAY && created >= now - 21 * DAY) {
            retention.d7.cohort += 1;
            if (sinceSignup >= 7 * DAY) retention.d7.returned += 1;
          }
          if (created < now - 30 * DAY && created >= now - 60 * DAY) {
            retention.d30.cohort += 1;
            if (sinceSignup >= 30 * DAY) retention.d30.returned += 1;
          }
        }
      }
    ...
    logger.info("[productMetrics] snapshot", {
      ...
      activated48h,
      cohort48hSize,
      retentionD1: `${retention.d1.returned}/${retention.d1.cohort}`,
      retentionD7: `${retention.d7.returned}/${retention.d7.cohort}`,
      retentionD30: `${retention.d30.returned}/${retention.d30.cohort}`,
      neverActivated,
...
async function rebuildQuizThemesAggregate(db) {
  ...
  // themeList incluso nel return (oltre ai due totali storici) cosi' chi
  // ricostruisce anche quizMeta/sagas subito dopo puo' riusarlo senza una
  // seconda scansione di quizQuestions (vedi rebuildQuizSagasAggregate).
  return { totalTitles: themeList.length, totalQuestions: counted, themeList };
}

// ============================================
// Quiz: aggregato saghe giocabili (quizMeta/sagas)
// Raggruppa i themes (quizMeta/themes) in franchise giocabili (Harry Potter,
// MCU, Star Wars, ...) cosi' una partita puo' pescare domande da piu' titoli
// della stessa saga invece che da un solo titolo. Logica di raggruppamento
// pura in modules/quizSagas.js (manuale + derivata da titles.collectionId),
// qui c'e' solo l'orchestrazione Firestore. Ricostruito insieme ai themes
// (stesso trigger: callable admin + scheduled 24h), mai da un trigger
// per-domanda, per lo stesso motivo dei themes (niente hotspot sugli import
// batch).
// ============================================
async function rebuildQuizSagasAggregate(db, { themeList } = {}) {
  let themes = themeList;
  if (!Array.isArray(themes)) {
    const themesSnap = await db.collection("quizMeta").doc("themes").get();
    themes = asObject(themesSnap.data()).themes || [];
  }

  // Solo i titleId che hanno effettivamente domande giocabili: leggiamo i
  // doc titles a batch da 30 e solo i campi che servono al raggruppamento
  // (fieldMask), non l'intero documento titolo.
  const titleIds = uniqueIdList(themes.map((t) => t && t.titleId).filter(Boolean));
  ...
  // totalQuestions qui = domande coperte da una saga (somma dei count delle
  // saghe pubblicate), non il totale globale di quizMeta/themes: i due
  // numeri divergono quando ci sono titoli con quiz che non appartengono a
  // nessuna saga (titolo isolato, o saga sotto soglia).
  ...
  return { totalSagas: sagas.length, totalQuestions };
}
...
exports.rebuildQuizThemes = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    const sagasResult = await rebuildQuizSagasAggregate(db, { themeList: result.themeList });
    return {
      ok: true,
      totalTitles: result.totalTitles,
      totalQuestions: result.totalQuestions,
      totalSagas: sagasResult.totalSagas,
      totalSagaQuestions: sagasResult.totalQuestions,
    };
  });
...
exports.scheduledRebuildQuizThemes = functions
  ...
  .onRun(async () => {
    ...
    try {
      ...
      logger.info("[quizThemes] rebuilt", { totalTitles: result.totalTitles, totalQuestions: result.totalQuestions });
    } catch (err) {
      logger.error("[quizThemes] rebuild failed", { message: err?.message || String(err) });
    }
    // Try/catch separato: un errore nella ricostruzione saghe non deve far
    // apparire fallita anche la ricostruzione themes (gia' scritta sopra, o
    // fallita per conto suo con il suo log dedicato).
    try {
      const sagasResult = await rebuildQuizSagasAggregate(db, themeList ? { themeList } : undefined);
      logger.info("[quizSagas] rebuilt", sagasResult);
    } catch (err) {
      logger.error("[quizSagas] rebuild failed", { message: err?.message || String(err) });
    }
    return null;
  });
...
// getGuestQuiz accetta anche {sagaId,count}: pesca da più titoli della stessa
// saga (quizMeta/sagas) invece che da un solo titolo — vedi buildGuestSagaQuiz.
// ============================================
const GUEST_QUIZ_MAX_COUNT = 10;
const GUEST_QUIZ_MIN_COUNT = 3;
// Saga guest quiz: pochi titoli della saga per partita (una query per
// titleId, MAI due clausole "in" nella stessa query composita), ma di ognuno
// si prende il pool COMPLETO. Il limite per titolo deve restare >= al numero
// di domande che un titolo puo' avere (oggi max 121), se no Firestore
// ritorna sempre le stesse prime N per documentId e meta' del corpus della
// saga non uscirebbe mai. La varieta' fra partite viene dall'estrazione
// casuale dei titoli, non dal troncamento della query.
const GUEST_QUIZ_SAGA_MAX_TITLES = 3;
const GUEST_QUIZ_SAGA_POOL_LIMIT_PER_TITLE = 130;
...
function formatGuestQuizQuestions(rawQuestions) {
...
// Pool di una saga: fino a GUEST_QUIZ_SAGA_MAX_TITLES titoli scelti a caso
// tra quelli della saga, interrogati IN PARALLELO (Promise.all) invece che
// con una singola query "titleId in [...]" + "status in [...]" — Firestore
// non permette due clausole "in" sulla stessa query.
async function buildGuestSagaQuiz(db, sagaId, count) {
  const sagasSnap = await db.collection("quizMeta").doc("sagas").get();
  const sagas = asObject(sagasSnap.data()).sagas || [];
  const saga = sagas.find((s) => s && s.sagaId === sagaId);
  if (!saga) throw new functions.https.HttpsError("not-found", "Saga non disponibile.");

  const candidateTitleIds = pickRandomN(
    Array.isArray(saga.titleIds) ? saga.titleIds : [],
    GUEST_QUIZ_SAGA_MAX_TITLES
  );
  const snaps = await Promise.all(candidateTitleIds.map((titleId) => db.collection("quizQuestions")
    .where("titleId", "==", titleId)
    .where("status", "in", QUIZ_THEME_PLAYABLE_STATUSES)
    .limit(GUEST_QUIZ_SAGA_POOL_LIMIT_PER_TITLE)
    .get()));

  const pool = snaps
    .flatMap((snap) => snap.docs.map((d) => ({ id: d.id, ...(d.data() || {}) })))
    .filter((q) => isQuizLanguage(q));
  if (!pool.length) throw new functions.https.HttpsError("not-found", "Nessuna domanda per questa saga.");

  const questions = formatGuestQuizQuestions(pickRandomN(pool, count));
  const sessionToken = await issueGuestQuizSession(db, questions.map((q) => q.questionId));
  await maybeCleanupExpiredGuestQuizSessions(db).catch(() => {});

  return {
    ok: true,
    sessionToken,
    sagaId,
    sagaName: safeString(saga.name || "", 180),
    isSaga: true,
    titleId: questions[0]?.titleId || "",
    title: questions[0]?.title || "",
    mediaType: safeString(saga.mediaType || questions[0]?.mediaType || "", 20),
...
exports.getGuestQuiz = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    // Saga esplicita (es. "gioca tutta la saga di Harry Potter"): pesca da
    // più titoli invece che da uno solo. Firma invariata per il resto —
    // titleId continua a funzionare esattamente come prima.
    const sagaId = toId(data?.sagaId);
    if (sagaId) return buildGuestSagaQuiz(db, sagaId, count);
...
exports.submitGuestQuiz = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (ids.length !== answers.length || new Set(ids).size !== ids.length) {
      throw new functions.https.HttpsError("invalid-argument", "questionId mancanti o duplicati.");
    }
    await requireGuestQuizSession(db, data?.sessionToken, ids);

    const refs = ids.map((id) => db.collection("quizQuestions").doc(id));
    const snaps = await db.getAll(...refs);
    const byId = new Map();
    for (const s of snaps) {
      const q = s.exists ? (s.data() || {}) : {};
      if (!s.exists || !QUIZ_THEME_PLAYABLE_STATUSES.includes(q.status) || !isQuizLanguage(q)) {
        throw new functions.https.HttpsError("failed-precondition", "Una domanda non è più disponibile. Ricomincia la partita.");
      }
      byId.set(s.id, q);
    }
...
async function runAccountDataCleanup({ uid, source, deleteAuthUser = false }) {
  const db = admin.firestore();
  ...
  if (!lease.acquired) return { ok: true, skipped: true, reason: lease.reason };

  try {
    const userSnap = await db.collection("users").doc(uid).get().catch(() => null);

    // Campanella admin: chi se ne va va saputo mentre succede, non a fine mese
    // dagli analytics. Il nome si legge PRIMA della cancellazione, se no resta
    // solo un uid.
    const deletedName = (() => {
    ...
    void recordAdminActivity({
      ...
      eventId: `${uid}_${source}`,
      detail: { what: source ? `via ${source}` : "" },
    });
    ...
    logger.error("[account-delete] failed", {
      uid,
      source,
      code: String(err?.code || "unknown").slice(0, 80),
    });
...
exports.reconcileUserDataOnAuthDelete = functions
  .runWith({ timeoutSeconds: 540, memory: "1GB" })
  .region("europe-west1")
  .auth.user().onDelete(async (userRecord) => {
    const uid = String(userRecord?.uid || "").trim();
    if (!uid) return null;
    const deletionSnap = await admin.firestore()
      .collection("accountDeletionRequests")
      .doc(uid)
      .get()
      .catch(() => null);
    const source = authDeletionSource(deletionSnap?.data?.() || {});
    const result = await runAccountDataCleanup({ uid, source });
    if (result.skipped && result.reason === "lease-active") {
      throw new Error("account-deletion-lease-active");
    }
    return null;
  });
...
exports.deleteMyAccount = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (userData.isAdmin === true || getAdminUids().includes(uid)) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Gli account admin non possono essere eliminati da questa schermata."
      );
    }

    try {
      const result = await runAccountDataCleanup({
        uid,
        source: "self-serve",
        deleteAuthUser: true,
      });
      if (result.skipped && result.reason === "lease-active") {
        throw new functions.https.HttpsError(
          "aborted",
          "La cancellazione dell'account è già in corso."
        );
      }
      return result;
    } catch (err) {
      ...
      try {
        ...
        if (!target?.tmdbId) {
          ...
          const detailsItWithRuntimes = await withTvEpisodeRuntimes(detailsIt?.data || {}, {
            ...
            db,
            state,
            cacheScope: "metadataBackfillSeasonRuntimes_tv",
          });
...
// --- Persone da seguire ------------------------------------------------------
// Il grafo follower esiste dall'inizio ma non viene mai proposto da nessuna
// parte: al 2026-08-19 sono 23 utenti su 343 ad aver seguito qualcuno (91 archi
// in tutto), quindi ogni post nasce con un pubblico di zero.
//
// Segnale usato: i voti. `ratings` e' gia' leggibile da qualsiasi utente
// loggato, quindi citare "3 voti in comune, tra cui Dune" non rivela niente di
// nuovo. `titleStates` invece e' owner-only e non puo' entrare nella
// motivazione — vedi il commento in `lib/peopleSuggestions.js`.
const PEOPLE_SUGGESTIONS_TTL_MS = 12 * 60 * 60 * 1000;
const PEOPLE_SUGGESTIONS_RATING_SCAN = 400;

async function loadMyTitleRatings(db, uid, { limit = PEOPLE_SUGGESTIONS_RATING_SCAN } = {}) {
  const snap = await db
    .collection("ratings")
    .where("uid", "==", uid)
    .where("level", "==", "title")
  ...
  return snap.docs
    .map((docSnap) => {
      const data = docSnap.data() || {};
      return {
        titleId: toId(data.titleId),
        rating: Number(data.rating),
        updatedAtMs: toMillis(data.updatedAt) || toMillis(data.createdAt) || 0,
      };
    })
    .filter((row) => row.titleId);
}

/**
 * Votanti per titolo. Una query per seed, in gruppi da 6 per non aprire 25
 * connessioni insieme. Si tiene solo il livello `title`: i voti per stagione o
 * episodio dello stesso utente sono lo stesso segnale contato piu' volte.
 */
async function loadRatersByTitle(db, titleIds, { perTitle = PEOPLE_MAX_RATERS_PER_TITLE } = {}) {
  const out = new Map();
  const ids = safeArray(titleIds).map((id) => toId(id)).filter(Boolean);
  const GROUP = 6;
  ...
  for (let i = 0; i < ids.length; i += GROUP) {
    ...
    group.forEach((titleId, idx) => {
      ...
      snap.docs.forEach((docSnap) => {
        ...
        seenUids.add(uid);
        rows.push({ uid, rating: Number(data.rating) });
      });
      if (rows.length) out.set(titleId, rows);
    });
  }
...
/**
 * Pagine editoriali da proporre in coda ai suggerimenti.
 *
 * Stanno in fondo e non in mezzo alle persone: una pagina non e' qualcuno che
 * hai in comune con qualcosa, e' un canale. Chi gia' la segue non la rivede.
 */
async function loadEditorialPagesToFollow(db, excludeUids) {
  const snap = await db
    .collection("users")
  ...
  return (snap?.docs || [])
    .filter((docSnap) => !excludeUids.has(docSnap.id))
    .map((docSnap) => {
      const data = docSnap.data() || {};
      return {
        uid: docSnap.id,
        ...
        photoURL: String(data.photoURL || data.avatarURL || "") || null,
        followersCount: Number(data.followersCount || 0) || 0,
...
/** uid gia' seguiti: non ha senso suggerirli di nuovo. */
async function loadFollowingUids(db, uid) {
  const snap = await db
    .collection("users")
    .doc(uid)
...
/** uid che ho bloccato io. Chi ha bloccato me si verifica solo sui finalisti. */
async function loadBlockedUids(db, uid) {
  const snap = await db
    .collection("users")
    .doc(uid)
...
exports.getPeopleSuggestions = functions
  .region("europe-west1")
  .https.onCall(async (data, context) => {
    if (!context.auth?.uid) {
      throw new functions.https.HttpsError("unauthenticated", "Devi accedere per vedere i suggerimenti.");
    }

    const uid = context.auth.uid;
    const db = admin.firestore();
    await enforceCallableRateLimit(db, uid, "peopleSuggestions", {
      windowSeconds: 10,
      maxInWindow: 3,
      dailyMax: 60,
    });

    const max = clamp(Number(data?.max || 8), 3, PEOPLE_MAX_SUGGESTIONS);
    const force = data?.force === true;
    const cacheRef = db.collection("users").doc(uid).collection("suggestions").doc("people");

    if (!force) {
      const cached = await cacheRef.get().catch(() => null);
      const computedAtMs = toMillis(cached?.data()?.computedAt) || 0;
      if (cached?.exists && Date.now() - computedAtMs < PEOPLE_SUGGESTIONS_TTL_MS) {
        return {
          people: safeArray(cached.data()?.people).slice(0, max),
          computedAtMs,
          cached: true,
        };
      }
    ...
    const [myRatings, followingUids, blockedUids] = await Promise.all([
      loadMyTitleRatings(db, uid),
      loadFollowingUids(db, uid),
      loadBlockedUids(db, uid),
    ]);

    const excludeFromPages = new Set([uid, ...followingUids, ...blockedUids]);
    const seedTitleIds = pickSeedTitles(myRatings, { max: PEOPLE_MAX_SEED_TITLES });
    if (!seedTitleIds.length) {
      // Senza voti non sappiamo a chi somigli, e inventare somiglianze sarebbe
      // peggio del vuoto. Le pagine pero' si possono proporre a chiunque: sono
      // canali, non persone con cui hai qualcosa in comune.
      const pagesOnly = await loadEditorialPagesToFollow(db, excludeFromPages);
      return { people: pagesOnly, computedAtMs: Date.now(), cached: false, reason: "no_ratings" };
    }

    const myRatingByTitle = new Map(myRatings.map((row) => [row.titleId, row.rating]));
    const ratersByTitle = await loadRatersByTitle(db, seedTitleIds);

    const excludeUids = new Set([uid, ...followingUids, ...blockedUids]);
    const ranked = scorePeopleCandidates({
      myRatingByTitle,
      ratersByTitle,
      excludeUids,
      // Si classifica largo e si filtra dopo: profili cancellati, sintetici o
      // che mi hanno bloccato si scoprono solo leggendo i doc utente.
      max: max * 3,
    });
    ...
    if (!ranked.length) {
      const pagesOnly = await loadEditorialPagesToFollow(db, excludeFromPages);
      await cacheRef.set(
        { people: pagesOnly, computedAt: admin.firestore.FieldValue.serverTimestamp() },
        { merge: true }
      );
      return { people: pagesOnly, computedAtMs: Date.now(), cached: false, reason: "no_candidates" };
    }

    const userDocs = await db.getAll(...ranked.map((row) => db.collection("users").doc(row.uid)));
    const usable = [];
    ranked.forEach((row, idx) => {
      const snap = userDocs[idx];
      const user = snap?.exists ? snap.data() || {} : null;
      if (!user) return;
      if (user.isDeleted === true) return;
      if (isGuidedUid(row.uid) || isGuidedUserData(user)) return;
      const displayName = String(user.displayName || user.username || "").trim();
      if (!displayName) return;
      usable.push({ ...row, user, displayName });
    });

    // Chi ha bloccato me: una lettura per finalista, solo sui pochi che restano.
    const finalists = usable.slice(0, max);
    ...
    const visible = finalists.filter((_, idx) => !blockChecks[idx]);

    // Nomi dei titoli votati da entrambi: sono gia' pubblici (ratings e'
    // leggibile dai loggati) e servono al client per la motivazione.
    const titleIdsForReason = new Set();
    visible.forEach((row) =>
      row.sharedTitleIds.slice(0, PEOPLE_MAX_REASON_TITLES).forEach((id) => titleIdsForReason.add(id))
    );
    const titleNameById = new Map();
    if (titleIdsForReason.size) {
      const titleSnaps = await db.getAll(
        ...Array.from(titleIdsForReason).map((id) => db.collection("titles").doc(id))
      );
      titleSnaps.forEach((snap) => {
        if (snap.exists) titleNameById.set(snap.id, String(snap.data()?.name || "").trim());
      });
    }
    ...
    const people = visible.map((row) => ({
      ...
      photoURL: String(row.user.photoURL || row.user.avatarURL || "") || null,
      followersCount: Number(row.user.followersCount || 0) || 0,
      sharedCount: row.sharedCount,
      // I nomi servono al client per scrivere "tra cui X e Y" nella sua lingua:
      // la frase non si costruisce qui, altrimenti resta italiana ovunque.
      sharedTitleNames: row.sharedTitleIds
        .slice(0, PEOPLE_MAX_REASON_TITLES)
        .map((id) => titleNameById.get(id))
        .filter(Boolean),
    }));

    const pages = await loadEditorialPagesToFollow(db, excludeFromPages);
    const items = [...people, ...pages];

    await cacheRef.set(
      { people: items, computedAt: admin.firestore.FieldValue.serverTimestamp() },
      { merge: true }
    );

    return { people: items, computedAtMs: Date.now(), cached: false };
  });
...
async function buildPersonalAnalyticsUserRow(db, userDoc, lifecycleContext = {}) {
  const uid = userDoc.id;
  const data = userDoc.data() || {};
  const profilePresent = lifecycleContext.profilePresent ?? userDoc.exists !== false;
  ...
  const [
    ...
    lifecycleContext.authUser === undefined
      ? admin.auth().getUser(uid).catch(() => null)
      : Promise.resolve(lifecycleContext.authUser),
    db.collection("usersPrivate").doc(uid).get().catch(() => null),
    userRef.collection("_system").doc("signupAttribution").get().catch(() => null),
  ...
  const authUser = authUserLookup || null;
  ...
  const providerIds = providerIdsForAuthUser(authUser).map((id) => safeString(id, 48));
  const authCreatedAt = authUser?.metadata?.creationTime
    ? new Date(authUser.metadata.creationTime).toISOString()
    : null;
  const authLastSignInAt = authUser?.metadata?.lastSignInTime
    ? new Date(authUser.metadata.lastSignInTime).toISOString()
    : null;
  const createdAtIso = analyticsIso(data.createdAt)
    || authCreatedAt
    || analyticsIso(lifecycleContext.pendingData?.createdAt)
    || analyticsIso(lifecycleContext.deletionData?.requestedAt);
  // lastActiveAt, welcome chat e notifiche sono artefatti automatici: non
  // rappresentano un'attivazione reale.
  const firstUsefulAt = analyticsFirstIso(firstTitleStateIso, firstQuizIso, firstImportIso);
  ...
  const isSynthetic = isGuidedUserData(data);
  ...
  const lifecycleStatus = classifySignupLifecycle({
    ...
    emailVerified,
    isSynthetic,
    pendingStatus: lifecycleContext.pendingData?.status || null,
    deletionStatus: lifecycleContext.deletionData?.status || null,
  });
  const row = {
    uid,
    ...
    auth: {
      ...
      legacyEmailUnverified: authUser?.customClaims?.legacyEmailUnverified === true,
    },
    profilePresent,
    lifecycleStatus,
    pendingStatus: lifecycleContext.pendingData?.status || null,
    ...
    attribution: {
      provider: safeString(attribution.provider || "", 48) || null,
      surface: safeString(attribution.surface || "", 20) || "unknown",
      appVersion: safeString(attribution.appVersion || "", 40) || null,
      selfReportedSource: safeString(attribution.selfReportedSource || "", 40) || null,
    },
    accountType: safeString(data.accountType || "", 40) || (isSynthetic ? "synthetic" : "real"),
    isSynthetic,
    platform: {
  ...
  row.realActionCount = Object.entries(row.summary)
    .filter(([key]) => !["minutes", "quizXp"].includes(key))
    .reduce((sum, [, value]) => sum + Number(value || 0), 0);
  return row;
}

function buildPersonalAnalyticsOverview(users, signupLifecycle = null) {
  const nowMs = Date.now();
  const dayMs = 24 * 60 * 60 * 1000;
  const realUsers = users.filter((u) => !u.isSynthetic && u.lifecycleStatus !== "deleted");
  const activeProfiles = users.filter((u) => ["active", "legacy_unverified"].includes(u.lifecycleStatus));
  const syntheticUsers = users.filter((u) => u.isSynthetic).length;
  const lifecycleCounts = users.reduce((counts, user) => {
    const key = user.lifecycleStatus || "unknown";
  ...
  const activationTiming = aggregateActivationTiming(realUsers.map((user) => ({
    createdAtMs: Date.parse(user.createdAt || "") || 0,
    firstUsefulAtMs: Date.parse(user.firstUsefulAt || "") || 0,
  })));

  return {
    sampleSize: users.length,
    realUsers: activeProfiles.length,
    syntheticUsers,
    lifecycleCounts: signupLifecycle?.lifecycleCounts || lifecycleCounts,
    signupLifecycle,
...
async function listAllAnalyticsAuthUsers() {
  const users = [];
  let pageToken;
  do {
    // Il progetto verificato ha poche centinaia di account. La paginazione
    // resta esplicita per non trasformare il pannello in una vista parziale.
    // eslint-disable-next-line no-await-in-loop
    const page = await admin.auth().listUsers(1_000, pageToken);
    users.push(...page.users);
    pageToken = page.pageToken;
  } while (pageToken);
  return users;
...
function analyticsDoc(id, data, exists = true) {
...
exports.getPersonalAdminAnalytics = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    const [authUsers, usersSnap, pendingSnap, deletionSnap] = await Promise.all([
      listAllAnalyticsAuthUsers(),
      db.collection("users").get(),
      db.collection("pendingSignups").get().catch(() => ({ docs: [] })),
      db.collection("accountDeletionRequests").get().catch(() => ({ docs: [] })),
    ]);

    const authByUid = new Map(authUsers.map((user) => [user.uid, user]));
    const profileByUid = new Map(usersSnap.docs.map((doc) => [doc.id, doc]));
    const pendingByUid = new Map(pendingSnap.docs.map((doc) => [doc.id, doc.data() || {}]));
    const deletionByUid = new Map(deletionSnap.docs.map((doc) => [doc.id, doc.data() || {}]));
    ...
    const candidateRows = [...candidateUids].map((uid) => {
      const authUser = authByUid.get(uid) || null;
      const profileDoc = profileByUid.get(uid) || null;
      const profileData = profileDoc?.data?.() || {};
      const pendingData = pendingByUid.get(uid) || null;
      const deletionData = deletionByUid.get(uid) || null;
      const sortMs = Math.max(
        analyticsTimestampMs(profileData.createdAt),
        analyticsTimestampMs(authUser?.metadata?.creationTime),
        analyticsTimestampMs(pendingData?.createdAt),
        analyticsTimestampMs(deletionData?.updatedAt)
      );
      return { uid, authUser, profileDoc, pendingData, deletionData, sortMs };
    }).sort((left, right) => right.sortMs - left.sortMs).slice(0, max);

    const signupLifecycle = aggregateSignupLifecycleMetrics([...candidateUids].map((uid) => {
      const authUser = authByUid.get(uid) || null;
      const profileData = profileByUid.get(uid)?.data?.() || {};
      const pendingData = pendingByUid.get(uid) || {};
      const deletionData = deletionByUid.get(uid) || {};
      return {
        authPresent: Boolean(authUser),
        profilePresent: profileByUid.has(uid),
        providerIds: providerIdsForAuthUser(authUser),
        emailVerified: authUser?.emailVerified === true,
        isSynthetic: isGuidedUserData(profileData),
        pendingStatus: pendingData.status || null,
        deletionStatus: deletionData.status || null,
        deletionSource: deletionData.source || null,
        surface: pendingData.attribution?.surface || "unknown",
        reservedEmailDomain: isReservedEmailDomain(authUser?.email || ""),
        pendingCreatedAtMs: analyticsTimestampMs(pendingData.createdAt),
        pendingCompletedAtMs: analyticsTimestampMs(pendingData.completedAt),
      };
    }));
    ...
    for (const candidate of candidateRows) {
      // eslint-disable-next-line no-await-in-loop
      users.push(await buildPersonalAnalyticsUserRow(
        db,
        candidate.profileDoc || analyticsDoc(candidate.uid, {}, false),
        {
          authUser: candidate.authUser,
          profilePresent: Boolean(candidate.profileDoc),
          pendingData: candidate.pendingData,
          deletionData: candidate.deletionData,
        }
      ));
    }
    ...
    return {
      ...
      overview: buildPersonalAnalyticsOverview(users, signupLifecycle),
      users,
    };
  });
...
exports.importRecentTmdbTitles = functions
  ...
  .onRun(async () => {
    ...
    try {
      ...
      // Le uscite per prime: sono l'unica cosa in questa funzione che ha una
      // scadenza. Il resto del catalogo puo' aspettare il giro dopo, un film che
      // esce fra tre giorni no.
      if (TMDB_UPCOMING_IMPORT_ENABLED) {
        try {
          const upcomingOptions = {
            aheadDays: TMDB_UPCOMING_AHEAD_DAYS,
            minPopularity: TMDB_UPCOMING_MIN_POPULARITY,
            pages: TMDB_UPCOMING_PAGES,
            voteCountGte: 10,
            nowMs: startedAtMs,
          };
          const [upcomingMovies, upcomingTv] = await Promise.all([
            fetchTmdbUpcomingCandidatesForType("movie", state, upcomingOptions),
            fetchTmdbUpcomingCandidatesForType("tv", state, upcomingOptions),
          ]);
          const upcoming = [
            ...(upcomingMovies.candidates || []),
            ...(upcomingTv.candidates || []),
          ];
          stats.upcomingPoolCount = upcoming.length;
          stats.upcomingSelected = await enqueueCandidates(upcoming);
          logger.info(`[tmdb-import] in uscita: ${upcoming.length} candidati, ${stats.upcomingSelected} nuovi`);
        } catch (err) {
          // Una finestra uscite che fallisce non deve fermare l'import normale.
          stats.upcomingError = safeString(err?.message || String(err), 220);
          logger.warn(`[tmdb-import] corsia uscite fallita: ${stats.upcomingError}`);
        }
      }
...
exports.onUserListCreatedSlug = functions
  ...
  .onCreate(async (snap, context) => {
    ...
    // Campanella admin: una lista creata e' un segnale d'uso che non compare
    // altrove (le liste private non entrano nel feed). Sta qui e non in un
    // trigger suo perche' su `userLists` un onCreate gia' gira: una funzione
    // in meno da deployare e da pagare.
    const listOwnerUid = toId(data.ownerUid);
    if (listOwnerUid) {
      void recordAdminActivity({
        kind: "list",
        actorUid: listOwnerUid,
        eventId: String(context.params.listId || snap.id || ""),
        detail: {
          what: `${compactText(data.name, 60) || "senza nome"} (${String(data.visibility || "private").trim().toLowerCase()})`,
          ctaUrl: `/lista.html?id=${encodeURIComponent(String(context.params.listId || snap.id || ""))}`,
        },
      });
    }
