...
const { buildClientErrorAlert, isAlreadyExistsError } = require("../lib/clientErrorAlert");
...
module.exports = ({
  ...
  adminUids = [],
  getAdminUids = null,
  // Campanella admin sull'attivita' degli utenti (modules/adminActivity.js).
  // Best-effort: se non arriva, i trigger continuano a funzionare uguale.
  recordAdminActivity = null,
}) => {
  ...
  function getPushCooldownMs(type) {
    ...
    if (type === "client_error_alert") return 30 * 60 * 1000;
  ...
  async function acquirePushCooldownWindow(db, userId, notifType, data = {}) {
    ...
    // Sulle uscite la finestra e' per titolo (lib/pushDelivery.js); per tutto
    // il resto resta una sola finestra per tipo.
    const scopeKey = pushCooldownScope(notifType, data);
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      const byScope = snap.data()?.byScope || {};
      const lastScopeMs = toMillis(byScope[scopeKey]);
      if (lastScopeMs && (nowMs - lastScopeMs) < cooldownMs) {
        return { allowed: false, remainingMs: cooldownMs - (nowMs - lastScopeMs) };
      }
      // Restano solo le finestre ancora aperte, quindi la mappa tiene pochi
      // titoli. mergeFields e non merge: merge fonderebbe le mappe e non
      // toglierebbe mai una chiave scaduta.
      const open = {};
      for (const [key, value] of Object.entries(byScope)) {
        const ms = toMillis(value);
        if (ms && (nowMs - ms) < cooldownMs) open[key] = value;
      }
      open[scopeKey] = admin.firestore.Timestamp.now();
      tx.set(
        cooldownRef,
        { lastPushAt: admin.firestore.Timestamp.now(), byScope: open },
        { mergeFields: ["lastPushAt", "byScope"] }
      );
      return { allowed: true, remainingMs: 0 };
    });
  ...
  // Gli errori runtime gia' raccolti in `clientErrors` non devono restare
  // invisibili in Firestore. L'id deterministico giorno+fingerprint produce
  // al massimo una notifica/push al giorno per errore distinto. Il payload
  // non copia messaggio, stack, query string o user agent.
  const notifyAdminOnClientError = functions
    .region("europe-west1")
    .firestore
    .document("clientErrors/{errorId}")
    .onCreate(async (snap, context) => {
      const errorData = snap.data() || {};
      // Estensioni del browser e promise rifiutate dal motore non sono bug
      // nostri: il doc resta (TTL 14gg) ma non sveglia nessuno.
      if (isIgnorableClientError(errorData)) {
        logger.info("[clientErrors] rumore di terze parti, nessun alert", {
          errorId: context.params.errorId,
        });
        return null;
      }

      const adminUids = [...new Set(resolveAdminUids())];
      if (!adminUids.length) {
        logger.warn("[clientErrors] ADMIN_UIDS vuoto, alert non inviato");
        return null;
      }
      ...
      await Promise.all(adminUids.map(async (adminUid) => {
        const ref = db.collection("users").doc(adminUid)
          .collection("notifications").doc(plan.notificationId);
        try {
          await ref.create({
            toUid: adminUid,
            fromUid: "system",
            type: "client_error_alert",
            data: plan.data,
            read: false,
            createdAt: admin.firestore.FieldValue.serverTimestamp(),
            expiresAt: expiresAtValue(),
      ...
      logger.error("[clientErrors] errore web segnalato agli admin", {
        fingerprint: plan.fingerprint,
        pagePath: plan.pagePath,
        admins: adminUids.length,
      });
      return null;
    });
  ...
  const pushOnNotificationCreate = functions
    ...
    .onCreate(async (snap, context) => {
      ...
      // L'esito resta sul doc (`pushDelivery`): "gli e' arrivata?" si legge
      // li', e la guardia giornaliera (modules/notificationHealth.js) conta gli
      // esiti senza passare dai log. update() e non set(merge): se l'utente ha
      // gia' cancellato la notifica, non va resuscitata.
      async function recordDelivery(fields) {
        try {
          await snap.ref.update({
            pushDelivery: {
              ...buildPushDelivery(fields),
              at: admin.firestore.FieldValue.serverTimestamp(),
            },
          });
        } catch (err) {
          if (Number(err?.code) === 5 || String(err?.code || "") === "not-found") return;
          logger.warn(`[push] pushDelivery write failed for user=${userId} notif=${notificationId}: ${err.message}`);
        }
      }
      ...
      if (!allowed) {
        ...
        await recordDelivery({ status: PUSH_STATUS.PREFS_OFF });
        return null;
      }

      logger.info(`[push] Trigger for user=${userId} notif=${notificationId} type=${notif.type}`);

      // Token prima del cooldown: senza token la push non puo' partire, e
      // prenotare la finestra servirebbe solo a scrivere un doc in piu' (tre
      // tentativi su quattro, misura del 2026-09-10). Cosi' `cooldown`
      // nell'esito vuol dire "raggiungibile ma trattenuta", non "forse".
      ...
      if (!tokens.length) {
        ...
        await recordDelivery({ status: PUSH_STATUS.NO_TOKEN });
        return null;
      }

      logger.info(`[push] Found ${tokens.length} token(s) for user=${userId}`);

      const cooldownCheck = await acquirePushCooldownWindow(db, userId, notif.type, notif.data || {});
      if (!cooldownCheck.allowed) {
        logger.info(
          `[push] Cooldown active for user=${userId} type=${notif.type} remainingMs=${cooldownCheck.remainingMs}`
        );
        await recordDelivery({ status: PUSH_STATUS.COOLDOWN, tokens: tokens.length });
        return null;
      }
      ...
      let notificationLanguage = "it";
      try {
        const languageSnap = await db.collection("usersPrivate").doc(userId).get();
        notificationLanguage = String(languageSnap.data()?.language || "it").trim().toLowerCase();
      } catch (err) {
        logger.warn(`[push] language lookup failed for uid=${userId}: ${err.message}`);
      }
      const titleUpdate = titleUpdatePresentation(notif.data || {}, notificationLanguage);
      ...
      const fallbackTitle =
        ...
        type === "weekly_digest" ? "Novità sui tuoi titoli" :
        ...
        type === "admin_activity" ? fromName :
        type === "client_error_alert" ? "Errore web rilevato" :
        type === "push_coverage_report" ? "Copertura notifiche" :
        type === "official_source_report" ? "Fonti ufficiali da controllare" :
        type === "import_health_alert" ? "Import da controllare" :
        type === "notification_health_alert"
          ? (notif.data?.state === "recovered" ? "Notifiche di nuovo regolari" : "Notifiche da controllare")
          :
      ...
      const fallbackBody =
        ...
        type === "weekly_digest" ? (notif.data?.message || notif.data?.preview || "Ci sono novità sui titoli che segui") :
        ...
        type === "official_source_report" ? (notif.data?.message || "Ci sono annunci ufficiali da rivedere") :
        type === "import_health_alert" ? (notif.data?.message || "Ci sono import da controllare") :
        type === "notification_health_alert" ? (notif.data?.message || "Ci sono notifiche da controllare") :
        type === "admin_import_started" ? (() => {
        ...
        type === "admin_activity" ? (notif.data?.message || "Nuova attivita' su Somto") :
        type === "client_error_alert" ? (notif.data?.message || "Somto ha rilevato un errore web") :
        type === "comment_review_pending" ? commentReview.body :
        type === "friend_post" ? (preview || "ha pubblicato un post") :
        "Hai una nuova notifica";

      const localizedPresentation = notificationPresentation(type, notif.data || {}, notificationLanguage);
      const title = localizedPresentation.title || fallbackTitle;
      const body = localizedPresentation.body || fallbackBody;
      ...
      if (type === "thread_message" && notif.data?.threadId) {
        ...
        // Il link porta dove e' successa la cosa (post, titolo, thread); se
        // manca, al profilo di chi l'ha fatta.
        url = String(notif.data?.ctaUrl || "").trim()
          || (notif.fromUid ? `/user.html?uid=${encodeURIComponent(String(notif.fromUid))}` : url);
        ...
        url = String(notif.data?.ctaUrl || "/notifications.html");
        ...
        url = String(notif.data?.ctaUrl || "/import.html");
      } else if (type === "push_coverage_report" || type === "import_health_alert" || type === "notification_health_alert") {
        url = String(notif.data?.ctaUrl || "/admin-analytics.html");
      } else if (type === "official_source_report") {
        url = String(notif.data?.ctaUrl || "/admin-official-updates.html");
      } else if (type === "client_error_alert") {
        url = String(notif.data?.ctaUrl || "/admin-analytics.html");
      ...
      for (let i = 0; i < messages.length; i += BATCH_SIZE) {
        ...
        response.responses.forEach((resp, idx) => {
          if (!resp.success) {
            logger.warn(`[push] Token[${i + idx}] failed: code=${resp.error?.code || ""} msg=${resp.error?.message}`);
          }
          responses.push(resp);
        });
      }

      // Si cancella solo un token davvero morto: `invalid-argument` arriva
      // anche per un payload malformato, e li' il device e' sano
      // (lib/pushDelivery.js).
      const sent = summarizeSendResults(responses, tokens);
      successCount = sent.success;

      logger.info(`[push] Results: success=${successCount} failure=${tokens.length - successCount}`);
      await recordDelivery({
        status: successCount > 0 ? PUSH_STATUS.DELIVERED : PUSH_STATUS.FAILED,
        tokens: tokens.length,
        success: sent.success,
        failure: sent.failure,
        deadTokens: sent.deadTokens.length,
        errorCodes: sent.errorCodes,
      });
      ...
      // Pulizia token morti
      if (sent.deadTokens.length) {
        logger.info(`[push] Cleaning ${sent.deadTokens.length} invalid token(s)`);
        const batch = db.batch();
        tokensSnap.docs.forEach(docSnap => {
          if (sent.deadTokens.includes(docSnap.data().token)) batch.delete(docSnap.ref);
        });
        await batch.commit();
      }
  ...
  const notifyOnThreadMessageV2 = firestoreV2.onDocumentCreated(
    ...
    async (event) => {
      ...
      // Campanella admin: un messaggio in una discussione e' conversazione, la
      // cosa che gli analytics non raccontano. Raggruppata a finestra corta.
      if (typeof recordAdminActivity === "function" && !isImportedCommentMessageId(messageId)) {
        void recordAdminActivity({
          kind: "thread_message",
          actorUid: senderUid,
          detail: {
            what: String(msg.text || "").slice(0, 120),
            threadId,
            ctaUrl: `/thread.html?tid=${encodeURIComponent(threadId)}`,
          },
        });
      }
  ...
  // I like ai commenti sotto un voto finiscono in
  // ratingFeed/{eventId}/comments/{commentId}/likes, e nessun trigger li
  // ascoltava (il gemello qui sopra legge solo posts/...): chi riceveva un like
  // su un commento a un voto non lo sapeva mai. Gen2 obbligatorio: i trigger
  // gen1 NUOVI non sono piu' creabili sul progetto (Firestore eur3).
  const notifyOnRatingCommentLike = firestoreV2.onDocumentCreated(
    {
      document: "ratingFeed/{eventId}/comments/{commentId}/likes/{likeUid}",
      region: "europe-west1",
    },
    async (event) => {
      const db = admin.firestore();
      const { eventId, commentId, likeUid } = event.params;
      const parsed = parseRatingEventId(eventId);
      if (!parsed) return null;

      const commentSnap = await db.collection("ratingFeed").doc(eventId)
        .collection("comments").doc(commentId).get();
      if (!commentSnap.exists) return null;
      const ownerUid = String(commentSnap.data()?.uid || "").trim();
      if (!ownerUid || ownerUid === likeUid) return null;
      if (await usersAreBlocked(db, likeUid, ownerUid)) return null;

      const fromName = await getUserDisplayName(db, likeUid);
      // Id deterministico: gen2 consegna almeno una volta, e un retry non deve
      // produrre una seconda notifica per lo stesso like.
      await createInAppNotification({
        toUid: ownerUid,
        fromUid: likeUid,
        type: "comment_like",
        notificationId: notificationDocumentId(["comment_like", eventId, commentId, likeUid]),
        data: {
          fromName,
          eventId,
