...
//   cancelQuizExternalInvite  -> the inviter withdraws an unclaimed invite
...
// Una frase sola per un concetto solo: stessa stringa nel titolo della landing
// e nell'errore della callable di claim, cosi' l'invitato legge sempre questo.
const INVITE_REVOKED_COPY = "Invito annullato";
...
// Precondizioni di annullamento di un invito esterno. Isolate dalla callable
// perche' sono l'unica parte testabile senza emulatore Firestore.
// Lancia HttpsError; non ritorna niente se la sfida e' annullabile.
function assertCancellableExternalChallenge(challenge, uid) {
  const data = challenge && typeof challenge === "object" ? challenge : null;
  if (!data) {
    throw new functions.https.HttpsError("not-found", "Sfida non trovata.");
  }
  if (!uid || data.fromUid !== uid) {
    throw new functions.https.HttpsError(
      "permission-denied", "Non sei il mittente di questa sfida."
    );
  }
  if (data.inviteType !== "external") {
    throw new functions.https.HttpsError("failed-precondition", "Sfida non valida.");
  }
  if (data.status !== "pending_external_signup") {
    // Copy riusata da claimQuizExternalInvite: una sola frase per il concetto.
    throw new functions.https.HttpsError(
...
// `reason` distingue i modi di essere non valido: "revoked" = ritirato dal
// mittente, tutto il resto = scaduto/inesistente. In nessuno dei due casi la
// pagina espone dati dell'invito (ne' nome, ne' numero domande, ne' link).
function renderInvitePage({ valid, inviterName, numQuestions, token, reason }) {
  const name = escapeHtml(inviterName || "Un amico");
  const count = Number(numQuestions) || 10;
  const revoked = !valid && reason === "revoked";
  const heading = valid
    ? `${name} ti ha sfidato su Somto Quiz`
    : (revoked ? INVITE_REVOKED_COPY : "Invito non più disponibile");
  const sub = valid
    ? `Una sfida da ${count} domande su film e serie TV. Scarica Somto per giocare, oppure continua nel browser.`
    : (revoked
      ? "Chi ti ha invitato ha annullato la sfida. Chiedi al tuo amico di inviartene uno nuovo."
      : "Questo invito è scaduto o non è valido. Chiedi al tuo amico di inviartene uno nuovo.");
  // La sfida e' giocabile anche sul web: /quiz-invite.html legge ?token=,
  // chiede il login se serve e chiama claimQuizExternalInvite. Senza questo
  // link la pagina era irraggiungibile, perche' l'hosting manda tutto
  // /quiz/invite/** qui. Il token e' gia' sanificato a monte (safeString) ed
  // e' base64url, ma passa comunque da encodeURIComponent + escapeHtml.
  const webPlayUrl = `${SITE_URL}/quiz-invite.html?token=${escapeHtml(encodeURIComponent(token || ""))}`;
  return `<!doctype html>
<html lang="it">
<head>
  <meta charset="utf-8">
...
<body>
  <div class="card">
    ...
    ${valid ? `<a class="cta is-secondary" href="${webPlayUrl}">Continua nel browser</a>` : ""}
...
function registerQuizInvite(exports) {
  ...
  exports.createQuizExternalInvite = functions
    ...
    .https.onCall(async (data, context) => {
      ...
      const inviteUrl = `${SITE_URL}/quiz/invite/${token}`;
      ...
      batch.set(challengeRef, {
        ...
        completedAt: null,
        expiresAt,
        // Link condivisibile salvato in chiaro sul doc sfida: senza di questo
        // il mittente vedeva il token una volta sola (nel return della create)
        // e non poteva piu' ricondividerlo. Il doc e' leggibile solo da
        // fromUid e toUid, e al claim il token viene consumato: vederlo non
        // da' nessun potere in piu' al destinatario. In `quizInvites` resta
        // comunque solo l'hash SHA-256.
        inviteUrl,
      });
      batch.set(inviteRef, {
  ...
  exports.claimQuizExternalInvite = functions
    ...
    .https.onCall(async (data, context) => {
      ...
      // Invito ritirato dal mittente: stessa risposta di un invito non valido,
      // nessun dato della sfida esce di qui.
      if (inviteInitial.status === "revoked") {
        throw new functions.https.HttpsError("failed-precondition", INVITE_REVOKED_COPY);
      }
      ...
      const result = await db.runTransaction(async (tx) => {
        ...
        // L'annullamento cancella il doc sfida e marca l'invito revoked nella
        // stessa batch: qui ricontrolliamo dentro la transazione perche' il
        // ritiro puo' arrivare tra la lettura di sopra e questo punto.
        if (!challengeSnap.exists || invite.status === "revoked") {
          throw new functions.https.HttpsError("failed-precondition", INVITE_REVOKED_COPY);
        }
  ...
  // -- cancelQuizExternalInvite ---------------------------------------------
  // Il mittente ritira un invito esterno non ancora accettato.
  // Il doc sfida viene CANCELLATO invece di passare a uno stato nuovo: le
  // build iOS gia' sullo Store decodificano `status` come enum e uno stato
  // sconosciuto le romperebbe. Stesso pattern delle sorelle del modulo
  // (nessun runWith: qui App Check non e' imposto, come in create/claim).
  exports.cancelQuizExternalInvite = functions
    .region(REGION)
    .https.onCall(async (data, context) => {
      const uid = context.auth?.uid;
      if (!uid) {
        throw new functions.https.HttpsError(
          "unauthenticated", "Devi accedere per annullare un invito."
        );
      }
      const db = admin.firestore();
      await enforceCallableRateLimit(db, uid, "cancelQuizExternalInvite", {
        windowSeconds: 20, maxInWindow: 5, dailyMax: 60,
      });
      ...
      assertCancellableExternalChallenge(challengeSnap.data(), uid);

      // Il doc invito si ritrova per `challengeId`, campo gia' scritto dalla
      // create: equality su singolo campo, indicizzata di default, nessun
      // indice composito da aggiungere.
      const inviteSnap = await db.collection("quizInvites")
      ...
      if (!inviteSnap.empty) {
        // Il token resta solo come hash: marcarlo revoked basta a far
        // rispondere "invito non valido" a preview e claim, anche se il link
        // e' gia' stato condiviso in giro.
        batch.update(inviteSnap.docs[0].ref, {
          status: "revoked",
          revokedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
      } else {
        logger.warn("[cancelQuizExternalInvite] invito non trovato", { challengeId });
      }
      await batch.commit();
      ...
      try {
        ...
        if (token) {
          ...
          if (!snap.empty) {
            ...
            // Lo stato revoked basta: cancelQuizExternalInvite lo scrive nella
            // stessa batch in cui cancella la sfida, quindi non serve una
            // seconda lettura su quizChallenges per ogni apertura del link.
        ...
        const html = renderInvitePage({ valid, inviterName, numQuestions, token, reason });
...
module.exports = {
  registerQuizInvite,
  _renderInvitePage: renderInvitePage,
  _assertCancellableExternalChallenge: assertCancellableExternalChallenge,
};
