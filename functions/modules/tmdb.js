...
/**
 * Candidati IN USCITA: i titoli che debuttano nei prossimi giorni.
 *
 * PERCHE' UNA SECONDA CORSIA — `fetchTmdbRecentCandidatesForType` scopre per
 * `vote_count.gte`, che misura il passato: un titolo che deve ancora uscire non
 * ha voti. Misurato il 2026-08-26 su 120 uscite dei 60 giorni successivi: solo
 * il 16% arrivava a 10 voti, e il 75% delle 167 uscite in finestra NON era a
 * catalogo. Senza il doc `titles` non esiste niente a valle — post, card in
 * home, pubblico per affinita', notifica, e nemmeno il feed
 * `/prossime-uscite.json` che alimenta il widget iOS.
 *
 * VOTE_COUNT RESTA — non come cancello (qui escluderebbe l'84% dei candidati)
 * ma come seconda porta: si accetta un titolo se e' abbastanza popolare OPPURE
 * se ha gia' abbastanza voti. Un titolo che arriva alla finestra con voti veri
 * entra anche se la popolarita' e' bassa.
 *
 * LA SOGLIA DI POPOLARITA' SERVE — dei 126 titoli in uscita mancanti solo 23
 * stavano sopra 5. Il resto e' coda lunga che sporcherebbe ricerca e pool dei
 * candidati senza che nessuno la guardi.
 *
 * IL FILTRO PER PROVIDER ITALIANI NON SI PUO' USARE — provato il 2026-08-26:
 * `watch_region=IT&with_watch_providers=...` sulla finestra futura restituisce
 * ZERO risultati, perche' una serie non ancora uscita non ha ancora provider su
 * TMDB. E' lo stesso uovo-e-gallina di `vote_count`.
 *
 * Niente cursore: la finestra e' piccola e si rilegge intera ad ogni run, tanto
 * `enqueueCandidates` a valle scarta quello che esiste gia'.
 */
async function fetchTmdbUpcomingCandidatesForType(type, state = {}, options = {}) {
  const mediaType = type === "tv" ? "tv" : "movie";
  const stateObj = state && typeof state === "object" ? state : {};
  const language = safeString(options.language || "it-IT", 16) || "it-IT";
  const region = safeString(options.region || "IT", 8) || "IT";
  const aheadDays = clamp(Number(options.aheadDays || 45), 7, 120);
  const pages = clamp(Number(options.pages || 3), 1, 10);
  const minPopularity = Math.max(0, Number(options.minPopularity ?? 5));
  const voteCountGte = Math.max(0, Math.floor(Number(options.voteCountGte ?? 10)));
  const nowMs = Number(options.nowMs) || Date.now();

  const day = (offsetDays) => new Date(nowMs + offsetDays * 24 * 60 * 60 * 1000)
    .toISOString().slice(0, 10);
  const from = day(0);
  const to = day(aheadDays);

  const dateParams = mediaType === "tv"
    ? {
      "first_air_date.gte": from,
      "first_air_date.lte": to,
      // Soap, talk show, news e reality: mai utili come consiglio e sono un
      // quinto della finestra (307 titoli -> 246). NON si filtra per lingua o
      // paese: nella stessa coda ci sono i drama coreani, che qui hanno un
      // pubblico misurato.
      without_genres: "10766,10767,10763,10764",
    }
    : {
      "primary_release_date.gte": from,
      "primary_release_date.lte": to,
      region,
      // 2 = uscita limitata, 3 = sala: le date che contano per il pubblico
      // italiano. Senza, entrano le date di festival e i premiere esteri.
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    const rows = safeArray(payload?.results);
    if (!rows.length) break;
    for (const row of rows) {
      if (!row?.poster_path) continue;
      const popular = Number(row?.popularity || 0) >= minPopularity;
      const voted = Number(row?.vote_count || 0) >= voteCountGte;
      if (!popular && !voted) continue;
  ...
  return { candidates, pagesFetched, exhausted: true };
}
...
function buildTmdbTitleDoc(row, posterUrl) {
  ...
  // `genre_ids` arriva dalle /search (numeri), `genres` dai /movie|tv/{id}
  // (oggetti {id, name}). Senza il ramo sugli oggetti finivano in catalogo 632
  // titoli con il genere letterale "[object Object]": invisibili ai filtri e
  // muti per il profilo gusti.
  const genres = safeArray(row?.genre_ids || row?.genres)
    .map((g) => {
      if (typeof g === "number") return `tmdb_${g}`;
      if (g && typeof g === "object") {
        const id = Number(g.id);
        return Number.isFinite(id) && id > 0 ? `tmdb_${id}` : "";
      }
      const text = String(g || "").trim();
      return text.includes("[object") ? "" : text;
    })
    .filter(Boolean);
...
async function upsertTmdbTitle(db, bucket, row, logicalDupCache) {
  try {
    ...
    if (targetDocId && existing.exists && existing.data()?.tmdbSync?.syncDisabled === true) {
    ...
    // `createdAt` SOLO alla creazione (fix 2026-08-26).
    //
    // `buildTmdbTitleDoc` lo mette sempre e la scrittura e' in merge, quindi
    // ogni fusione lo riportava a "adesso" — su titoli creati da utenti veri
    // anni prima. Non e' cosmetico: `titles` si ordina per `createdAt` desc in
    // "aggiunti di recente" (web, iOS e home), quindi ~124 titoli vecchi
    // risalivano in cima tre volte al giorno, uno per ogni run del cron.
...
module.exports = {
  ...
  fetchTmdbUpcomingCandidatesForType,
  tmdbTitleDocId,
