...
function renderTitlePage(docId, t, opts = {}) {
  ...
  // Aggiornamenti editoriali: contenuto fresco sulla scheda + link interni
  // verso /novita/{slug}, che senza questi resterebbero pagine orfane.
  const updates = Array.isArray(opts.updates) ? opts.updates : [];
  const updatesHtml = updates.length
    ? `<div class="t-block">
        <h2 class="t-h2">${updates.length === 1 ? "Ultimo aggiornamento" : "Ultimi aggiornamenti"}</h2>
        <ul class="t-updates">${updates.map((u) => {
      const date = u.publishedIso
        ? new Intl.DateTimeFormat("it-IT", { day: "numeric", month: "long", year: "numeric", timeZone: "Europe/Rome" })
          .format(new Date(u.publishedIso))
        : "";
      return `<li class="t-update-item">
            <a class="t-update-link" href="/novita/${encodeURIComponent(u.slug)}">
              <span class="t-update-title">${escapeHtml(u.title)}</span>
              ${date ? `<span class="t-update-date">${escapeHtml(date)}</span>` : ""}
              ${u.summary ? `<span class="t-update-summary">${escapeHtml(u.summary)}</span>` : ""}
            </a>
          </li>`;
    }).join("")}</ul>
      </div>`
  ...
  // CONTRACT 2 — saga: altri film della stessa collection TMDB, gia'
  // approvati in catalogo. Riusa lo stile card di `t-updates` (nessun CSS
  // nuovo): e' l'unica lista "titoli collegati" gia' presente in pagina.
  const saga = Array.isArray(opts.saga) ? opts.saga : [];
  const sagaHtml = saga.length
    ? `<div class="t-block">
        <h2 class="t-h2">Saga${t.collectionName ? ` · ${escapeHtml(String(t.collectionName))}` : ""}</h2>
        <ul class="t-updates">${saga.map((s) => `<li class="t-update-item">
            <a class="t-update-link" href="${escapeHtml(s.href)}">
              <span class="t-update-title">${escapeHtml(s.name)}</span>
              ${s.year ? `<span class="t-update-date">${escapeHtml(String(s.year))}</span>` : ""}
            </a>
          </li>`).join("")}</ul>
      </div>`
    : "";
...
<head>
  ...
  <style>
    ...
    .t-updates { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 10px; }
    .t-update-item { margin: 0; }
    .t-update-link {
    ...
    a.t-update-link:hover { border-color: var(--text-secondary, #A8A8A8); }
    .t-update-title { font-size: 15px; font-weight: 700; }
    .t-update-date { font-size: 13px; color: var(--text-secondary, #A8A8A8); }
    .t-update-summary { font-size: 14px; line-height: 1.5; color: var(--text-secondary, #A8A8A8); }
...
<body class="lp-body">
  ...
  <footer class="lp-footer">
    ...
    <div class="lp-footer-base">
      <img src="/img/tmdb-logo.svg" alt="The Movie Database" width="110" height="8" style="height:11px;width:auto;vertical-align:middle;margin-right:.45rem;" loading="lazy" decoding="async" />
      Somto usa TMDB e le API di TMDB, ma non è approvato, certificato o altrimenti avallato da TMDB.
    </div>
...
// Gli aggiornamenti editoriali pubblicati su questo titolo. Servono due volte:
// danno alla scheda il contenuto fresco che TMDB non ha, e sono l'unico link
// interno da una pagina indicizzata verso /novita/{slug} (community.html e'
// noindex e monta i suoi link in JavaScript).
async function fetchTitleUpdates(db, titleId, limit = 4) {
  const id = asTrimmedString(titleId, 160);
  if (!id) return [];
  try {
    const snap = await db.collection("officialUpdates")
      .where("linkedTitleIds", "array-contains", id)
    ...
    for (const doc of snap.docs || []) {
      const d = doc.data() || {};
      const slug = asTrimmedString(d.slug || doc.id, 160);
      if (!/^[a-z0-9][a-z0-9-]{0,127}$/.test(slug)) continue;
      let publishedIso = "";
      try {
        const ts = d.publishedAt && typeof d.publishedAt.toDate === "function" ? d.publishedAt.toDate() : null;
        if (ts) publishedIso = ts.toISOString();
      } catch (_) { /* data illeggibile: si omette */ }
      out.push({
        slug,
        title: asTrimmedString(d.title, 160),
        summary: asTrimmedString(d.summary, 200) || asTrimmedString(d.text, 200),
        publishedIso,
      });
    }
    return out.filter((row) => row.title);
  } catch (err) {
    // Indice mancante o query lenta non devono togliere la scheda dal web.
    logger.warn("[titlePage] fetchTitleUpdates error", { titleId: id, err: String(err) });
    return [];
  }
}

// CONTRACT 2 — saga (link ufficiali fra i film di una stessa collection
// TMDB). Stessa query composta che i client interrogano direttamente
// (`titles where collectionId == X and status == "approved" orderBy year
// asc limit 30`, indice in firestore.indexes.json), letta qui via Admin SDK
// per la scheda SSR. Solo i film hanno `collectionId` (vedi
// modules/quizSagas.js): le serie TV restano fuori, non serve controllare
// `type` a parte perche' senza `collectionId` valorizzato la query non
// restituisce nulla.
async function fetchSaga(db, collectionId, excludeTitleId, limit = 30) {
  ...
  try {
    ...
    const out = [];
    for (const doc of snap.docs || []) {
      if (doc.id === excludeTitleId) continue;
      const d = doc.data() || {};
      ...
      out.push({
        ...
        href: canonicalPath(d.type, readSlug(d) || doc.id),
      });
    }
    return out;
  } catch (err) {
    // Indice mancante o query lenta non devono togliere la scheda dal web.
    logger.warn("[titlePage] fetchSaga error", { collectionId: id, err: String(err) });
    return [];
  }
}
  ...
  exports.titlePage = functions
    ...
    .https.onRequest(async (req, res) => {
      try {
        ...
        const [quizIds, updates, saga] = await Promise.all([
          getQuizTitleIds(db),
          fetchTitleUpdates(db, snap.id),
          fetchSaga(db, data.collectionId, snap.id),
        ]);
        const html = renderTitlePage(snap.id, data, { hasQuiz: quizIds.has(snap.id), updates, saga });
