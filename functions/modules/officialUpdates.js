...
function officialSourceIngestionEnabled(env = process.env) {
  const configured = String(env.OFFICIAL_SOURCE_INGESTION_ENABLED || "").trim().toLowerCase();
...
function registerOfficialUpdates({ functions, admin, logger, isAdminCaller, enforceCallableRateLimit, getAdminUids }) {
  ...
  return {
    ...
    // Fonti ufficiali soltanto, output soltanto in bozza. Il primo run crea
    // una baseline senza backfill; ogni run successivo processa al massimo 12
    // URL nuovi e non pubblica mai autonomamente.
    scanOfficialSources: onSchedule(
      {
        schedule: "23 */6 * * *",
        timeZone: "Europe/Rome",
        region: "europe-west1",
      ...
      async () => {
        if (!officialSourceIngestionEnabled()) {
          logger.info("[officialSources] scanner disabilitato dal kill switch");
          return null;
        }
        const report = await runOfficialSourceIngestion({ db: admin.firestore(), admin });
        logger.info("[officialSources] scanner run", {
    ...
    reportOfficialSourceCoverage: onSchedule(
      {
        schedule: "15 10 * * *",
        timeZone: "Europe/Rome",
        region: "europe-west1",
      ...
      async () => {
        if (!officialSourceIngestionEnabled()) return null;
        const report = await reportOfficialSourceIngestion({
          db: admin.firestore(),
          admin,
          getAdminUids,
        });
        logger.info("[officialSources] coverage report", report);
        return null;
      }
    ),
...
module.exports = { officialSourceIngestionEnabled, registerOfficialUpdates };
