...
test("legacy unverified login queues one non-blocking verification prompt", async () => {
  const [login, appShell] = await Promise.all([
    read("public/js/pages/login.page.js"),
    read("public/js/components/appShell.js"),
  ]);

  assert.match(login, /queueLegacyVerificationPrompt\(user\)/);
  assert.match(login, /sessionStorage\.setItem\(LEGACY_VERIFICATION_PROMPT_KEY/);
  assert.match(appShell, /showLegacyVerificationPromptIfQueued\(user\.uid\)/);
  assert.match(appShell, /sessionStorage\.removeItem\(LEGACY_VERIFICATION_PROMPT_KEY\)/);
  assert.match(appShell, /Verifica l'email appena puoi per proteggere il tuo account\./);
});
