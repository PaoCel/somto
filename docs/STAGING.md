...
## Piano QA signup verificato (gated)

Il codice e i test locali sono pronti, ma al 2026-08-23 non e stato fatto
alcun deploy staging per questo piano. La lettura cloud non mutante ha trovato:

- IAM `auditConfigs: []`: Firestore `DATA_READ` non e abilitato;
- bucket Logging `_Default` globale, 30 giorni, unlocked e attivo;
- bucket `_Required` globale, 400 giorni, locked e attivo;
- sink `_Default`/`_Required` standard, nessuna exclusion;
- config Auth v2 non leggibile con le credenziali correnti (`403`), quindi
  subtype Identity Platform e `client.permissions` non vanno dedotti.

Dopo autorizzazione al deploy/config staging, eseguire in quest'ordine:

1. snapshot IAM/Auth/App Check/bucket/sink e piano rollback;
2. deploy mirato delle nuove Functions con enforcement signup ancora off;
3. deploy hosting web compatibile; verificare password pending, resend,
   cross-device e completion idempotente;
4. test iOS su simulatore/device con account QA dedicato;
5. dry-run claim legacy e scheduler, senza `--execute` e senza abilitare il job;
6. abilitare `DATA_READ` Firestore solo dopo approvazione costo/privacy; provare
   GetDocument, BatchGetDocuments, RunQuery e Listen e misurare volume;
7. impostare `disabledUserDeletion` solo dopo smoke di `deleteMyAccount` e
   verifica che signup resti consentito (`disabledUserSignup=false`);
8. osservare App Check e client adoption prima di qualunque enforcement Rules.

Account, email e UID QA restano nel password manager. Non usare account reali e
non eseguire test locali iOS contro produzione.
