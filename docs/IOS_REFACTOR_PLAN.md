...
- **DerivedData dedicata** in `ios/build/ci-derived-data` (già in `.gitignore`),
  ...
  **Aggiornamento 2026-08-29 — sull'SSD attuale funziona, e serve una terza
  variabile.** Con il disco interno a 136 MiB liberi, gate `--fast`, test,
  archive Release e upload sono passati tutti su un'immagine APFS montata da
  `/Volumes/SomtoBuild`, in tempi utilizzabili: la conclusione "non conviene"
  sopra vale per il disco misurato nell'agosto 2026, non per questo. Rimisurare
  con `dd` prima di ripetere l'una o l'altra affermazione.

  Spostare `SOMTO_IOS_DERIVED_DATA` e `SOMTO_IOS_BUILD_DIR` **non basta**:
  swift-frontend e swbuild scrivono i temporanei in `/private/var/folders`, che
  resta sul volume interno, e da soli lo riempiono. L'errore che ne esce non
  nomina il disco (`The file "swbuild.tmp.XXXX" couldn't be saved in the folder
  "arm64"`). Da qui i due script spostano `TMPDIR` insieme alla DerivedData:
  automatico quando si usano quelle variabili, `SOMTO_IOS_TMPDIR` per
  sovrascriverlo.
