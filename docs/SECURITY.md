...
Non usiamo **custom claims** di Firebase Auth per i permessi admin. Il modello
admin resta basato su un campo documento:
...
I custom claim sono ora usati soltanto per la compatibilita temporanea del
lifecycle signup (`legacyEmailUnverified`). Lo script di migrazione preserva
l'intera mappa preesistente, non assegna il ruolo admin e richiede manifest per
il rollback. `storage.rules` conserva un ramo legacy per il delete di
`peopleAvatars/{personId}/{fileName}` che controlla
`request.auth.token.admin == true`: non e la fonte di verita del ruolo Somto e
resta fail-closed per gli admin basati sul documento. Da allineare quando si
tocca quel path.
...
## 3.1 Lifecycle signup preparato (non ancora deployato)

Il codice su `main` separa la creazione Auth password dal provisioning Somto:

- `registerPendingSignup` e `completeVerifiedSignup` richiedono Auth e App
  Check; la seconda rilegge Auth con Admin SDK e accetta solo provider
  approvati con email verificata;
- `pendingSignups/{uid}`, `users/{uid}/_system/signupAttribution` e
  `accountDeletionRequests/{uid}` sono deny-all a qualunque SDK client;
- attribution conserva solo superficie/versione, path interno, hostname,
  UTM minimizzate, lingua, consenso analytics e fonte auto-dichiarata;
- il rate limit guest usa un hash dell'IP, mai l'IP grezzo;
- `reconcileUserDataOnAuthDelete` rende idempotente il cleanup anche per delete
  Auth diretti; il bulk `deleteUsers` non va usato perche non emette `onDelete`;
- lo scheduler pending e disabilitato e dry-run per default, con cutoff 72 ore,
  esclusioni e circuit breaker;
- la compatibilita `legacyEmailUnverified` e preparata ma non applicata ad
  utenti reali; il gate Rules `isVerifiedOrLegacy` non e ancora attivo.

Stato operativo al 2026-08-23: nessuna nuova Function/Rule/flag e stata
deployata da questo piano. Deploy, claim, enforcement, scheduler e modifiche
Auth/App Check restano gate espliciti. App Check riduce richieste non attestate,
ma non dimostra che un client web sia umano o non modificato.
...
1. **CSP XSS completa ancora da migrare** (`firebase.json`): dal 2026-08-22 e'
   attiva una baseline enforcing sicura (`object-src 'none'`, `base-uri 'self'`,
   `frame-ancestors 'self'`). La vecchia Report-Only senza endpoint e' stata
   rimossa: produceva solo rumore locale e nessun alert. Per restringere anche
   `script-src`/`style-src` servono nonce/hash sugli inline script delle pagine
   buildless; non aggiungere di nuovo `'unsafe-inline'` spacciandolo per una
   protezione completa.
...
4. **Profilo pubblico troppo ampio**: `users/{uid}` e leggibile integralmente
   dagli utenti autenticati e include presenza, consenso, gusto, ruoli e
   engagement. L'inventario e il rollout reversibile sono in
   `docs/PUBLIC_PRIVATE_PROFILE_MIGRATION.md`; schema e tightening sono un
   progetto separato e gated.
...
- **Errori strutturati**: nei log signup/metriche usare un codice bounded, non
  `error.message`, stack, email, referrer completo o IP grezzo.
...
- `docs/SIGNUP_SECURITY_AND_OBSERVABILITY_PLAN.md` — piano signup e gate.
- `docs/PUBLIC_PRIVATE_PROFILE_MIGRATION.md` — inventario e proposta split profili.
