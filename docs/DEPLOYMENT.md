...
Ogni comando `deploy:*` esegue prima `node scripts/check-deploy-safety.mjs <staging|prod>` e
`npm run verify:hosting` (unit web, grafo ES module completo, public-readiness), poi lancia
`firebase deploy` con `--project` risolto da `scripts/print-deploy-project.mjs`. Dopo il rilascio
esegue lo smoke remoto sull'hosting realmente servito (`smoke:prod` / `smoke:staging`): il comando
resta fallito se pagine, security header o grafo dei moduli remoti non sono validi.
...
7. Non bypassare i comandi canonici con `firebase deploy --only hosting`: salteresti sia il gate
   pre-deploy sia lo smoke del dominio live. Il deploy manuale resta solo break-glass e va seguito da
   `npm run smoke:prod`.

## Rollout signup verificato (ordine obbligatorio)

Questo rollout non e stato eseguito al 2026-08-23. Ogni deploy staging/prod,
claim reale, TestFlight e modifica cloud richiede conferma esplicita.

1. **Snapshot e baseline**: salvare Functions/Rules live, IAM audit config,
   Auth config, App Check, bucket/sink/retention e conteggi lifecycle. Verificare
   i dry-run senza mutazioni.
2. **Functions compatibili**: deploy mirato di
   `registerPendingSignup`, `completeVerifiedSignup`, `getSignupState`,
   `reconcileUserDataOnAuthDelete`, dashboard analytics e scheduler. Mantenere
   `SIGNUP_VERIFICATION_ENFORCED=false` e
   `PENDING_SIGNUP_CLEANUP_ENABLED=false`; il job resta dry-run.
3. **Web compatibile**: deploy hosting e smoke email/password, Google, reset
   password, cambio account e cancellazione. `signup_completed` deve seguire il
   successo server, non la sola creazione Auth.
4. **iOS compatibile**: build simulatore + device, VoiceOver/Dynamic Type,
   offline/cross-device, poi TestFlight solo con autorizzazione. Osservare
   adozione della versione minima concordata.
5. **Claim legacy**: dry-run aggiornato, revisione admin, apply in batch <=50 con
   manifest esterno e rollback testato. Questo passaggio modifica utenti reali
   e resta gated.
6. **Provisioning nuovi account**: dopo adozione client, attivare la flag che
   impedisce al trigger Auth di creare profili password non verificati. Smoke
   social provider e provisioning idempotente.
7. **Rules**: deployare l'helper `isVerifiedOrLegacy` per slice secondo
   `docs/SIGNUP_VERIFICATION_ENFORCEMENT_MATRIX.md`, mai tutto insieme. Suite
   emulator verde e rollback Rules gia provato.
8. **Scheduler**: partire in staging dry-run, poi enable con cap/circuit breaker;
   produzione solo dopo baseline. I delete sono individuali per preservare
   `auth.user().onDelete`.
9. **Cloud hardening**: `DATA_READ`, log access/retention, activity logging Auth,
   `disabledUserDeletion`, Identity Platform e DNS/template sono gate separati;
   non inserirli implicitamente in un deploy Firebase.

Rollback per slice:

- flag provisioning off ripristina il trigger compatibile;
- Rules al ruleset precedente mantengono il claim legacy;
- scheduler disabled interrompe nuove scadenze, senza rollback delle delete gia
  concluse;
- manifest ripristina esattamente i custom claim precedenti;
- hosting rollback mantiene Functions backward-compatible;
- `disabledUserDeletion` torna al valore snapshot con la stessa update mask;
- IAM/log config torna allo snapshot preservando binding ed etag.
