...
- **Bucket `countries`** — vuoto su 205 profili su 205 fino al 2026-08-26:
  `extractTitleFeatures` cercava il paese in `countries`/`originCountry`/
  `countryCode` top-level, campi mai popolati. Il paese vive in
  `meta.originCountry`. Corretto l'estrattore + backfill dei profili esistenti
  (`functions/scripts/backfill-taste-countries.js`). Attenzione: `scoreTasteBias`
  **non** legge il bucket, lo usa solo `rankTasteAudience` come scostamento dalla
  media di popolazione (`provenanceLift`) — vedi `docs/EDITORIAL_SYSTEM.md`.
  Il Match e' volutamente rimasto fuori: il perimetro del benchmark non cambia.
- **Bucket `people` e `directors`** — erano poveri non per il modello ma per i
  dati: `castIds` stava sul 16% del catalogo e `directorIds` sul 7,2%, perche' li
  riempie `enrichTitleAssets` alla prima apertura della scheda. Sistemati il
  2026-08-26 in due passi: `backfill-title-cast.js` su tutto il catalogo
  (`--scope library|rest|all`; cast dal 16% al **94,2%**, registi dal 7,2% al
  **67,7%** su 21.134 titoli) e `backfill-taste-people.js` sui profili
  (persone 34.721 → 41.731, registi 9.055 → 19.610 su 152 profili).
  Il re-fold NON sostituisce: **fonde** per id, perche' 64 profili sarebbero
  rimasti a zero — il loro contenuto veniva da segnali cancellati e non
  ricostruibili. Tre sorgenti in ordine di precedenza: segnali superstiti,
  voti (`ratings`, qualunque source), import senza voto. Un titolo marcato visto
  in app senza voto non scriveva un segnale e non entra. Effetto sul deck Match
  misurato prima di scrivere: sovrapposizione 9,1/10 nei primi dieci, 49% dei
  profili con almeno un cambio. Rollback col file in ~/somto-taste-backups/.
