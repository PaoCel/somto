...
| `users/{userId}/_system/signupAttribution` | Provenienza signup minimizzata (surface/versione, path interno, hostname, UTM, lingua/consenso, fonte dichiarata) | **deny-all client** | **server-only** | valore client non forense; creato al provisioning verificato |
...
| `pendingSignups/{userId}` | Stato transitorio signup password (`pending/completed`, retention proposta 72h) | **deny-all** | **deny-all** | callable Admin SDK; niente password o copia email |
| `accountDeletionRequests/{userId}` | Audit trail minimo cleanup (`source/status/attempt/count`) | **deny-all** | **deny-all** | self-service, Auth onDelete o pending-expiry; nessun client, incluso admin, lo legge |
...
| `titleUpdateEvents/{eventId}` | Timeline persistente trailer/teaser/uscite/episodi | pubblico solo se `status=published` | **deny-all** | writer/scanner Admin SDK; id deterministico, URL HTTPS allow-listed, backfill mai notificabile; `linkedTitleIds[]` (titleId + film della stessa saga, cap 12) alimenta anche il fanout notifiche verso chi segue un altro titolo collegato — vedi `docs/context/NOTIFICATIONS.md` |
| `titleRelatedSuggestions/{uid}__{titleId}__{suggestedTitleId}` | Suggerimento community "questi titoli sono collegati" | owner o admin | client (create, `status` forzato a `pending`); update/delete solo admin | auto-approvazione (`titles.related` arrayUnion su entrambi) quando la stessa coppia raccoglie abbastanza proponenti distinti — trigger gen2 `onTitleRelatedSuggestionCreated`, soglia in `functions/lib/relatedSuggestions.js` |
...
Esistono nel codice (scritte da Cloud Functions via Admin SDK, che bypassa le rules) ma **non compaiono affatto in `firestore.rules`**: sono comunque "nelle rules" nel senso che il catch-all finale `match /{document=**} { allow read, write: if false; }` le nega totalmente ai client. Nessun client (nemmeno admin via SDK client-side) può leggerle o scriverle; solo Admin SDK server-side. `pendingSignups` e `accountDeletionRequests` non sono piu in questo gruppo: hanno un deny-all esplicito.
...
#### 3.1.1 Signup verificato e cancellazione

Il codice preparato su `main` usa `pendingSignups/{uid}` come stato transitorio
server-only per gli account password non ancora verificati. La callable
`completeVerifiedSignup` rilegge Firebase Auth, crea in modo idempotente
`users`, `usersPrivate`, username, attribution e support thread, poi marca il
pending `completed`. La notifica admin e la metrica di completamento nascono
solo dopo questa transizione.

`reconcileUserDataOnAuthDelete` copre anche cancellazioni avvenute fuori da
`deleteMyAccount`. Il cleanup usa una lease in
`accountDeletionRequests/{uid}`, rimuove dati personali, relazioni, file e
artefatti automatici e conserva soltanto il record minimo di esito. Le fonti
sono `self-serve`, `auth-on-delete`, `pending-expiry` o `ops-cleanup`; retry e
concorrenza non devono duplicare il lavoro. Lo scheduler pending e presente ma
disabilitato/dry-run di default e non e attivo finche non supera il gate.

Il root `users/{uid}` resta oggi troppo ampio per una directory. La
classificazione campo-per-campo e il futuro schema `publicUserProfile/{uid}`
sono in `docs/PUBLIC_PRIVATE_PROFILE_MIGRATION.md`; non sono ancora attivi.
...
- **`collectionId`** (int, solo film) — id della collection TMDB, gia' usato da `modules/quizSagas.js` per raggruppare le saghe derivate. Query pubblica "saga": `titles where collectionId == X and status == "approved" orderBy year asc limit 30` (indice dedicato, vedi §5). Alimenta anche `titleUpdateEvents.linkedTitleIds` (§3.x sotto) e `related` (arrayUnion da `titleRelatedSuggestions`, community).
...
### 3.6.2 `titleRelatedSuggestions/{uid}__{titleId}__{suggestedTitleId}` — titoli collegati (community)

Id vincolato nella rule stessa, come `titleEmotions`. Create solo con `status: "pending"` e `createdAt == request.time`; `titleId != suggestedTitleId`; `hasOnly` chiude i campi a `uid/titleId/suggestedTitleId/status/createdAt` (niente `reviewedAt`/`reviewedBy` dal client). Read: solo il proponente o un admin — chi ha proposto cosa non è enumerabile pubblicamente. Update/delete: solo admin.

Il trigger gen2 `onTitleRelatedSuggestionCreated` (`europe-west1`) conta i proponenti **distinti** per la stessa coppia non ordinata (A→B e B→A contano uguale — `unorderedPairKey`, `functions/lib/relatedSuggestions.js`) fra `status in [pending, approved]`; alla soglia (`RELATED_SUGGESTION_AUTO_APPROVE_THRESHOLD`, default 3) collega i due titoli con `titles.related: arrayUnion` **solo se entrambi esistono e sono `approved`**, e marca i suggerimenti pending come `approved`/`reviewedBy:"auto"`. La callable admin `reviewTitleRelatedSuggestion({id, decision})` applica la stessa logica di collegamento per approve/reject manuale.
...
| `cleanupExpiredPendingSignups` | `17 3 * * *` (Europe/Rome) | **disabilitata e dry-run per default**; pianifica account password non verificati >72h, salta legacy/admin/esclusi e apre circuit breaker prima di delete individuali |

### 3.13 Funzioni callable per area (Firestore-facing)

| Area | Callable |
|---|---|
| Signup/Auth | `registerPendingSignup`, `completeVerifiedSignup`, `getSignupState`, `deleteMyAccount` |
...
- `titleRelatedSuggestions/{suggestionId}` = `<uid>__<titleId>__<suggestedTitleId>` (vincolato nella rule stessa)
...
**47 indici totali** (44 `COLLECTION`, 3 `COLLECTION_GROUP`: `imports`, `messages`, `quizStats`) + **14 `fieldOverrides`**.

| Collection | # indici | Campi |
|---|---|---|
| `titleRelatedSuggestions` | 2 | `titleId`+`status`+`createdAt`↓ (elenco per titolo); `status`+`createdAt`↓ (coda admin) — la query di conteggio del trigger (`titleId`+`suggestedTitleId`+`status in [...]`) e' solo uguaglianze, index merging automatico, nessun indice dedicato |
| `titles` | 10 | `genres`(CONTAINS)+`ratingCount`↓; `status`+`castIds`(CONTAINS); `status`+`directorIds`(CONTAINS); `search.tokens`(CONTAINS)+`ratingCount`↓; `status`+`createdAt`↓; `status`+`nameLower`; `status`+`tmdbId`; `status`+`meta.tmdbId`; `status`+`ratingCount`↓; `collectionId`+`status`+`year` (query pubblica saga) |
...
| `titleUpdateEvents` | 6 | 4 gia' esistenti su `titleId`/`eventType`+`status`(+`episode`)+`effectiveAt`/`sortAt` (§3.12); + `linkedTitleIds`(CONTAINS)+`status`+`sortAt`↓ e `linkedTitleIds`(CONTAINS)+`status`+`effectiveAt` (fanout/listing sui titoli collegati alla stessa saga) |
