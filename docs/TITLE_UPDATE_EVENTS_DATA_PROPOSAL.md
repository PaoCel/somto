...
## Addendum 2026-09-03 — `linkedTitleIds` (saga)

Aggiunto un campo array `linkedTitleIds` (`titleId` sempre primo + film
approvati della stessa `titles.collectionId`, cap 12), scritto da
`writeTitleUpdateEvent` in ogni path di creazione/upsert (scanner, writer
trailer, script editoriale) — `titleId` scalare resta invariato per
compatibilità. Nessuna modifica alle Rules: la lettura pubblica (`status ==
"published"`) copre anche il nuovo campo. Due indici nuovi
(`linkedTitleIds` CONTAINS + `status` + `sortAt`/`effectiveAt`, mirror dei 2
già esistenti su `titleId`). Backfill dello storico:
`scripts/backfill-title-update-linked-ids.js` (dry-run di default, `--expand`
per calcolare le saghe sui film esistenti). Il fanout notifiche usa il campo
per raggiungere anche chi segue un altro titolo della stessa saga — dettagli
in `docs/context/NOTIFICATIONS.md` ("Fanout collegato (saga)").
