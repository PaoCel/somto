...
## Pending residui
- **Stagione Netflix sulle serie multi-stagione: 4.605 righe ancora senza.**
  L'export Netflix marca la stagione in tre modi — `Stagione N`, `<Nome serie> N`
  (Stranger Things) e per niente. I primi due sono coperti dal parser
  (`netflixCsv.js`); del terzo si recupera solo il caso mono-stagione, dove la
  stagione e' per forza la 1 (`writeTitleStates.js`). Restano **4.605 righe su
  serie multi-stagione** (JoJo 344, Shadowhunters 224, L'attacco dei giganti 176,
  Friends 168) dove Netflix scrive solo il nome dell'episodio: li' la stagione si
  potrebbe ricavare solo risolvendo quel NOME contro gli episodi TMDB della serie,
  una chiamata per stagione piu' un match fuzzy sui titoli tradotti.
  **Cosa NON e' rotto**: il conteggio episodi e i minuti sono giusti lo stesso —
  `writeTitleStates` conta le chiavi distinte sul nome episodio. L'effetto e'
  solo `seasonsCompletedCount` a 0 e il badge del profilo senza stagione.
  Misurato il 2026-09-08 su 19 import Netflix.
- **259 serie TV senza runtime episodio su TMDB.**
  `repair-tv-episode-runtime.js` le salta: TMDB non espone il runtime di nessun
  loro episodio, quindi restano col valore che avevano (spesso quello di un solo
  episodio, vedi `docs/DECISIONS.md` 2026-09-08). Nessuna fonte alternativa in
  casa; si sistemano da sole se TMDB si popola.
- **Digest settimanale: LIVE dal 2026-08-25, primo invio giovedì 27 alle 18.**
  `sendWeeklyDigest` (giovedì 18:00 Europe/Rome) manda **una** notifica a
  settimana con le novità sui titoli che hai in libreria o watchlist: massimo
  3 righe, un titolo per riga, id notifica sulla settimana ISO (un run ripetuto
  non duplica). Logica pura in `functions/lib/weeklyDigest.js`, anteprima senza
  scritture con `functions/scripts/preview-weekly-digest.js`.
  **Al posto delle notifiche per-episodio**: erano 553, lette al 2%. Spente in
  **tre** punti, perché uno solo non bastava: il flag
  `TITLE_UPDATE_DUE_SWEEP_ENABLED` (assente = off), il job scheduler di
  `notifyDueTitleUpdates` in **europe-west1** messo in PAUSED, e — scoperto in
  prod dopo la pausa — il ramo `new_episode` del trigger
  `notifyOnTitleUpdatePublished`, che notificava lo stesso gli episodi
  **usciti in giornata** senza passare dalla sweep. Il fan-out immediato degli altri tipi di evento (trailer, rinnovi, date)
  resta acceso: quello sta su `TITLE_UPDATE_NOTIFICATIONS_ENABLED`.
  **Eccezione dal 2026-09-10**: le premiere di stagione (episodio 1) hanno
  la loro push il giorno dell'uscita, quindi il job della sweep delle 9 e' di
  nuovo ENABLED e lavora solo su quelle (`premieresOnly`).
  Dry-run del 25/08: **108 persone su 350**, 237 novità, 55 digest da 3 righe.
  **Da guardare il 3 settembre** (dopo il secondo invio): tasso di lettura
  contro il gate del **15%** e quanti riaprono entro 48 ore. Sotto il 15% il
  canale non si ritara, si spegne — vedi `docs/GROWTH_PLAN_2026_08.md`.
  **Debiti noti**: (1) il run legge ~880 documenti per utente (stati,
  preferenze e notifiche già inviate per ogni titolo in finestra): a 350 utenti
  sono ~280k letture a settimana, ~0,17 $, ma non scala oltre qualche migliaio
  di utenti e andrà girato per evento invece che per persona; (2) se il run va
  in timeout a metà, basta rilanciarlo — l'id deterministico salta chi ha già
  ricevuto; (3) la resa iOS dedicata esce con la prossima build, prima di
  allora la campanella usa il campo `message` in chiaro.
- **Notifiche di re-engagement: da riguardare fra due settimane (2026-08-24).**
  I tre cooldown engagement (`sendInactivityNudges`, `sendWatchlistReminders`,
  `sendFriendActivityDigest`) non hanno mai frenato niente: erano scritti con
  `batch.set()` e chiavi puntate (`"engagement.lastNudgeAt"`), che in `set()`
  diventano un **campo letterale** e non un percorso, quindi la lettura
  annidata trovava sempre `undefined`. Effetto: 11.116 `engagement_nudge`
  (letti allo **0,5%**) e 2.780 promemoria watchlist (2,1%), circa uno al
  giorno a testa. Fix deployato: scrittura annidata, lettura con fallback sul
  campo letterale (`engagementMillis`/`engagementNumber` in `lib/pureUtils.js`),
  campo vecchio cancellato nello stesso write, cooldown a 14 giorni (watchlist
  21) e **stop dopo 4 richiami senza rientro**. Primo run reale dopo il deploy:
  `sent=1 skippedExhausted=217 scanned=220` (prima ~63 al giorno).
  **Da verificare fra due settimane**: che il volume resti basso e che il tasso
  di lettura salga; se resta sotto l'1% il canale va ripensato, non ritarato.
- **Pubblico per affinità: alzato, ora parla anche in notifica (2026-08-24).**
  `DEFAULT_TASTE_AUDIENCE_LIMIT` da 25 a 75, e i primi 10 sopra `0,5` di
  punteggio ricevono anche la notifica (prima l'affinità finiva solo nel feed).
  Soglia misurata sui 205 profili veri: ~155 candidati per titolo, massimo
  ~1,5, mediana 0,13, quindi 0,5 seleziona il 5% più affine.
  **Resta aperto**: nella console admin il pubblico per affinità è ancora
  **spento di default** (`tasteAudienceLimit: 0`), quindi ne beneficiano solo i
  post automatici sulle uscite. Accenderlo di default è una scelta editoriale,
  non tecnica.
- **Portata degli aggiornamenti ufficiali: avviso, non blocco (2026-08-24).**
  16 post su 62 erano usciti con audience e feed a zero. Ora l'anteprima
  (`dryRun`) restituisce due avvisi: `audience_empty` e `audience_tiny` (meno di
  5 destinatari in notifica). **Non bloccano di proposito**: un aggiornamento
  che arriva a due persone che hanno quella serie in libreria è la notifica più
  pertinente che Somto sappia mandare. **Da fare**: mostrarli nella console
  `/admin-official-updates.html`, che oggi legge `warnings` ma va verificata su
  questi due codici nuovi.
- **Backup Firestore: pronto ma SPENTO per costo (aperto 2026-08-24).** Fino a
  oggi `scheduledFirestoreExport` falliva ogni notte (`7 PERMISSION_DENIED`):
  il bucket non esisteva e il service account gen1 `gia-visto@appspot` non
  aveva `roles/datastore.importExportAdmin`. Sistemati entrambi: bucket
  `gs://gia-visto-backups` (EU, NEARLINE, lifecycle 30 giorni, public access
  bloccato) + ruolo sul progetto + `roles/storage.admin` sul bucket. Un export
  completo e' andato a buon fine: **1,48 GB, 1026 file, ~2,5 minuti**, ed e'
  l'unica copia esistente (si autocancella dopo 30 giorni).
  Poi **spento**, perche' l'export e' fatturato **un read per documento**:
  1.557.396 doc = ~0,93 $ a run, cioe' ~28 $/mese da giornaliero (2x a
  settimana ~8 $, settimanale ~4 $; lo storage e' irrilevante, ~0,45 $/mese per
  30 copie). Interruttore: env `FIRESTORE_BACKUP_ENABLED` (la function esce
  subito se non vale `1`) **piu'** il job scheduler
  `firebase-schedule-scheduledFirestoreExport-europe-west1` (che sta in
  **us-central1**, non in europe-west1) messo in PAUSED. La guardia e' doppia
  perche' `firebase deploy --only functions` riabilita il job da solo.
  Per riaccendere: env a `1`, deploy della function, job su ENABLED.
- **Righe grezze degli import (`items`): si tengono, verificato 2026-08-24.**
  Sono 972.600 doc su 973.106 dell'intero collection group `items` (gli altri
  506 sono i contenuti delle liste utente) e il 63% del database. Sembravano il
  candidato ovvio alla potatura, ma **costano ~0,10 $/mese** di storage (581
  byte medi, ~0,57 GB) e pesavano solo sul backup, che ora e' spento. Soprattutto
  sono **l'unica copia rimasta**: i file caricati vengono cancellati dal
  lifecycle di Storage (`manualImports/` a 7 giorni, `supportImports/` a 14), e
  tutti gli script di riparazione (`repair-netflix-episode-zero`,
  `repair-series-progress-zero`, `repair-import-season-mismatch`,
  `reprocess-tvtime-votes`, `rematch-import-unresolved`, `import-health-scan`)
  leggono da qui. Ripartizione: 856k `completed`, 94,7k `awaiting_confirmation`,
  21k `failed`; 956k arrivano dall'ondata di luglio 2026. Se un giorno il backup
  torna acceso, e' li' che la spesa va rivista (allowlist senza `items` oppure
  potatura dei soli import completati e vecchi), non prima.
...
- **Repository pubblico**: completare `docs/PUBLIC_RELEASE_CHECKLIST.md`, cambiare visibilità e riattivare subito ruleset/branch protection.
...
- **Copertura push: 86% degli utenti non è raggiungibile (aperto, 2026-08-03).**
  ...
  almeno un token / utenti totali". Rimisurata il 2026-09-10: 47 su 372
  (12,6%), 13 dei 32 attivi della settimana; 43 iOS, 4 web.
- **Cooldown sulle uscite: per titolo dal 2026-09-10, da rimisurare.** Il
  cooldown di 1 ora per tipo aveva trattenuto 51 push di uscite su 378 verso
  utenti raggiungibili (13,5%), quasi tutte fra le 9 e le 10. Paolo ha scelto
  la finestra per titolo (scartato l'accorpamento in una push sola). Da
  controllare verso il 2026-09-24 con `pushDelivery` sulle notifiche
  `title_update`/`official_update`: quante push di uscite riceve una persona
  in una mattina, e se la lettura regge.
- **Perche' mancano i token: il dato arriva con la 1.10.1 (2026-09-10).**
  `report-push-permission.js` divide chi non ha le push fra denied, mai
  chiesto e concesso-senza-token, ma lo stato lo riporta solo la 1.10.1,
  ancora su TestFlight: al 10/09 nessun utente l'ha riportato. Serve
  l'uscita su App Store.
...
- **Cast, registi e profili gusti: chiuso il 2026-08-26.**
  `backfill-title-cast.js` ha portato i 15.437 titoli in libreria da cast 16% a
  94,3% e registi da 7,2% a 64,7%; `backfill-taste-people.js` ha rifuso i
  profili (persone +7.010, registi +10.555 su 152 profili). Dettagli e regole in
  `docs/context/TASTE_ENGINE.md`, decisioni in `docs/DECISIONS.md`.
  Esteso poi a tutto il catalogo (`--scope rest`): 5.548 titoli, cast 94,2% e
  registi 67,7% su 21.134. Quelli fuori libreria non entrano in nessun profilo
  ma stanno nel pool dei candidati di Match e consigli, dove un titolo senza
  cast ne' regista non puo' somigliare a niente.
  Quel che resta e' irrecuperabile da TMDB: 6.134 serie senza un regista a
  livello di serie e ~1.200 titoli senza credits.

- **Web: fondere Home e Community come su iOS (2026-09-04).** Su iOS il feed
  e' la seconda meta' della Home (`HomeStreamSection`) e la tab Community non
  esiste piu' (`docs/DECISIONS.md` 2026-09-04). Sul sito `home.html` e
  `community.html` sono ancora due pagine: replicare l'intreccio (post +
  corsie a posizioni fisse), la riga composer e "Pubblica il voto" dopo il voto
  rapido. Finche' non si fa, iOS e web mostrano lo stesso contenuto in due
