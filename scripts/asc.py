...
SOMTO_BUNDLE_ID = "com.paolocelestini.twowatch.ios"


def app_id_for_bundle(cfg, bundle_id=SOMTO_BUNDLE_ID):
    """L'app id di Somto, preso dal bundle che questo repo spedisce davvero.

    PERCHE' NON BASTA `app_id` DEL CONFIG — chiave e issuer valgono per tutto
    l'account Apple, quindi `~/.appstoreconnect/config.json` e' condiviso da
    tutti i progetti di Paolo e il suo `app_id` di primo livello e' solo "su
    quale app puntano gli script adesso". E' una variabile globale mutabile: il
    2026-08-25 una sessione su un altro progetto l'ha spostata sulla propria app
    (annotandolo come una correzione, e dal suo punto di vista lo era), e la
    release di Somto del giorno dopo ha letto numero di build e versione
    pubblica dall'app sbagliata. Non se n'era accorto nessuno perche' i numeri
    letti erano plausibili — semplicemente di un altro prodotto.

    Questo repo sa gia' cosa sta spedendo: `PRODUCT_BUNDLE_IDENTIFIER` in
    `ios/project.yml`. Risolvere da li' rende la release di Somto indipendente
    da cosa fanno gli altri progetti, in entrambe le direzioni.

    Ritorna `None` se il registro `apps` non c'e' o non contiene quel bundle:
    in quel caso si ricade sul default di prima, senza peggiorare nulla.
    `ASC_APP_ID` resta comunque piu' forte — un override esplicito vince su una
    deduzione.
    """
    apps = cfg.get("apps")
    ...
    for name, entry in apps.items():
        if not isinstance(entry, dict):
            continue
        if str(entry.get("bundle_id", "")).strip() == bundle_id:
            app_id = str(entry.get("app_id", "")).strip()
            if not app_id:
                continue
            ambient = str(cfg.get("app_id", "")).strip()
            if ambient and ambient != app_id:
                info(
                    f"app risolta dal bundle {bundle_id}: {name} ({app_id}). "
                    f"Il default del config punta altrove ({ambient}): ignorato."
                )
            return app_id
    return None
...
def load_config():
    ...
    resolved = {
        ...
        "app_id": os.environ.get("ASC_APP_ID") or app_id_for_bundle(cfg) or cfg.get("app_id"),
