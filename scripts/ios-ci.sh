...
# TMPDIR va spostato insieme alla DerivedData, se no non serve a niente
# spostare la DerivedData: swift-frontend e swbuild scrivono i temporanei in
# `/private/var/folders`, che sta sul volume INTERNO qualunque cosa dica
# `-derivedDataPath`. Con la DerivedData sull'SSD il disco interno si riempiva
# lo stesso, e la build moriva con un errore che non nomina il disco:
#   error: The file "swbuild.tmp.XXXX" couldn't be saved in the folder "arm64"
# Default: una `tmp/` accanto alla DerivedData, cioe' sullo stesso volume che
# l'utente ha gia' scelto. `SOMTO_IOS_TMPDIR` per puntarla altrove.
if [ -n "${SOMTO_IOS_TMPDIR:-}" ]; then
  TMPDIR="${SOMTO_IOS_TMPDIR}"
elif [ -n "${SOMTO_IOS_DERIVED_DATA:-}" ]; then
  TMPDIR="$(dirname "${SOMTO_IOS_DERIVED_DATA}")/somto-build-tmp"
fi
if [ -n "${SOMTO_IOS_TMPDIR:-}${SOMTO_IOS_DERIVED_DATA:-}" ]; then
  mkdir -p "${TMPDIR}" 2>/dev/null || true
  export TMPDIR
  printf '\033[33m  TMPDIR: %s\033[0m\n' "${TMPDIR}"
fi
