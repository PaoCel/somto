...
# Stessa regola del gate (`ios-ci.sh`): spostare DerivedData e archive senza
# spostare TMPDIR non basta. I temporanei di swift-frontend e swbuild stanno in
# `/private/var/folders`, cioe' sul volume interno, e un archive Release li
# riempie da solo: la build muore con `The file "swbuild.tmp.XXXX" couldn't be
# saved`, che sembra tutto tranne un disco pieno.
if [[ -n "${SOMTO_IOS_TMPDIR:-}" ]]; then
  TMPDIR="${SOMTO_IOS_TMPDIR}"
elif [[ "$BUILD_DIR" != "$IOS_DIR/build" ]]; then
  TMPDIR="$BUILD_DIR/somto-build-tmp"
elif [[ -n "${SOMTO_IOS_DERIVED_DATA:-}" ]]; then
  TMPDIR="$(dirname "${SOMTO_IOS_DERIVED_DATA}")/somto-build-tmp"
fi
if [[ -n "${SOMTO_IOS_TMPDIR:-}${SOMTO_IOS_DERIVED_DATA:-}" || "$BUILD_DIR" != "$IOS_DIR/build" ]]; then
  mkdir -p "$TMPDIR"
  export TMPDIR
  info "TMPDIR: $TMPDIR"
fi
