...
import { t as i18nT, initI18n, applyStaticTranslations, getLocale } from "../i18n/index.js";
      ...
      <div class="app-shell-drawer-body">
        ...
        <section class="app-shell-section">
          ...
          <div class="app-shell-grid">
            ...
            <a class="app-shell-link app-shell-link-with-sub" href="${getLocale() === "en" ? "/en/sostieni.html" : "/sostieni.html"}">
              <span class="app-shell-link-head">
                <span class="app-shell-icon" aria-hidden="true">${ICONS.coffee}</span>
                <span class="app-shell-link-text"><strong>${i18nT("Offri un caffè")}</strong><small>${i18nT("Sostieni lo sviluppo di Somto")}</small></span>
              </span>
            </a>
          </div>
        </section>
...
function showLegacyVerificationPromptIfQueued(uid) {
  try {
    const raw = sessionStorage.getItem(LEGACY_VERIFICATION_PROMPT_KEY);
    if (!raw) return;
    sessionStorage.removeItem(LEGACY_VERIFICATION_PROMPT_KEY);
    const payload = JSON.parse(raw);
    if (payload?.uid !== uid) return;
    toast(
      i18nT("Verifica l'email appena puoi per proteggere il tuo account."),
      i18nT("Verifica consigliata"),
      { type: "info", timeout: 8_000 }
    );
  } catch (_e) {
    try { sessionStorage.removeItem(LEGACY_VERIFICATION_PROMPT_KEY); } catch (_) {}
  }
}
...
function bindUserData() {
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    showLegacyVerificationPromptIfQueued(user.uid);
