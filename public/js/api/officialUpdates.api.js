...
/**
 * Pagine editoriali che possono firmare un post ufficiale.
 * Sono doc utente sintetici con `accountType: "page"`, creati dallo script
 * `functions/scripts/create-editorial-pages.js`. Se la lettura fallisce la
 * console resta usabile: si pubblica come Somto, che e' il default.
 */
export async function listEditorialPages({ max = 20 } = {}) {
  try {
    ...
    return snap.docs
      .map((docSnap) => ({
        uid: docSnap.id,
        displayName: String(docSnap.data()?.displayName || docSnap.id),
      }))
      .sort((a, b) => a.displayName.localeCompare(b.displayName));
  } catch (err) {
    console.warn("[editoriale] elenco pagine non disponibile", err?.message || err);
    return [];
  }
}
