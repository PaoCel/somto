...
// Rumore di terze parti: script iniettati dalle estensioni del browser e
// promise rifiutate dal motore (view transition annullata, ResizeObserver).
// Non sono bug di Somto e non devono generare l'alert admin.
// Stessa lista, in CommonJS, in `functions/lib/clientErrorNoise.js`:
...
function isIgnorableClientError(info) {
  ...
  // "Script error." cross-origin: nessun source, nessuno stack, zero informazione.
 ...
 * - l'errore arriva da un'estensione del browser o è rumore del motore
...
export function reportClientError(info) {
  try {
    ...
    if (isIgnorableClientError(info)) return;
