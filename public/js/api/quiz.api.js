...
function decodeChallenge(snap) {
  ...
  return {
    ...
    // Scritto dal server alla creazione dell'invito esterno: gli inviti creati
    // prima non ce l'hanno, quindi chi lo usa deve reggere il null.
    inviteUrl: typeof data.inviteUrl === "string" ? data.inviteUrl : null,
    // Presenti solo sulle sfide create su una saga (gruppo curato di titoli).
    sagaId: typeof data.sagaId === "string" ? data.sagaId : null,
    sagaName: typeof data.sagaName === "string" ? data.sagaName : null,
...
/**
 * Restituisce l'elenco completo delle saghe giocabili (es. "Harry Potter") dal
 * doc aggregato `quizMeta/sagas` — UNA sola read, stesso stile e stessa
 * gestione errori di `fetchPlayableThemes`. A differenza dei temi non c'è un
 * fallback a campionamento: una saga è un raggruppamento curato di titleIds
 * che non si può ricostruire da una query live su `quizQuestions`. Se il doc
 * non esiste ancora (backend non deployato) o è vuoto, ritorna lista vuota
 * senza lanciare.
 */
export async function fetchPlayableSagas() {
  try {
    const snap = await getDoc(doc(db, "quizMeta", "sagas"));
    if (snap.exists()) {
      const data = snap.data() || {};
      const list = Array.isArray(data.sagas) ? data.sagas : [];
      return list
        .filter((s) => s && s.sagaId && Array.isArray(s.titleIds) && s.titleIds.length)
        .map((s) => ({
          sagaId: s.sagaId,
          name: s.name || i18nT("Senza titolo"),
          mediaType: s.mediaType || "unknown",
          titleIds: s.titleIds,
          titles: Array.isArray(s.titles) ? s.titles : [],
          questionCount: Number(s.count) || 0,
        }));
    }
  } catch (err) {
    console.warn("[quiz] fetchPlayableSagas aggregate read failed", err);
  }
  return [];
}
...
// è validato da submitGuestQuiz lato server. Nessuna scrittura utente, nessun XP;
// il server conserva solo una sessione effimera deny-all.
// ============================================================

/**
 * Domande per il guest (no answer-key). `{titleId?, sagaId?, count}` →
 * { titleId, title, questions[] } oppure, per una saga, anche
 * { sagaId, sagaName, isSaga: true }. `titleId` e `sagaId` sono alternativi:
 * quando c'è `sagaId` il server risolve i titoli della saga e pesca le
 * domande da tutti quanti.
 */
export async function getGuestQuiz({ titleId = null, sagaId = null, count = 5 } = {}) {
  const fn = httpsCallable(functions, "getGuestQuiz");
  const res = await fn({ titleId: titleId || null, sagaId: sagaId || null, count });
  return res.data;
}

/** Invia la sessione emessa dal server e le risposte del guest. */
export async function submitGuestQuiz(sessionToken, answers = []) {
  const fn = httpsCallable(functions, "submitGuestQuiz");
  const res = await fn({ sessionToken, answers });
  return res.data;
}
 ...
 *   numQuestions?, sharedTitleIds?, sagaId?, sagaName? }
 */
export async function sendChallenge(challenge) {
  ...
  // Solo per le sfide su una saga: `sharedTitleIds` da solo non basta a
  // ricostruire il nome del gruppo, e sulle sfide su titolo i due campi non
  // devono proprio esistere.
  const sagaId = String(challenge.sagaId || "").trim();
  const sagaName = String(challenge.sagaName || "").trim();
  if (sagaId && sagaName) {
    payload.sagaId = sagaId;
    payload.sagaName = sagaName;
  }
...
/**
 * Annulla un invito esterno ancora in attesa tramite la callable
 * `cancelQuizExternalInvite`. Il server cancella il doc sfida (e con esso il
 * token), quindi il link condiviso smette di funzionare: chiamala solo dopo
 * conferma esplicita di chi ha creato l'invito.
 *
 * @param {object} params
 * @param {string} params.challengeId
 * @returns {Promise<{ok:boolean}>}
 */
export async function cancelExternalInvite({ challengeId } = {}) {
  if (!challengeId) throw new Error("challengeId mancante");
  const callable = httpsCallable(functions, "cancelQuizExternalInvite");
  const result = await callable({ challengeId });
  const dict = result?.data;
  return { ok: dict?.ok !== false };
}
