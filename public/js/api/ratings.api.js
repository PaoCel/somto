...
/**
 * Come listMyRatingsAnyLevel, ma piu'-recenti-prima e tagliata a `max`: serve
 * ai default "Con chi l'hai visto?" (companionDefaults.js), che ragionano
 * sulle ultime ~60 review, non su tutto lo storico. Stessa query a indice
 * singolo (nessun orderBy server-side per non richiedere un indice composito
 * in piu'), l'ordinamento per data e' lato client.
 */
export async function listMyRecentRatingsAnyLevel(uid, { max = 60 } = {}) {
  if (!uid) return [];
  const q = query(collection(db, "ratings"), where("uid", "==", uid));
  const snap = await getDocs(q);
  const toMillis = (v) => {
