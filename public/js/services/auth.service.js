...
export async function sendSignupVerification(user = auth.currentUser) {
  if (!user) throw new Error(i18nT("Nessun account autenticato."));
  const origin = typeof window !== "undefined" ? window.location.origin : "https://somto.it";
  await sendEmailVerification(user, { url: `${origin}/login.html?verify=1` });
}

export async function refreshSignupVerification(user = auth.currentUser) {
  if (!user) throw new Error(i18nT("Nessun account autenticato."));
  await reload(user);
...
export async function changePendingSignupEmail(email, user = auth.currentUser) {
  if (!user) throw new Error(i18nT("Nessun account autenticato."));
  await updateEmail(user, String(email || "").trim());
