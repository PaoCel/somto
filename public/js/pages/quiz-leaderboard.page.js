...
const state = {
  ...
  myRank: null,
  myScore: 0,
  status: "loading",      // loading | ready | empty | error | signedOut
  errorMessage: "",
};

const RING_BY_RANK = ["var(--quiz-gold)", "var(--quiz-silver)", "var(--quiz-bronze)"];
const LOGIN_HREF = "/login.html?next=%2Fquiz-leaderboard.html";

/** La classifica legge `collectionGroup("quizStats")`, che le rules aprono solo
 *  a `isSignedIn()`: da sloggato Firestore risponde permission-denied. */
...
function renderScopePicker() {
  ...
  return `
    <div class="quiz-segmented" role="tablist" aria-label="${i18nT("Periodo classifica")}">
      ${opts.map((o) => `
...
function podiumColumn(entry, rank) {
  ...
  return `
    <div class="quiz-podium-col${rank === 1 ? " is-first" : ""}">
      ...
      ${isMe ? `<div>${statusPill(i18nT("Tu"), { tint: "var(--brand-primary)", filled: true })}</div>` : ""}
    ...
    <div class="quiz-lb-row${isMe ? " is-me" : ""}">
      ...
      <div class="quiz-lb-info">
        <div class="quiz-lb-nameline">
          ...
          ${isMe ? statusPill(i18nT("Tu"), { tint: "var(--brand-primary)", filled: true }) : ""}
        </div>
        <span class="quiz-lb-sub">${i18nT("{count} partite", { count: entry.attemptsCount || 0 })}</span>
      </div>
  ...
  return `
    <div class="quiz-myrank-bar">
      <div class="quiz-myrank-inner">
        ...
        <span class="quiz-myrank-score">${i18nT("{count} punti", { count: formatScore(state.myScore) })}</span>
      </div>
    </div>`;
}
...
function render() {
  if (state.status === "signedOut") {
    // Niente errore Firestore grezzo a schermo: il picker Settimanale/All time
    // non serve, qui l'unica azione utile è accedere.
    root.innerHTML = emptyState({
      icon: "trophy",
      title: i18nT("Accedi per vedere la classifica"),
      actionLabel: i18nT("Accedi"),
      actionId: "quizLeaderboardLogin",
    });
    document.getElementById("quizLeaderboardLogin")
      ?.addEventListener("click", () => { window.location.href = LOGIN_HREF; });
    return;
  }
  ...
  root.innerHTML = `
    ...
    ${state.scope === "weekly"
      ? `<div class="quiz-lb-footer">${ICONS.clock}<span>${i18nT("Classifica della settimana in corso")}</span></div>`
      : ""}
    ${renderMyRankBar()}`;
  bindScope();
}
...
async function load() {
  ...
  try {
    ...
    if (isPermissionDenied(err)) {
      // Sessione scaduta o token non ancora pronto: stesso invito al login.
      state.status = "signedOut";
    } else {
      state.status = "error";
