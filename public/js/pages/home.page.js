...
import { getTitlesByIds, listPopularTitles, listTitlesByTmdbIds } from "../api/titles.api.js";
...
async function renderHome(myUid) {
  try {
    const [suggestion, blogPosts, popular, myStates, trending, matchPayload, upcoming] = await Promise.all([
    ...
    // "Per te": i titoli ordinati per gusto, **senza filtro di piattaforma**.
    //
    // Sono gli stessi `items` che `getMatchQueue` restituisce e che la home
    // scaricava gia' ad ogni apertura senza mostrarli: veniva renderizzata solo
    // `providerLane`, cioe' lo stesso elenco ridotto a una piattaforma sola.
    // Quel filtro toglieva buoni suggerimenti per il motivo sbagliato — il
    // valore di Somto e' proprio consigliare a prescindere da chi trasmette.
    const forYouSection = document.getElementById("homeForYouSection");
    const forYouScroll = document.getElementById("homeForYouScroll");
    const forYouItems = Array.isArray(matchPayload?.items) ? matchPayload.items : [];
    if (forYouSection && forYouScroll) {
      if (forYouItems.length) {
        forYouScroll.innerHTML = forYouItems.map(renderTitlePoster).join("");
        forYouSection.style.display = "";
      } else {
        forYouScroll.innerHTML = "";
