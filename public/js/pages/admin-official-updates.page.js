...
const tasteAudienceInput = qs("#tasteAudienceInput");
...
function buildPayload({ dryRun, status }) {
  ...
  return {
    ...
    authorUid: authorSelect?.value || "somto_official",
    // 25 e' il default del modulo: qui si sceglie solo acceso/spento.
    tasteAudienceLimit: tasteAudienceInput?.checked ? 25 : 0,
...
function renderPreviewPanel(result) {
  ...
  // Numeri distinti: chi riceve notifica e push (libreria/follower), chi vede
  // il post nel feed per affinita' e, dentro quest'ultimo gruppo, i pochi piu'
  // affini che dal 2026-08-24 ricevono anche la notifica. Confonderli farebbe
  // sembrare la portata piu' grande di quanto sveglia davvero.
  const taste = Number(result.tasteAudienceCount || 0);
  const tasteNotify = Number(result.tasteNotifyCount || 0);
  const tasteNote = tasteNotify
    ? `${fmtInt(taste)} per affinità (${fmtInt(tasteNotify)} in notifica)`
    : `${fmtInt(taste)} per affinità (solo feed)`;
  previewRecipientCount.textContent = taste
    ? `${fmtInt(result.recipientCount)} utenti + ${tasteNote}`
    : `${fmtInt(result.recipientCount)} utenti`;
...
previewBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    const tasteCount = Number(res?.tasteAudienceCount || 0);
    toast(
      tasteCount
        ? `Raggiungerebbe ${fmtInt(res?.recipientCount)} utenti, piu' ${fmtInt(tasteCount)} nel feed per affinità.`
        : `Raggiungerebbe ${fmtInt(res?.recipientCount)} utenti.`,
      "Anteprima"
    );
  } catch (err) {
    console.error(err);
...
/**
 * Riempie il selettore autore. Somto resta la prima opzione e il default:
 * una pagina si sceglie apposta, non per distrazione.
 */
async function loadAuthors() {
  if (!authorSelect) return;
  const pages = await listEditorialPages();
