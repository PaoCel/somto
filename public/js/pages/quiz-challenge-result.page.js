...
const CATEGORY_LABELS = {
  anagraphic: i18nT("Anagrafica"), character: i18nT("Personaggi"), plot: i18nT("Trama"),
  scene: i18nT("Scene"), relationship: i18nT("Relazioni"), object: i18nT("Oggetti"),
  motivation: i18nT("Motivazioni"), consequence: i18nT("Conseguenze"),
  episode: i18nT("Episodi"), quotePAraphrase: i18nT("Citazioni"),
  chronology: i18nT("Cronologia"), trivia: i18nT("Curiosità"),
};

function categoryLabel() {
  const cats = state.questions.map((q) => q.category).filter(Boolean);
  const mixed = i18nT("Mista");
  if (!cats.length) return mixed;
  const unique = new Set(cats);
  if (unique.size > 1) return mixed;
  return CATEGORY_LABELS[cats[0]] || mixed;
}
...
function renderOpponentHeader() {
  ...
  return `
    <div class="quiz-opponent-header">
      ${avatar}
      <span class="quiz-opponent-eyebrow">${i18nT("SFIDA A {name}", { name: escapeHtml(name.toUpperCase()) })}</span>
...
function challengeStatePill(c, isReceived, hasPlayed) {
  ...
  if (complete) {
    ...
    if (o === "tie") return statusPill(i18nT("Pareggio"), { tint: "var(--accent)", icon: "equal" });
...
function renderScoreboard() {
  ...
  let pillLabel = i18nT("Sfida in corso");
  if (o === "win") { pillIcon = ICONS.crown; pillLabel = i18nT("Vittoria"); }
  else if (o === "loss") { pillIcon = ICONS.flag; pillLabel = i18nT("Sconfitta"); }
  else if (o === "tie") { pillIcon = ICONS.equal; pillLabel = i18nT("Pareggio"); }
  return `
    <div class="quiz-scoreboard quiz-glow-card${o === "pending" ? " glow-none" : ""}">
      <div class="quiz-scoreboard-row">
        ${scoreColumn(i18nT("Tu"), myScore(), o === "win")}
...
function renderInfoRows() {
  ...
  return `
    <div class="quiz-inforows">
      <div class="quiz-inforow">${ICONS.list}<span class="lbl">${i18nT("Domande")}</span><span class="val">${c.numQuestions}</span></div>
      ...
      <div class="quiz-inforow">${c.inviteType === "external" ? ICONS.link : ICONS.people}<span class="lbl">${i18nT("Modalità")}</span><span class="val">${c.inviteType === "external" ? i18nT("Invito esterno") : i18nT("Sfida amico")}</span></div>
      ${c.sagaName ? `<div class="quiz-inforow">${ICONS.film}<span class="lbl">${i18nT("Saga")}</span><span class="val">${escapeHtml(c.sagaName)}</span></div>` : ""}
      <div class="quiz-inforow">${ICONS.filmStack}<span class="lbl">${i18nT("Categoria")}</span><span class="val">${escapeHtml(categoryLabel())}</span></div>
    </div>`;
}
  ...
  return `
    <section>
      ...
      <div class="quiz-stat-grid is-trio">
        ...
        ${statTile({ label: i18nT("Serie"), value: String(best), icon: "flame", accent: "var(--brand-warm)", sublabel: i18nT("migliore") })}
      </div>
    </section>`;
}
    ...
    return `
      <div class="quiz-review-card">
        <div class="quiz-review-top">
          <span class="quiz-review-index">${i18nT("DOMANDA {n}", { n: i + 1 })}</span>
        ...
        <div class="quiz-review-answers">
          ${answerComparisonRow(i18nT("Tu"), mineMap.get(q.questionId), q)}
