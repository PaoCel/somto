...
import { ensureUserDoc, listFriends, listFollowing, getUserPublic, searchUsersByPrefix, followUser } from "../api/users.api.js";
import { getPeopleSuggestions } from "../api/peopleSuggestions.api.js";
...
const peopleSuggestEl = qs("#peopleSuggest");
...
// --- Persone da seguire ------------------------------------------------------
// Il feed vive di archi follow, ma finora nessuna schermata ne proponeva uno:
// 23 utenti su 343 avevano seguito qualcuno. Qui mostriamo chi ha votato i tuoi
// stessi titoli, con il motivo scritto accanto.
function peopleReasonText(person) {
  // Una pagina non ha voti in comune: e' un canale, e si presenta con la sua
  // riga di descrizione. Della bio si mostra la prima frase — il resto (la
  // dichiarazione che e' un canale editoriale) sta sul profilo.
  if (person?.kind === "page") {
    const bio = String(person.bio || "").trim();
    const firstSentence = bio.split(/\.\s/)[0];
    return firstSentence ? `${firstSentence}.` : i18nT("Canale editoriale di Somto");
  }
  const count = Number(person?.sharedCount) || 0;
  const head = count === 1
    ? i18nT("1 voto in comune")
    : i18nT("{v0} voti in comune", { v0: String(count) });
  const names = Array.isArray(person?.sharedTitleNames) ? person.sharedTitleNames.filter(Boolean) : [];
  if (!names.length) return head;
  return `${head}, ${i18nT("tra cui {v0}", { v0: names.join(" e ") })}`;
}

function peopleCardHtml(person) {
  const uid = escapeHtml(person.uid);
  const name = escapeHtml(person.displayName || "");
  const href = `/user.html?uid=${encodeURIComponent(person.uid)}`;
  const initial = escapeHtml((person.displayName || "?").trim().charAt(0).toUpperCase());
  const avatar = person.photoURL
    ? `<img class="people-card-avatar" src="${escapeHtml(person.photoURL)}" alt="" loading="lazy">`
    : `<span class="people-card-avatar" aria-hidden="true">${initial}</span>`;

  const badge = person.kind === "page"
    ? `<span class="people-card-badge">${i18nT("Pagina")}</span>`
    : "";

  return `
    <div class="people-card" data-people-uid="${uid}">
      <a href="${escapeHtml(href)}" aria-label="${name}">${avatar}</a>
      <a class="people-card-name" href="${escapeHtml(href)}">${name}</a>
      ${badge}
      <div class="people-card-reason">${escapeHtml(peopleReasonText(person))}</div>
      <button class="people-card-btn" type="button" data-people-follow="1" data-uid="${uid}">${i18nT("Segui")}</button>
    </div>
  `;
}

async function renderPeopleSuggestions(myUid) {
  if (!peopleSuggestEl) return;

  const { people } = await getPeopleSuggestions({ max: 8 });
  if (!people.length) {
    peopleSuggestEl.style.display = "none";
    peopleSuggestEl.innerHTML = "";
    return;
  }

  peopleSuggestEl.innerHTML = `
    <div class="people-suggest-head">
      <span class="people-suggest-title">${i18nT("Persone da seguire")}</span>
      <span class="people-suggest-sub">${i18nT("Il feed si riempie quando segui qualcuno")}</span>
    </div>
    <div class="people-suggest-row">${people.map(peopleCardHtml).join("")}</div>
  `;
  peopleSuggestEl.style.display = "block";

  peopleSuggestEl.querySelectorAll("[data-people-follow]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        btn.textContent = i18nT("Seguito");
        btn.classList.add("is-following");
        logEvent("people_suggestion_follow", { target_uid: targetUid });
      } catch (err) {
        btn.disabled = false;
        toast(err?.message || i18nT("Operazione non riuscita"));
      }
    });
  });
}
...
async function buildHomeFeed(myUid) {
  ...
  renderPeopleSuggestions(myUid).catch(() => {});
