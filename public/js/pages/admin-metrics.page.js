...
function renderKpis(docs) {
  ...
  kpiGrid.innerHTML = [
    ...
    // Ritorno per coorte: derivato dai dati, quindi vale anche per chi usa
    // l'app iOS, dove gli eventi di funnel non arrivano.
    retentionTile(1, snap?.retention?.d1),
    retentionTile(7, snap?.retention?.d7),
    retentionTile(30, snap?.retention?.d30),
  ].join("");
}

function retentionTile(days, row) {
  const label = i18nT("Tornati dopo {count} giorni", { count: days });
  const cohort = Number(row?.cohort || 0);
  const returned = Number(row?.returned || 0);
  if (cohort < 5) {
    return kpiTileHtml(label, "—", `coorte ${fmtInt(cohort)} · attendo ≥5`);
  }
  return kpiTileHtml(
    label,
    `${Math.round((returned / cohort) * 100)}%`,
    `${fmtInt(returned)} su ${fmtInt(cohort)} iscritti`
  );
}
