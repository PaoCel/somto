...
function openReportSheet() {
  ...
  if (!q) return;
  const reasons = [
    i18nT("Risposta errata"), i18nT("Domanda ambigua"), i18nT("Più risposte plausibili"),
...
async function submitReport(reason) {
  ...
  try {
    ...
    showReportToast(i18nT("Segnalazione inviata. Grazie."));
  } catch (err) {
  ...
  return `
    <div class="quiz-play-progresshead">
      <div class="row">
        ...
        ${remaining > 0 ? `<span class="quiz-play-remaining">${i18nT("{count} restanti", { count: remaining })}</span>` : ""}
      </div>
...
function renderOutcomeBanner(q) {
  ...
  if (outcome === "pending") {
    ...
    const streak = currentStreak();
    // Due di fila senza enfasi, dal terzo in poi col punto esclamativo;
    // il quarto ha una frase sua.
    if (streak === 4) title += i18nT(" · 4 di fila, sei in fiamme!");
    else if (streak >= 2) title += ` · ${i18nT("{count} di fila", { count: streak })}${streak > 2 ? "!" : ""}`;
  ...
  root.innerHTML = `
    <div class="quiz-result-page">
      ...
      <div class="quiz-breakdown">
        ...
        <div class="quiz-breakdown-row"><span style="color:var(--brand-primary)">${ICONS.xCircle}</span><span class="lbl">${i18nT("Errate")}</span><span class="val">${wrongCount}</span></div>
        <div class="quiz-breakdown-row"><span style="color:var(--text-muted)">${ICONS.forward}</span><span class="lbl">${i18nT("Saltate")}</span><span class="val">${skippedCount}</span></div>
      </div>
    </div>
    <div class="quiz-actionbar">
