...
let searchSeq = 0;
// Nessuna sorgente in evidenza: dopo la chiusura di TV Time (15/07/2026) il
// default e' Netflix, che e' l'export che chiunque puo' ancora richiedere.
// TV Time resta una tab come le altre, per chi lo ZIP ce l'ha gia'.
...
let selectedSource = VALID_SOURCES.includes(REQUESTED_SOURCE) ? REQUESTED_SOURCE : "netflix_csv";
...
function renderTvTimePickerStep() {
  ...
  root.appendChild(el("header", { class: "imp-page-header" }, [
    ...
    el("p", {}, i18nT("Porta su Somto i film e le serie che hai visto, i voti e la watchlist.")),
  ]));
  ...
  // Niente istruzioni su come procurarsi l'export: il tool self-service di
  // TV Time e' irraggiungibile da luglio 2026, quindi dire "richiedilo" manda
  // le persone contro un muro. Qui si carica e basta, come per le altre fonti;
  // chi non ha il file lo scopre da /vieni-da-tv-time.html.
  const dropzone = el("label", { class: "imp-dropzone", for: "importZipInput" }, [
    ...
    el("small", {}, i18nT("Lo zip dell'export di TV Time, senza aprirlo né rinominarlo.")),
  ]);
