...
import { getTitleById, getTitlesByIds, updateTitle, buildTitleSearchIndex, refreshTitleFromTmdbIfNeeded, enrichTitleAssets, getRelatedTitlesUnified, listSagaTitles, createRelatedSuggestion } from "../api/titles.api.js";
import { rankRelatedTitles } from "../utils/relatedRanking.js";
import { listGenres, tmdbGenreCatalog } from "../api/genres.api.js";
import { proposeTitleEdit } from "../api/titleEdits.api.js";
import { listFriends, getUserPublic, listFollowing, listUsersPublicByIds, searchUsersByPrefix } from "../api/users.api.js";
...
import { bestStreamingDestination, streamingDestination } from "../utils/streamingDestination.js";
import {
...
// NEW
import { upsertRating, listRatingsForTitle, getMyRating, updateReviewText, migrateRatingLevel, deleteRating, getDerivedRating, listMyRecentRatingsAnyLevel } from "../api/ratings.api.js";
import { computeCompanionDefaults } from "../utils/companionDefaults.js";
...
const revWatchedWithQuick = qs("#revWatchedWithQuick");
...
const btnRelatedSeeAll = qs("#btnRelatedSeeAll");
const relatedFullModal = qs("#relatedFullModal");
const relatedFullGrid = qs("#relatedFullGrid");
const btnSuggestRelated = qs("#btnSuggestRelated");
...
const sagaCard = qs("#sagaCard");
const sagaTitle = qs("#sagaTitle");
const sagaList = qs("#sagaList");

const RELATED_PREVIEW_MAX = 8;
const RELATED_MODAL_MAX = 40;

let relatedTitles = []; // Array of related title IDs (curated, approvati)
let relatedRankedTitles = []; // Elenco completo gia' ordinato (curati + affini)
...
if (relatedFullModal) wireModalClose(relatedFullModal);
...
// Un logo, un indirizzo. Prima portavano TUTTI sulla pagina "dove guardare" di
// TMDB, anche quando il server aveva gia' risolto il link diretto: toccare
// Netflix e finire su una pagina che ti richiede di scegliere Netflix e' un
// passaggio che non serviva a niente. Gemello di `TitleWatchProvidersSection`
// su iOS, che faceva gia' cosi'.
function providerBadge(provider, fallbackHref = "", context = null) {
  const name = escapeHtml(provider?.name || i18nT("Provider"));
  const logo = String(provider?.logoUrl || "").trim();
  const resolved = context
    ? streamingDestination({
      providerName: provider?.name || "",
      deepLinks: context.deepLinks,
      titleName: context.titleName,
      tmdbId: context.tmdbId,
      isSeries: context.isSeries,
    })
    : null;
  const href = String(resolved?.url || fallbackHref || "").trim();
  const inner = `
...
function providerGroup(label, rows, fallbackHref = "", context = null) {
  if (!Array.isArray(rows) || !rows.length) return "";
  return `
    <div class="provider-group">
      <div class="provider-group-label">${escapeHtml(label)}</div>
      <div class="provider-row">
        ${rows.map((row) => providerBadge(row, fallbackHref, context)).join("")}
      </div>
    </div>
  `;
}
...
/**
 * Il contesto che serve a decidere dove porta un logo: i link diretti risolti
 * dal server piu' quelli gia' denormalizzati sul titolo, il nome (per la
 * ricerca) e l'id TMDB (per l'ultimo ripiego).
 *
 * Si UNISCONO invece di scegliere: la callable puo' aver risolto un link che
 * sul documento titolo non e' ancora stato denormalizzato, e viceversa dopo un
 * refresh del catalogo.
 */
function watchDestinationContext(payload) {
  const fromTitle = (currentTitle && typeof currentTitle.watchDeepLinks === "object" && currentTitle.watchDeepLinks) || {};
  const fromPayload = (payload && typeof payload.deepLinks === "object" && payload.deepLinks) || {};
  return {
    deepLinks: { ...fromTitle, ...fromPayload },
    titleName: String(currentTitle?.name || ""),
    tmdbId: Number(payload?.tmdbId || currentTitle?.tmdbId || 0) || null,
    isSeries: String(payload?.type || currentTitle?.type || "movie") === "tv",
  };
}

/** I nomi delle piattaforme su cui il titolo si vede qui, in ordine TMDB. */
function watchProviderNamesFrom(payload) {
  const providers = payload?.providers || {};
  const names = [];
  for (const key of ["flatrate", "free", "ads"]) {
    for (const row of Array.isArray(providers[key]) ? providers[key] : []) {
      const name = String(row?.name || "").trim();
      if (name) names.push(name);
    }
  }
  if (names.length) return names;
  return Array.isArray(currentTitle?.watchProviderNames) ? currentTitle.watchProviderNames : [];
}

/**
 * Il tasto "vai a guardarlo", gemello di `TitleWatchNowButton` su iOS.
 *
 * Compare SOLO se sappiamo aprire una piattaforma: se resta il ripiego TMDB il
 * bottone sparisce, perche' quel salto ce l'ha gia' la sezione con i loghi e un
 * bottone grosso che apre una pagina di TMDB e' la promessa che fa dire "non
 * funziona".
 */
function renderWatchNowButton(payload) {
  const btn = qs("#watchNowBtn");
  const label = qs("#watchNowLabel");
  const logo = qs("#watchNowLogo");
  if (!btn || !label) return;

  const context = watchDestinationContext(payload);
  const best = bestStreamingDestination({
    providerNames: watchProviderNamesFrom(payload),
    deepLinks: context.deepLinks,
    titleName: context.titleName,
    tmdbId: context.tmdbId,
    isSeries: context.isSeries,
  ...
  if (!best || best.source === "tmdbWatchPage") {
  ...
  // Un link diretto apre la scheda del titolo; una ricerca no. Prometterle
  // uguali e' il modo piu' rapido per far sembrare rotto un bottone che ha
  // fatto quello che poteva.
  label.textContent = best.isExact
    ? i18nT("Guarda su {provider}", { provider: best.providerName })
    : i18nT("Cerca su {provider}", { provider: best.providerName });
  btn.setAttribute("aria-label", label.textContent);

  if (logo) {
    const url = watchProviderLogoFor(best.providerName, payload);
    if (url) {
      // Un logo che non carica lascia l'icona di immagine rotta dentro il
      // bottone principale della scheda: meglio nessun logo.
...
/** Il logo della piattaforma scelta, dal payload o da quelli sul titolo. */
function watchProviderLogoFor(providerName, payload) {
  const wanted = String(providerName || "").toLowerCase();
  if (!wanted) return "";
  const providers = payload?.providers || {};
  for (const key of ["flatrate", "free", "ads", "rent", "buy"]) {
    for (const row of Array.isArray(providers[key]) ? providers[key] : []) {
      if (String(row?.name || "").toLowerCase().startsWith(wanted) && row?.logoUrl) return String(row.logoUrl);
    }
  }
  const fromTitle = Array.isArray(currentTitle?.watchProviderLogos) ? currentTitle.watchProviderLogos : [];
  const hit = fromTitle.find((row) => String(row?.name || "").toLowerCase().startsWith(wanted));
  return hit?.logoUrl ? String(hit.logoUrl) : "";
}
...
function renderWatchProviders(payload, region = "IT") {
  ...
  const groups = [
    providerGroup("Streaming", providers.flatrate, tmdbLink, context),
    providerGroup("Gratis / Ads", providers.free, tmdbLink || "", context),
    providerGroup("Noleggio", providers.rent, tmdbLink || "", context),
    providerGroup("Acquisto", providers.buy, tmdbLink || "", context),
    providerGroup("Ads", providers.ads, tmdbLink || "", context),
    customProviderGroup(customAdmin),
  ].filter(Boolean);

  renderWatchNowButton(payload);
...
async function loadWatchProviders(t, { region = "IT" } = {}) {
  ...
  // Il bottone si nasconde subito: e' l'unico elemento della pagina che
  // porterebbe fuori da Somto, e lasciarlo puntato al titolo precedente mentre
  // si carica il nuovo e' il modo piu' rapido per mandare qualcuno a guardare
  // la serie sbagliata.
  hideWatchNowButton();
...
// Cella poster+titolo condivisa tra la corsia, la griglia "Vedi altri" e la
// riga Saga: stessa card, tre contesti diversi.
function relatedItemHtml(t, { highlightCurrent = false } = {}) {
  const isCurrent = highlightCurrent && t.id === currentTitle?.id;
  return `
    <a href="/title.html?id=${encodeURIComponent(t.id)}" class="related-item${isCurrent ? " is-current" : ""}">
...
function renderRelatedInto(container, titles) {
  if (!container) return;
  container.innerHTML = (titles || []).map((t) => relatedItemHtml(t)).join("");
}

function renderSaga(sagaTitlesRows) {
  if (!sagaCard || !sagaList) return;
  if (!currentTitle?.collectionId || sagaTitlesRows.length <= 1) {
    sagaCard.style.display = "none";
    sagaList.innerHTML = "";
    return;
  }
  if (sagaTitle) sagaTitle.textContent = currentTitle.collectionName || i18nT("Saga");
  sagaList.innerHTML = sagaTitlesRows.map((t) => relatedItemHtml(t, { highlightCurrent: true })).join("");
  sagaCard.style.display = "block";
}

// Correlati unificati (curati + collaborativi + fallback per genere,
// ordinati in relatedRanking.js) + Saga. Letture: vedi il commento su
// getRelatedTitlesUnified/listSagaTitles in titles.api.js.
async function loadRelated() {
  ...
  const sagaTitlesRows = currentTitle.collectionId
    ? await listSagaTitles(currentTitle.collectionId).catch(() => [])
    : [];
  renderSaga(sagaTitlesRows);

  try {
    const unified = await getRelatedTitlesUnified(currentTitle, { uid: currentUser?.uid || null });
    relatedTitles = unified.curatedIds;
    const excludeIds = [currentTitle.id, ...sagaTitlesRows.map((t) => t.id)];
    relatedRankedTitles = rankRelatedTitles({
      curatedIds: unified.curatedIds,
      candidates: unified.titles,
      similarityById: unified.similarityById,
      featureSums: unified.featureSums,
      excludeIds,
    }).slice(0, RELATED_MODAL_MAX);
  } catch (e) {
...
function renderRelated() {
  ...
  if (!relatedRankedTitles.length) {
    relatedList.innerHTML = "";
    relatedEmpty.style.display = "block";
    if (btnRelatedSeeAll) btnRelatedSeeAll.style.display = "none";
    return;
  }

  relatedEmpty.style.display = "none";
  renderRelatedInto(relatedList, relatedRankedTitles.slice(0, RELATED_PREVIEW_MAX));

  if (btnRelatedSeeAll) {
    btnRelatedSeeAll.style.display = relatedRankedTitles.length > RELATED_PREVIEW_MAX ? "inline-flex" : "none";
  }
}

if (btnRelatedSeeAll) {
  btnRelatedSeeAll.addEventListener("click", () => {
    renderRelatedInto(relatedFullGrid, relatedRankedTitles);
    openModal(relatedFullModal);
  });
}

// Admin/trusted (canAutoApprove) scrivono direttamente su titles.related;
// tutti gli altri utenti loggati propongono via titleRelatedSuggestions
// (le rules non permettono a nessun altro l'arrayUnion diretto).
function isTrustedForRelated() {
  return !!myUserDoc?.isAdmin || canAutoApprove(myUserDoc);
}

// Apre il picker titolo condiviso da "Aggiungi" (header) e "Suggerisci un
// titolo" (modale "Vedi altri"): stessa ricerca, stesso addRelatedTitle.
...
if (btnAddRelated) {
  btnAddRelated.addEventListener("click", openRelatedPicker);
}

if (btnSuggestRelated) {
  btnSuggestRelated.addEventListener("click", () => {
    closeModal(relatedFullModal);
    openRelatedPicker();
  });
}
...
// Propone un correlato: crea titleRelatedSuggestions (auto-approvato dal
// server al terzo utente distinto). Percorso per chi NON e' admin/trusted —
// le rules non concedono a nessun altro l'arrayUnion diretto su titles.related.
async function suggestRelatedTitle(titleId) {
  try {
    await createRelatedSuggestion(currentUser.uid, currentTitle.id, titleId);
    closeModal(relatedModal);
    toast(i18nT("Grazie, suggerimento ricevuto."), i18nT("Fatto"));
  } catch (e) {
    console.error("Error suggesting related:", e);
    toast(e?.message || i18nT("Errore"), "Ops");
  }
}

// Bug fissato: il bottone "Aggiungi" era visibile a chiunque fosse loggato
// ma le rules permettono l'arrayUnion diretto solo ad admin/trusted — per
// tutti gli altri, ora, questa funzione propone invece di scrivere (403 evitato).
async function addRelatedTitle(titleId) {
  ...
  if (!isTrustedForRelated()) {
    await suggestRelatedTitle(titleId);
    return;
  }
  ...
  try {
    ...
    const addedMap = await getTitlesByIds([titleId]).catch(() => new Map());
    const addedTitle = addedMap.get(titleId);
    relatedRankedTitles = rankRelatedTitles({
      curatedIds: relatedTitles,
      candidates: addedTitle ? [addedTitle, ...relatedRankedTitles] : relatedRankedTitles,
      excludeIds: [currentTitle.id],
    });
    renderRelated();
...
async function createNewTitleForRelated(titleName) {
  ...
  const isTrusted = isTrustedForRelated();
...
let _revFollowing = null;      // cache seguiti (uid+displayName), condivisa con "Suggerisci titolo"
let _revWWSelected = [];       // persone taggate nella review corrente
let _revWWAlone = false;       // chip "Da solo" selezionata
let _revWWSearchOpen = false;  // ricerca inline "Cerca una persona" aperta
let _revWWSearchResults = [];  // ultimi risultati searchUsersByPrefix (ricerca globale, non solo seguiti)
let _revCompanionDefaults = { preselectAlone: false, frequentCompanions: [] }; // companionDefaults.js sui rating recenti
let _revRecentRatingsCache = null; // input grezzo per companionDefaults, una fetch per sessione pagina
...
async function openReviewModal(ratingValue) {
  ...
  // Reset condivisione di gruppo (deselezionata di default). La discussione
  // pubblica non e' piu' una casella: la review ci va sempre.
  ...
  // "Con chi l'hai visto?": chips già taggate + chip rapide (Da solo /
  // persone frequenti) + ricerca su tutti gli utenti, aperta a richiesta.
  // I default si calcolano in background dai rating recenti: il modale si
  // apre subito, senza aspettarli.
  const titleIdAtOpen = currentTitle.id;
  ...
  ensureReviewRecentRatingsLoaded().then((ratings) => {
    if (currentTitle?.id !== titleIdAtOpen) return; // titolo cambiato mentre calcolavamo i default
    _revCompanionDefaults = computeCompanionDefaults(ratings, { mediaType: currentTitle.type });
    // Non forzare "Da solo" sopra a tag gia' presenti (review esistente con
    // persone taggate): il default vale solo a mani vuote.
    if (_revCompanionDefaults.preselectAlone && !_revWWSelected.length) {
      _revWWAlone = true;
    }
    renderRevWWQuick();
  }).catch(() => {});
...
/**
 * Seguiti dell'utente (uid+displayName), condivisa per sessione. La review
 * NON la usa piu' per "Con chi l'hai visto?" (chip rapide + ricerca globale,
 * vedi sotto) — resta per "Suggerisci titolo", che la riusa cosi' com'e'.
 */
...
/**
 * Rating recenti (qualunque livello) dell'utente corrente, arricchiti con il
 * `type` del titolo — input di companionDefaults.js. Una fetch sola per
 * sessione di pagina: i default sono un suggerimento, non serve rileggerli
 * ad ogni apertura del modale.
 */
async function ensureReviewRecentRatingsLoaded() {
  if (Array.isArray(_revRecentRatingsCache)) return _revRecentRatingsCache;
  if (!currentUser) return [];
  const ratings = await listMyRecentRatingsAnyLevel(currentUser.uid, { max: 60 }).catch(() => []);
  const titleIds = Array.from(new Set(ratings.map((r) => String(r?.titleId || "").trim()).filter(Boolean)));
  const titlesById = titleIds.length ? await getTitlesByIds(titleIds).catch(() => new Map()) : new Map();
  _revRecentRatingsCache = ratings.map((r) => ({
    watchedWith: r.watchedWith,
    createdAt: r.createdAt,
    type: titlesById.get(String(r?.titleId || "").trim())?.type || null,
  }));
  return _revRecentRatingsCache;
}
...
// Chip rapide sopra la ricerca: "Da solo" (preselezionata quando i pattern
// recenti lo suggeriscono, companionDefaults.js), le persone piu' frequenti
// (gia' taggate restano fuori — le rimuovi dalla X sulla chip sopra) e il
// toggle "Cerca una persona" che apre la ricerca inline.
function renderRevWWQuick() {
  if (!revWatchedWithQuick) return;
  const selected = new Set(_revWWSelected.map((p) => p.uid));
  const frequent = (_revCompanionDefaults?.frequentCompanions || []).filter((c) => !selected.has(c.uid));

  const aloneChip = `
    <button type="button" class="rev-ww-quick-chip${_revWWAlone ? " is-selected" : ""}" data-quick-action="alone" aria-pressed="${_revWWAlone}">${escapeHtml(i18nT("Da solo"))}</button>`;

  const frequentLabel = frequent.length
    ? `<span class="rev-ww-quick-label">${escapeHtml(i18nT("Persone frequenti"))}</span>`
    : "";
  const frequentChips = frequent.map((c) => `
    <button type="button" class="rev-ww-quick-chip" data-quick-uid="${escapeHtml(c.uid)}" data-quick-name="${escapeHtml(c.displayName)}">${escapeHtml(c.displayName)}</button>`).join("");

  const searchChip = `
    <button type="button" class="rev-ww-quick-chip rev-ww-search-toggle${_revWWSearchOpen ? " is-selected" : ""}" data-quick-action="search" aria-pressed="${_revWWSearchOpen}">${escapeHtml(i18nT("Cerca una persona"))}</button>`;

  revWatchedWithQuick.innerHTML = aloneChip + frequentLabel + frequentChips + searchChip;
}

function toggleRevWWAlone() {
  _revWWAlone = !_revWWAlone;
  if (_revWWAlone && _revWWSelected.length) {
    _revWWSelected = [];
    renderRevWWChips();
  }
  renderRevWWQuick();
...
function toggleRevWWSearchOpen() {
  _revWWSearchOpen = !_revWWSearchOpen;
  if (revWatchedWithSearch) {
    revWatchedWithSearch.style.display = _revWWSearchOpen ? "" : "none";
    if (_revWWSearchOpen) {
      revWatchedWithSearch.value = "";
...
// Ricerca inline "Cerca una persona": su TUTTI gli utenti (searchUsersByPrefix,
// stessa funzione della ricerca profili), non piu' solo sui seguiti. Debounced
// per non interrogare Firestore ad ogni tasto; un token scarta risposte
// arrivate fuori ordine (query digitata e poi cambiata prima che risponda).
let _revWWSearchTimer = null;
let _revWWSearchToken = 0;

function scheduleRevWWSearch() {
  if (_revWWSearchTimer) clearTimeout(_revWWSearchTimer);
  const q = String(revWatchedWithSearch?.value || "").trim();
  if (q.length < 2) {
    _revWWSearchResults = [];
    renderRevWWResults();
    return;
  }
  const token = ++_revWWSearchToken;
  _revWWSearchTimer = setTimeout(async () => {
    const rows = await searchUsersByPrefix(q, { max: 8 }).catch(() => []);
    if (token !== _revWWSearchToken) return; // query superata da una piu' recente
    _revWWSearchResults = rows
      .filter((u) => u.uid !== currentUser?.uid)
      .map((u) => ({ uid: u.uid, displayName: String(u?.displayName || "").trim() || u.uid }));
    renderRevWWResults();
  }, 250);
}

function renderRevWWResults() {
  if (!revWatchedWithResults) return;
  const q = String(revWatchedWithSearch?.value || "").trim();
  if (!_revWWSearchOpen || q.length < 2) {
    revWatchedWithResults.style.display = "none";
    revWatchedWithResults.innerHTML = "";
    return;
  }

  const selected = new Set(_revWWSelected.map((p) => p.uid));
  const matches = (_revWWSearchResults || []).filter((p) => !selected.has(p.uid));

  revWatchedWithResults.style.display = "block";
  revWatchedWithResults.innerHTML = matches.length
    ? matches.map((p) => `<button type="button" class="rev-ww-result" data-uid="${escapeHtml(p.uid)}" data-name="${escapeHtml(p.displayName)}">${escapeHtml(p.displayName)}</button>`).join("")
    : `<div class="rev-ww-empty">${i18nT("Nessuna persona trovata")}</div>`;
}

function addRevWatchedWithUser(person) {
  ...
  _revWWAlone = false;
  _revWWSelected = [..._revWWSelected, { uid, displayName: String(person?.displayName || "").trim() || uid }];
  renderRevWWChips();
  renderRevWWQuick();
  if (revWatchedWithSearch) {
    revWatchedWithSearch.value = "";
...
revWatchedWithSearch?.addEventListener("input", () => scheduleRevWWSearch());
revWatchedWithResults?.addEventListener("click", (e) => {
  const btn = e.target.closest?.(".rev-ww-result");
  if (btn?.dataset?.uid) addRevWatchedWithUser({ uid: btn.dataset.uid, displayName: btn.dataset.name });
});
...
revWatchedWithQuick?.addEventListener("click", (e) => {
  const actionBtn = e.target.closest?.("[data-quick-action]");
  if (actionBtn) {
    if (actionBtn.dataset.quickAction === "alone") toggleRevWWAlone();
    else if (actionBtn.dataset.quickAction === "search") toggleRevWWSearchOpen();
    return;
  }
  const companionBtn = e.target.closest?.("[data-quick-uid]");
  if (companionBtn?.dataset?.quickUid) {
    addRevWatchedWithUser({ uid: companionBtn.dataset.quickUid, displayName: companionBtn.dataset.quickName });
  }
});
...
async function submitReview() {
  ...
  try {
    ...
    // La review va nella discussione pubblica del titolo, senza spuntare
    // niente. Ma solo se e' NUOVA o CAMBIATA: se no ogni ritocco del voto
    // (stelle, foto, un tag in piu') sparava un altro messaggio nel thread.
    const previousReviewText = String(_revExistingRating?.reviewText || "").trim();
    if (text && text !== previousReviewText) {
  ...
  return `
    <div class="ep-row${seenByMe ? " is-seen" : ""}${isLastWatched ? " is-current" : ""}" data-season="${escapeHtml(String(seasonNumber))}" data-episode="${escapeHtml(String(ep))}" data-seen="${seenByMe ? "1" : "0"}">
      ...
      <div class="ep-row-actions">
        ...
        <button class="ep-row-seen${!seenByMe && unaired ? " is-unaired" : ""}" type="button" ${!seenByMe && unaired ? "disabled" : ""} aria-label="${seenByMe ? i18nT("Visto, tocca per togliere") : (unaired ? i18nT("Inedito") : i18nT("Segna visto fino a qui"))}">
          ${seenByMe
...
async function loadTitleNews(t) {
  ...
  // Eventi "in prestito" da un altro titolo della saga (linkedTitleIds):
  // batch unico per risolvere il nome della sorgente, mostrato come "Da {nome}".
  const sourceTitleIds = Array.from(new Set(
    events.filter((e) => e.fromOtherTitle && e.titleId).map((e) => e.titleId)
  ));
  const sourceTitlesById = sourceTitleIds.length
    ? await getTitlesByIds(sourceTitleIds).catch(() => new Map())
    : new Map();
  ...
  try {
    ...
    const eventRows = events.map((item) => {
      ...
      const sourceTitleName = item.fromOtherTitle ? sourceTitlesById.get(item.titleId)?.name : null;
      return `<${tag} ${attrs} id="title-update-${escapeHtml(item.id)}" class="title-news-row title-update-event${item.id === eventFocus ? " is-focused" : ""}">` +
        ...
        (sourceTitleName ? `<span class="title-news-source">${escapeHtml(i18nT("Da {name}", { name: sourceTitleName }))}</span>` : "") +
...
// Titoli creati senza castWithCharacters (import minimale, non ancora
// passati dalla enrichTitleAssets che iOS chiama sempre) mostravano "Cast non
// disponibile" nel picker voti personaggi anche quando la pagina pubblica
// SSR aveva gia' il cast piatto. Un tentativo silenzioso, una volta sola per
// titolo/apertura pagina: se arriva, si fonde in currentTitle e si ridisegna
// solo la sezione cast (i picker review/episodio leggono da currentTitle al
// momento dell'apertura, quindi non serve altro).
let _castEnrichAttemptedForTitleId = null;

async function maybeEnrichCastAssets(t) {
  if (!t?.id || !currentUser) return; // la callable richiede login
  if (_castEnrichAttemptedForTitleId === t.id) return;
  const hasCast = Array.isArray(t.castWithCharacters) && t.castWithCharacters.length > 0;
  const tmdbId = Number(t.tmdbId || t.meta?.tmdbId || 0);
  if (hasCast || !tmdbId) return;

  _castEnrichAttemptedForTitleId = t.id;
  try {
    const result = await enrichTitleAssets(t.id, {
      includeCast: true,
      includeTrailer: !String(t.trailerUrl || "").trim(),
      tmdbId,
      mediaType: String(t.type || "").toLowerCase() === "tv" ? "tv" : "movie",
    });
    const cast = Array.isArray(result?.castWithCharacters) ? result.castWithCharacters : [];
    if (!cast.length) return;
    if (currentTitle?.id !== t.id) return; // titolo cambiato mentre la callable era in volo

    currentTitle = { ...currentTitle, castWithCharacters: cast };
    if (result.trailerUrl && !currentTitle.trailerUrl) currentTitle.trailerUrl = result.trailerUrl;
    void renderCastSection(currentTitle);
  } catch (err) {
    console.warn("[title] maybeEnrichCastAssets failed", err?.message || err);
  }
}
