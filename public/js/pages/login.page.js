...
import { ensureUserDoc, getUserPublic, updateMyDisplayName } from "../api/users.api.js";
import { completeVerifiedSignup, getSignupState, registerPendingSignup } from "../api/signup.api.js";
...
import { getConsentState } from "../consent.js";
import { buildSignupAttribution } from "../utils/signupAttribution.js";
...
const verificationPanel = qs("#verificationPanel");
const verificationEmail = qs("#verificationEmail");
const verificationStatus = qs("#verificationStatus");
const btnCheckVerification = qs("#btnCheckVerification");
const btnResendVerification = qs("#btnResendVerification");
const btnChangeVerificationEmail = qs("#btnChangeVerificationEmail");
const btnExitVerification = qs("#btnExitVerification");
...
let localSignupInProgress = false;
let pendingRegistrationPayload = null;
let resendAvailableAt = 0;
const LEGACY_VERIFICATION_PROMPT_KEY = "somto.legacyVerificationPrompt";

function isPasswordUser(user) {
  return (user?.providerData || []).some((row) => row?.providerId === "password");
}

function currentSignupAttribution() {
  const consent = getConsentState();
  return buildSignupAttribution({
    pageUrl: window.location.href,
    referrer: document.referrer,
    language: navigator.language,
    analyticsConsent: consent.source === "unset" ? null : consent.analytics === true,
  });
}

function showVerificationState(user, message = "") {
  if (!verificationPanel) return;
  pendingRegistrationPayload = pendingRegistrationPayload || null;
  verificationEmail.textContent = maskEmail(user?.email || "");
  verificationStatus.textContent = message;
  verificationPanel.hidden = false;
  authForm.hidden = true;
  socialCard.hidden = true;
  document.querySelector(".auth-links")?.setAttribute("hidden", "");
  if (authHint) authHint.hidden = true;
  btnCheckVerification?.focus({ preventScroll: true });
}
...
function hideVerificationState() {
  ...
  document.querySelector(".auth-links")?.removeAttribute("hidden");
  if (verificationStatus) verificationStatus.textContent = "";
}

async function signupStateSafe() {
  try { return await getSignupState(); } catch (err) {
    console.error("[signup] state unavailable", err);
    return null;
  }
}

async function isLegacyUnverifiedUser(user, state) {
  if (state?.legacyEmailUnverified === true) return true;
  if (state?.lifecycleStatus === "legacy_unverified" && state?.pendingStatus !== "pending") return true;
  if (state) return false;
  return Boolean(await getUserPublic(user.uid).catch(() => null));
}

function queueLegacyVerificationPrompt(user) {
  try {
    sessionStorage.setItem(LEGACY_VERIFICATION_PROMPT_KEY, JSON.stringify({ uid: user.uid }));
  } catch (_) {
    // Il prompt e' best-effort: il login legacy non dipende dallo storage web.
  }
}

async function ensurePendingRegistration() {
  if (!pendingRegistrationPayload) return;
  await registerPendingSignup(pendingRegistrationPayload);
}

async function completeCurrentSignup(user) {
  await refreshSignupVerification(user);
  if (!user.emailVerified) return false;
  await ensurePendingRegistration();
  const completed = await completeVerifiedSignup({ attribution: currentSignupAttribution() });
  if (completed?.pendingCompleted === true) {
    void logEvent("signup_completed", { provider: "email", has_display_name: true });
    trackProductEvent("signup_completed");
  }
  pendingRegistrationPayload = null;
  return completed?.status === "active";
}
...
onAuth(async (user) => {
  if (user) {
    ...
    if (isPasswordUser(user) && !user.emailVerified) {
      if (localSignupInProgress) {
        showVerificationState(user);
        return;
      }
      const state = await signupStateSafe();
      const isLegacy = await isLegacyUnverifiedUser(user, state);
      if (!isLegacy) {
        showVerificationState(user);
        return;
      }
    ...
    if (isPasswordUser(user) && user.emailVerified) {
      const state = await signupStateSafe();
      if (state && (!state.profilePresent || state.pendingStatus === "pending")) {
        try { await completeCurrentSignup(user); } catch (err) { console.error(err); }
      }
    }
    hideVerificationState();
  ...
  runWithButtonLoading(btnSubmit, async () => {
    ...
    try {
      ...
      if (isPasswordUser(user) && !user.emailVerified) {
        ...
        const isLegacy = await isLegacyUnverifiedUser(user, state);
        if (!isLegacy) {
          showVerificationState(user, i18nT("Verifica l'email per continuare."));
          return;
        }
        queueLegacyVerificationPrompt(user);
      }
      if (isPasswordUser(user) && user.emailVerified) {
        const state = await signupStateSafe();
        if (state && (!state.profilePresent || state.pendingStatus === "pending")) {
          await completeCurrentSignup(user);
        }
      }
  ...
  runWithButtonLoading(btnSubmit, async () => {
    ...
    try {
      ...
      pendingRegistrationPayload = {
        displayName: checkedName.displayName,
        ageConfirmed: true,
        communitySafetyAccepted: true,
        communitySafetyAcceptedSource: "web_signup",
        attribution: currentSignupAttribution(),
      };
      await ensurePendingRegistration();
      await sendSignupVerification(user);
      resendAvailableAt = Date.now() + 60_000;
      showVerificationState(user, i18nT("Link inviato. Controlla anche lo spam."));
    } catch (err) {
      console.error(err);
      const msg = mapAuthError(err);
      showMessage(msg, "error");
...
async function finishSocialLogin(user, provider) {
  ...
  if (!user.emailVerified) {
    const err = new Error("social-email-not-verified");
    err.code = "auth/email-not-verified";
    throw err;
  }
  const state = await signupStateSafe();
  if (state && !state.profilePresent) {
    await completeVerifiedSignup({
      displayName: inferDisplayNameForSocial(user),
      communitySafetyAcceptedSource: "web_signup",
      attribution: currentSignupAttribution(),
    });
  }
...
btnCheckVerification?.addEventListener("click", () => {
  runWithButtonLoading(btnCheckVerification, async () => {
    try {
      const user = await refreshSignupVerification();
      if (!user.emailVerified) {
        verificationStatus.textContent = i18nT("Email non ancora verificata.");
        return;
      }
      await completeCurrentSignup(user);
      toast(i18nT("Email verificata."), i18nT("Benvenuto"), { type: "success" });
      window.location.href = nextUrl || "/home.html";
    } catch (err) {
      console.error(err);
      verificationStatus.textContent = i18nT("Verifica completata. Riprova tra poco.");
    }
  }, { loadingLabel: i18nT("Controllo...") });
});

btnResendVerification?.addEventListener("click", () => {
  runWithButtonLoading(btnResendVerification, async () => {
    const remainingSeconds = Math.ceil((resendAvailableAt - Date.now()) / 1000);
    if (remainingSeconds > 0) {
      verificationStatus.textContent = i18nT("Puoi inviare di nuovo tra {seconds} secondi.", { seconds: remainingSeconds });
      return;
    }
    try {
      await ensurePendingRegistration();
      ...
      resendAvailableAt = Date.now() + 60_000;
      verificationStatus.textContent = i18nT("Link inviato. Controlla anche lo spam.");
    } catch (err) {
      console.error(err);
      verificationStatus.textContent = mapAuthError(err);
    }
  }, { loadingLabel: i18nT("Invio...") });
});

btnChangeVerificationEmail?.addEventListener("click", async () => {
  const nextEmail = (window.prompt(i18nT("Inserisci la nuova email:")) || "").trim();
  if (!nextEmail) return;
  try {
    const user = await changePendingSignupEmail(nextEmail);
    resendAvailableAt = Date.now() + 60_000;
    showVerificationState(user, i18nT("Link inviato alla nuova email."));
  } catch (err) {
    console.error(err);
    verificationStatus.textContent = mapAuthError(err);
  }
...
btnExitVerification?.addEventListener("click", async () => {
  await logout();
  pendingRegistrationPayload = null;
  setMode("signIn");
});
...
function mapAuthError(e){
  ...
  if (code.includes("auth/email-not-verified")) return i18nT("Il provider non ha verificato l'email.");
