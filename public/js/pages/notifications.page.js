...
function mapNotificationView(notif, userMap) {
  ...
  if (importPresentation) {
    ...
    // Il titolo è già la novità più forte della settimana: ripeterlo in un
    // sopratitolo generico ruberebbe la riga a un nome di serie.
    icon = "🎞️";
    // Sopra il riassunto, sotto le altre novità per nome: contarle ("2 novità
    // questa settimana") non diceva quali fossero, e restavano invisibili.
    title = String(notif?.data?.message || i18nT("Novità sui tuoi titoli"));
    text = String(notif?.data?.preview || "");
